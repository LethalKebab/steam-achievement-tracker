...
* **家庭共享的游戏一样会被记录**：你用家庭成员的库玩过的游戏，会在下次同步时自动加进来并标上「家庭」，哪怕 Steam 并不把它算在你名下。
* **退款的游戏要自己加回来**：一旦退款，Steam 就不再把它算作你的，程序也就不会自动收录。在面板的搜索框里搜它的名字，Steam 商店的结果会列在你自己的游戏下面，点一下即可把这一行加回去。
* **Steam 不再提供进度的游戏**：同步会直接把它们点出来。家里也没有别人拥有的退款游戏，会停在最后一次读到的数字上，此后由你手动维护这一行。
...
面板会估算每个未完成游戏做到 100% 还需要多少小时，以及做满它能给你的平均完成率涨多少。按这一列排序，每小时收益最高的排在最前面。

* **估算会考虑剩下的是什么**，而不只是剩多少。你没解锁的成就，恰恰是大多数人也没解锁的那些，所以它们按稀有程度加权，而不是按个数平摊。在这个库上，所有未完成游戏的估算都因此变长了，其中卡在 90% 的那些涨得最多 —— 剩下的正是最难啃的部分。
* **给区间而不是假精度**：把鼠标停在数字上会显示它来自的范围，因为不同玩家的实际耗时本来就差这么多。样本太少的数字会显示为灰色，而不是当成定论。
* **时长来自 HowLongToBeat**，靠 Steam ID 而非游戏名匹配，所以不会把续作认成你手上这一部。它没收录的游戏就不显示数字，也不会反复去问。
