...
* **退款的游戏要自己加回来**：一旦退款，Steam 就不再把它算作你的，程序也就不会自动收录。在面板的搜索框里搜它的名字，Steam 商店的结果会列在你自己的游戏下面，点击即可把这一行加回去。
...
面板会估算每个未完成游戏做到 100% 还需要多少小时，以及解锁它的全部成就能给你的平均完成率涨多少。按这一列排序，每小时收益最高的排在最前面。

* **估算看的是剩下的成就有多难，不只是有多少**：你没解锁的，往往也是大多数人没解锁的，所以越稀有就按越费时来算。卡在 90% 的游戏，通常比这个百分比看起来离终点更远。
