...
面板会显示每个未完成游戏做到 100% 需要多久，「还差 23 个成就」不再是一个没法据此行动的数字。按这一列排序，用时最短的排在最前面。

* **给的是区间，不是一个数**：一个游戏显示「7h」，下面跟着「5h–12h」，因为不同玩家的实际耗时本来就差这么多。样本太少的数字会显示为灰色，而不是当成定论。
* **你还没拿到的最稀有成就**会显示在时长旁边，仅在全球解锁率极低时出现。短游戏里也可能藏着一个单独就要耗掉整个周末的成就，这是时长数字表达不了的。
