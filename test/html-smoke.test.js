...
describe('the wording in the confirmation dialog', () => {
  ...
  test('the tiers carry labels only', () => {
    // **Pinned so it stays a decision.** The tiers used to carry a sentence each, and the fast one
    // named the measured cost (9 of 16 entries came back as template sentences). Stacked
    // explanations turned a two-click confirmation into something to read, and the owner cut them.
    // The difference is not lost, it moved: docs/configuration.md carries the measured table in
    // full. A dialog is not where someone reads.
  ...
  test('every choice in the dialog really travels with the request', () => {
    ...
    // every run still uses the default. **Each control added to these dialogs belongs here**
    assert.match(js, /startGuideGen\(appid, false, choice\.value, null\)/, 'the generate path');
...
describe('switching AI vendor', () => {
  ...
  test('switching vendor first clears the key already typed in', () => {
    ...
    // to Anthropic, and without clearing, that string is stored as the Anthropic key. It is certain to
 ...
 *
 * Once every vendor shares one input, "pasting one vendor's key into another vendor's field" is a
 * mistake this revision **newly created**, and a hardcoded `sk-...` is wrong for Anthropic — far
 * from stopping it, it endorses the mistake. Reported by a user.
 */
describe('the AI key placeholder', () => {
  const html = read('Setup.html');

  test('the static markup hardcodes no vendor shape', () => {
    // For the instant before the JS starts, empty is better than wrong. Hardcoding one vendor's
    // shape means every other vendor is being lied to
  ...
  test('every vendor has its own shape, and they differ from one another', () => {
