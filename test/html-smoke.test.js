...
describe('确认框里的推理强度', () => {
  ...
  test('整篇那一支必须保留"会失去什么",哪怕正文别的都删光了', () => {
    /**
     * **这一句是这个确认框存在的理由,不是旁白。**
     *
     * 手动勾上的子步骤框是整篇重写唯一真正不可挽回的损失:成就框会被机械打勾按
     * 数据库勾回原样,子步骤匹配不到任何成就,重新生成后一律变回未勾选。它推不出来、
     * 别处也没写,而人正是在这一屏决定要不要付这个代价。
     *
     * 钉它是因为**这个正文已经被删过两轮**(先删了时长,再删了"整份替换"和"先备份"),
     * 每一轮的理由都成立,而下一轮很容易顺手把它一起删掉 —— 那时候丢的不是一句废话。
     *
     * **只能钉在源码上:** 真要渲染它,需要一份带手动勾选子步骤的攻略,而当前语料里
     * 一份都没有(checkbox-sync 只勾成就框,级联在 serve 那条路默认关着),
     * 两款抽查下来 atRisk 都是 0。
     */
    assert.match(js, /p\.atRisk\s*\n?\s*\?/, '整篇那一支要按 atRisk 分支');
    assert.match(js, /个手动勾的子步骤会变回未勾选/, '损失必须说出来');
  });
