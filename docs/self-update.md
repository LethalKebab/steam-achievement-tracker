...
- 替换步骤在本地用假 app 目录排练过 —— 目录里**故意混着** `config.json` / `data/steam.db` / 手写攻略 / `local.config.json`,两条路径(有清单、无清单)各跑一遍:程序文件按预期换掉,用户数据一个字节没动。这验证的是脚本的逻辑,不是真实环境。
- **还没做:两个真实发布之间的排练。** 需要两个都带清单的版本,所以最早也要等 1.1.4 和 1.1.5 都发出去。在那之前,**把第一次真实的自更新当作验证步骤** —— 更新完打开程序文件夹看一眼,别只看程序能不能启动。
- 生成出来的 PowerShell 会被 `[Parser]::ParseFile` 解析一遍(只在 Windows 上跑)。这条不是形式主义:那段脚本在一个 detached、没有控制台的进程里执行,语法错误不会被任何人看见,表现只是「程序自己退了,再也没起来」。
