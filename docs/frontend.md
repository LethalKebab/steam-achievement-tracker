...
**A thin sample marks the hours rather than hiding them.** Under ten completionist reports the figure is one stranger's playthrough — not wrong, unsupported. Marking it is what "treat this as a hint" looks like; hiding throws away the only estimate there is.

**The mark is a dotted underline, and it must never again be anything that carries lightness.** The tier below encodes value per hour as a lightness ramp, so "how good is this" and "how well supported is this" would be riding the same channel — and the second one wins. Measured on the `opacity: 0.5` version that shipped: half-opacity moved a step **0.32** in relative luminance while the entire four-tier ramp spans **0.27**, so a top-quarter row with four reports came out darker than a bottom-quarter row with ninety. It also put all four tiers under AA (3.07 / 2.64 / 2.25 / 1.95). Two facts, one channel, and the channel read as the wrong fact.

A dotted underline is a texture, so the tier keeps lightness to itself. It is not an invented mark: it is what browsers draw under `<abbr title>`, meaning "this term is annotated, the explanation is on hover" — which is the meaning wanted, and the cell already carries a `title`. It inherits `currentColor`, so it rides on the tier's colour instead of introducing a second one. Two things hold it up, and both are pinned in `html-smoke.test.js`: the rule goes on **`.cost-thin .cost-mid`**, never on `.cost-cell`, because `text-decoration` inherits into descendants and cannot be switched off by a child, so on the cell it would strike the gain line too; and nothing sets `cursor: pointer`, because a **solid** underline already means "link" on this page and a mark that reads as clickable would be a third meaning on one number.

**A mark that says nothing is noise, so the tooltip explains it.** `table.costThin` is the only thing on screen saying the figure is thinly supported, and it has to spend its one line on what is true of *this* row. An earlier wording ended 「估算仅供参考」 — equally true of the 92 rows carrying no mark, so half the tooltip said nothing the mark did not already say. **Every figure in this column is an estimate; what a marked row adds is that the median rests on a handful of playthroughs and can sit a long way from typical.** The ten-report threshold and the per-game count are both left out: neither is on screen, and neither changes what the reader does with the number. When a row has both a range and a thin sample the two compose on separate lines rather than one replacing the other.
...
**The colour is the ratio, and the two numbers are only its halves.** 「23h · +0.44pp」 beside 「9.6h · +0.04pp」 does not say which is the better hour — the first is four times the better. The sort knew it and the cell did not, so each row is painted by which quarter of the library its value per hour falls in: `--cost-1` (brightest, best quarter) through `--cost-4` (dimmest, worst).

**One hue, monotone lightness — not the progress bar's four colours.** An earlier version painted this column with `--prog-done`/`--prog-high`/`--prog-mid`/`--text-3`, reused because the tokens existed, not because a value-per-hour quartile is the same kind of thing as a completion state. Blue already means "100% done" on the rate column; repainting it here for "best value per hour" made one colour mean two things depending which column a reader was in. A quartile rank is ordinal, so it gets a single-hue lightness ramp instead (`--cost-1..4`, derived in the `:root` comment) — dimmer reads as worse, with no second hue to relearn. **That is exactly why nothing else in this cell may move lightness**, which is what the thin-sample marker above got wrong for as long as it used opacity.
...
**Every tier clears AA on both surfaces a row is drawn on, and the hover surface is the binding one.** `tr.game-row:hover td` repaints the cell on `--surface-2`, which an earlier derivation never checked: it recorded 8.02 / 6.33 / 4.96 / 3.84 against `--surface` and called only the last a deliberate exception, while on any hovered row the bottom two were 4.31 and 3.34. The ramp is now 8.02 / 7.03 / 6.13 / 5.19 and 6.98 / 6.11 / 5.33 / 4.51. **Check both, and compute them rather than copying a figure into a comment** — `html-smoke.test.js` reads the tokens and does the arithmetic, so a nudged token cannot leave a stale number behind.

**The AA floor is what fixes the range, and four steps have to fit inside it.** Holding 4.5:1 on the hover surface puts the dimmest step near L 0.53, leaving about 0.15 of HSL lightness below `--accent`; three gaps of the 0.06 an earlier revision asked for need 0.18, so that criterion and four tiers cannot both hold. The criterion gives, because **HSL lightness is not perceptually uniform** and 0.06 of it does not mean the same thing at both ends of a ramp. In CIELAB the adjacent steps are ΔL\* 4.3 / 4.4 / 5.0, roughly twice the ~2.3 just-noticeable difference. If a future change needs wider separation than that, the honest move is **three tiers, not a dimmer fourth** — terciles fit the same range with half again the gap.

**The dimmest tier has to stay clearly blue against `--text-3`.** `.cost-gain` uses that token one line below inside the same cell. A more muted bottom step was derived first and landed at ΔE 8.8 from it — close enough to read as the same colour as the line under it, flattening the cell's own hierarchy while every contrast figure still passed. The shipped ramp holds ΔE 23–25 across all four.
