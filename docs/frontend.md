...
- **An achievement it cannot attribute says 「攻略里还没写这条」 and keeps the search link** — chosen over silence because the blanks are the actionable half: each uncovered achievement says so on its own card, which makes the panel a coverage map that previously needed `guide-lint` across the whole corpus to produce. **The header does not also count them** — it states `还差 8 个成就` and stops, because a tally of what the cards below already show is the header narrating the panel rather than adding to it.
...
- **`.ach-grid .ach-item.row-expanded { align-self: start }`** — `stretch` would otherwise drag an expanded card's row-mates to its full height, mostly empty. **The class belongs to the card, and `markExpandedRows()` puts it only on the row that holds the expanded guide.** It began on the grid, which meant every card in it: expanding one guide in the fourth row pulled the first three apart as well, though a grid row's height is its own tallest card's and those rows had not moved at all. CSS cannot say "the row this card is in", so the row is counted out as `Math.floor(index / columnCount)`, and `auto-fill` makes that count change with the window, so a single `resize` listener re-marks. **That listener is registered once at startup and looks the grid up when it fires** — attached beside the grid it would be left behind by every repaint, since `render()` rebuilds the row's HTML.
