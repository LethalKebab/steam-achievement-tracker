...
**A mark that says nothing is noise, so the tooltip explains it.** `table.costThin` is the only thing on screen saying the figure is thinly supported. It says why the number is soft and deliberately not how soft — the per-game count is not on screen, and naming the ten-report threshold there would add a number the reader cannot act on. When a row has both a range and a thin sample the two compose on separate lines rather than one replacing the other.
