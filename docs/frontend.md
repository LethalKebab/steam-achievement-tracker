...
**One hue, monotone lightness — not the progress bar's four colours.** An earlier version painted this column with `--prog-done`/`--prog-high`/`--prog-mid`/`--text-3`, reused because the tokens existed, not because a value-per-hour quartile is the same kind of thing as a completion state. Blue already means "100% done" on the rate column; repainting it here for "best value per hour" made one colour mean two things depending which column a reader was in. A quartile rank is ordinal, so it gets a single-hue lightness ramp instead (`--cost-1..4`, derived and validated in the `:root` comment) — dimmer reads as worse, with no second hue to relearn.
