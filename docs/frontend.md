...
- **An achievement it cannot attribute says 「攻略里还没写这条」 and keeps the search link** — chosen over silence because the blanks are the actionable half: the header reads `44 个成就,还差 8 个 · 攻略里写了其中 8 个`, which makes the panel a coverage map that previously needed `guide-lint` across the whole corpus to produce.
