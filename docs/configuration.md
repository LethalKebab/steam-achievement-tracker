...
```jsonc
{
  ...
  "hltbRefreshBudget": 40,    // how many resolved games re-read their hours per run
...
Every tick is written to `sync_log` just like the manual command's, so `node tracker.js log 30` is the review path; the Dashboard also shows a notice naming the first few. It doesn't auto-dismiss — it is reporting a write to your own notes — and stays until you close it with the × in its corner. A Notion failure here is soft — it shows as its own notice and never turns into "sync failed", since the achievement data synced fine. Set to `false` to tick only when you run the command yourself.
...
**`effort` decides how much research goes into a guide, and it is the only setting here that changes how long a run takes.** It governs the rounds that write the guide. The one pass that does not follow it is the tidy-up that regroups the finished sections: it writes no prose and runs shallow whatever you pick, after a run at **Thorough** (「深度模式」) spent its entire token budget thinking about the grouping and produced no answer at all. You can set it three ways, in increasing order of scope: pick it in the confirmation that appears before each generation on the Dashboard; pass `--effort low` to `guide-gen` or `ai-check`; or change this value to move the default.
...
The dollar half never worked honestly. Rates change, there was no verified table for DeepSeek — the provider you're most likely to use — and **how server-side web search is billed was never measured at all**. So a "$5 cap" was a number computed from an amount nobody could stand behind. A cap you can't trust is worse than no cap: it reads as protection while protecting nothing.
...
**After any run, `ai-check` reports the search queries the model actually issued.** Declaring a web tool and getting zero searches back is the real answer to "does my key include grounding", and it is more reliable than any pricing page.
...
```bash
AI_PROVIDER=deepseek-openai DEEPSEEK_API_KEY=xxx node tracker.js ai-check --models
```
