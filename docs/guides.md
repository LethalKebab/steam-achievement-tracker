...
**The setup page runs the same check when you connect a database**, so you do not have to know this command exists to find out your database is unusable. Pasting an ID validates the whole schema, not just that the ID resolves; anything wrong is reported there and then, with a 帮我补上 or 帮我整理成看板 button when it is the kind of problem the program can fix. It also creates one page and immediately archives it — that is the only way to tell a read-only connection from a working one, and a read-only token otherwise passes every check and fails at the first `guide-gen`.
