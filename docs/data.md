...
- **`rarity` is the share of all owners holding the achievement, 0–100**, from `GetGlobalAchievementPercentagesForApp`. It is the only universal difficulty signal available: a game with four achievements left at 40% is an evening, and one with four left where a single one sits at 0.8% has a wall in it that no per-game hour figure can see. `rarity_checked_at` gates the refresh — the figure is computed across every owner and barely moves, so a game asked within the last 30 days is skipped, and a finished game is never asked at all.
