...
| `lib/ai.js` | AI provider layer (Anthropic only so far): request assembly, the server-tool loop, usage/cost accounting. Raw HTTP, no SDK. **Streaming is mandatory, not a performance choice** — `max_tokens` caps thinking *plus* prose, and a long researched answer hits undici's 5-minute HTTP timeout before it hits the token ceiling. Knows nothing about guides; adding OpenAI means touching this file and nothing else |
...
- **AI 攻略生成** — designed in full; **steps 1–3b of 9 are built** (the provider-search spike, `lib/guidelint.js` + mechanical ticking, `lib/ai.js`, and the orchestration `lib/guidegen.js`). `node tracker.js guide-gen <appid>` generates a guide end to end. Next by the doc's ordering is step 4 (OpenAI), but step 5 (a hard spend cap + live cost) is the more valuable one — `guide-gen` currently asks for confirmation and reports cost only *after* the run. Decisions, the reuse map, the ordering, the effort estimates, and the honest limits of what a machine gate can verify are in [docs/ai-guide-writing.md](docs/ai-guide-writing.md) — its 「动手顺序」 section is the live checklist and marks what's done. **Read it before redesigning** — every choice there came out of a question-by-question pass, including the two that are easy to get wrong: the checked state is applied mechanically rather than written by the model, and unvalidated drafts must not land anywhere `syncGuidesFromMarkdown` will scan.
