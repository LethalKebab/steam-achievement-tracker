...
| File | Role |
|---|---|
| `tracker.js` | CLI dispatch: `init` (`--notion`, `--ai`), `sync`, `serve`, `status`, `guides`, `checkbox-sync`, `guide-status`, `audit`, `ai-check`, `guide-gen`, `import`, `export`, `log` |
...
- **AI 攻略生成** — **working end to end.** `node tracker.js guide-gen <appid>` generated a researched 51-achievement guide that passed the gate on round one with zero rewrites, ticks verified one-by-one against Steam. Steps 1–4 of 9 are built (the spike, `lib/guidelint.js` + mechanical ticking, `lib/ai.js` + `lib/ai-anthropic.js`, the orchestration `lib/guidegen.js`, and `lib/ai-gemini.js`). `node tracker.js guide-gen <appid>` generates a guide end to end against either provider. **Step 4's second provider is Gemini, not OpenAI** — the switch was made because Gemini has a free tier and OpenAI has no usable free allowance, and being able to run the pipeline at all mattered more than the original list. Next is step 5 (a hard spend cap + live cost) — `guide-gen` currently asks for confirmation and reports cost only *after* the run. Decisions, the reuse map, the ordering, the effort estimates, and the honest limits of what a machine gate can verify are in [docs/ai-guide-writing.md](docs/ai-guide-writing.md) — its 「动手顺序」 section is the live checklist and marks what's done. **Read it before redesigning** — every choice there came out of a question-by-question pass, including the two that are easy to get wrong: the checked state is applied mechanically rather than written by the model, and unvalidated drafts must not land anywhere `syncGuidesFromMarkdown` will scan.
  ...
  - `node tracker.js guide-gen <appid>` runs the whole pipeline; `--dry-run` prints the assembled prompt and the landing plan without sending anything. It asks for confirmation before spending unless you pass `--yes`. Every refusal reason (no achievement schema, too many achievements, an existing Notion page, the target file already existing, Steam not returning unlock state) is raised up front in `planGuide()` — never halfway through, after you've paid.
  - **The live path has never actually run.** Every claim about it comes from the authoritative API docs plus offline tests; the machine it was written on has no `ANTHROPIC_API_KEY`. Treat the first real `ai-check` / `guide-gen` as the verification step, and expect the three spike unknowns (Chinese-guide-site coverage, the real `max_content_tokens` ceiling, how search itself is billed) — plus "does the model actually honour one-checkbox-per-achievement across 44 achievements" — to resolve only then.
