...
- **AI 攻略生成** — designed in full; **steps 1–2 of 9 are built** (the provider-search spike, and `lib/guidelint.js` + mechanical ticking). Next is step 3, `lib/ai.js` end-to-end against one provider. Decisions, the reuse map, the ordering, the effort estimates, and the honest limits of what a machine gate can verify are in [docs/ai-guide-writing.md](docs/ai-guide-writing.md) — its 「动手顺序」 section is the live checklist and marks what's done. **Read it before redesigning** — every choice there came out of a question-by-question pass, including the two that are easy to get wrong: the checked state is applied mechanically rather than written by the model, and unvalidated drafts must not land anywhere `syncGuidesFromMarkdown` will scan.
