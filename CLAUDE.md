...
- `checkbox-sync --dry-run` across all 44 eligible games proposes **0** ticks and produces 0 log lines — no skips, no failures, no ambiguous-name warnings. **This does not mean "Notion and Steam agree", and reading it that way was wrong.** A dry run only proposes ticks for achievements it can *match*; a guide whose boxes match nothing produces the same silent 0. `guidelint` measured the difference on 2026-08-10: **325 achievements across the 44 guides have no checkbox that name-matching can reach**, so they could never have been ticked and never showed up here. `audit` doesn't catch it either — it only inspects boxes that are *already ticked*. Treat 0 ticks as "nothing actionable was found", never as evidence of agreement.
