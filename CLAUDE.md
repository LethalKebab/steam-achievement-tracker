...
- **Same-name collisions, re-measured 2026-08-09:** **13 collisions across 12 games** library-wide. (A bilingual pair counts once — `妙手空空` and `Skilled Thief` are the same two achievements seen twice, so a naive count of colliding *name strings* returns 15 and overstates it.) **Only 4 of the 12 have a registered guide**, so only those can affect ticking at all: Civ VI, CK3, 鬼谷八荒, Plague Inc.
- **All four of those already resolve by description, verified 2026-08-09.** Civ VI, CK3 and 鬼谷八荒 were fixed earlier; Plague Inc turns out to have been fine all along — its two `生化武器大师` boxes each quote the official description verbatim (`…打败纳米病毒大师!` vs `…打败生化武器大师!`), and both descriptions are unique in the game, so the description-first pass separates them. All four will tick automatically when a second twin unlocks. **Don't "fix" these pages again.** The other 8 colliding games have no guide, so they cost nothing until one is written.
