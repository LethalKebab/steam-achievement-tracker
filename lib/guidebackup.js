...
/**
 * 局部重写的预检。**它要说的话和整篇重写正好相反**,所以是另一个函数而不是加个参数。
 *
 * 整篇重写的预检回答「你会失去什么」;局部重写的预检回答「什么会留下」—— 而后者
 * 才是这个功能的全部卖点。同一个函数被迫服务两种问法,两边都会写歪(这个项目在
 * `inspectGuideDb` 和 `previewGuideRewrite` 上已经两次得出同一个结论)。
 *
 * `atRiskTicks` 在这里是**收紧**过的:只有落在要改的那几条底下的手动勾选会丢,别处
 * 的一个都不动。`savedTicks` 就是这个差额 —— 它必须被报出来,因为它是用户选择局部
 * 重写而不是整篇重写时**唯一能拿到的、确凿的、可量化的好处**。
 */
