...
export async function backupGuide(config, { guide, appid, notion = null, now = new Date() }) {
  ...
  if (guide.kind === 'local') {
    ...
    if (!existsSync(from)) throw new Error(msg('bk.guideFileGone', { path: from }));
  ...
  if (!notion?.configured) throw new Error(msg('bk.notionMissing'));
  const blocks = await notion.fetchAllBlocks(extractNotionPageId(guide.url));
  if (!blocks.length) {
    throw new Error(msg('bk.emptyPage', { url: guide.url }));
  }
...
export function diffGuides({ oldTodos, newTodos, defs, oldText = '', newText = '' }) {
  ...
  for (const [apiName, todo] of before.byApiName) {
    if (!after.byApiName.has(apiName)) {
      ...
      lostAchievements.push({ apiName, name: achName(d) || apiName, wasChecked: Boolean(todo.checked) });
    }
  }
...
/**
 * Render the diff preview as a few lines for a person to read.
 *
 * **Terminal only.** All three formatters here are called from `tracker.js` and nowhere else; the
 * Dashboard's gate takes numbers from `previewGuideRewrite` instead, on the rule that a command
 * line can afford a report and the rewrite dialog cannot. That is why the wording lives in
 * `tracker-messages.js` with the rest of what only a terminal ever sees.
 */
export function formatDiff(d) {
  const lines = [
    clog('pre.counts', {
      oldCount: d.oldCount, newCount: d.newCount, oldChecked: d.oldChecked, newChecked: d.newChecked,
    }),
    clog('pre.covered', { old: d.oldCovered, new: d.newCovered }),
    clog('pre.subSteps', { old: d.oldSubSteps, new: d.newSubSteps }),
  ];
  if (d.oldChars || d.newChars) {
    const pct = d.oldChars ? Math.round(((d.newChars - d.oldChars) / d.oldChars) * 100) : 0;
    lines.push(clog('pre.chars', {
      old: d.oldChars, new: d.newChars, sign: pct >= 0 ? '+' : '', pct,
    }));
  }
  if (d.lostAchievements.length) {
    lines.push(clog('pre.lostAch', { n: d.lostAchievements.length }));
    for (const a of d.lostAchievements.slice(0, 8)) {
      // Seven spaces and a name: indentation, so it stays here rather than in a table
      lines.push(`       ${a.name}${a.wasChecked ? clog('pre.wasChecked') : ''}`);
    }
    if (d.lostAchievements.length > 8) lines.push(clog('pre.andMore', { n: d.lostAchievements.length - 8 }));
  }
  if (d.lostTicks.length) {
    lines.push(clog('pre.lostTicks', { n: d.lostTicks.length }));
    for (const t of d.lostTicks.slice(0, 5)) lines.push(`       ${t.text.split('\n')[0].slice(0, 50)}`);
    if (d.lostTicks.length > 5) lines.push(clog('pre.andMore', { n: d.lostTicks.length - 5 }));
  }
  if (!d.lostAchievements.length && !d.lostTicks.length) {
    lines.push(clog('pre.nothingLost'));
  }
  return lines.join('\n');
}
...
export function formatPatchPreflight(p, { defsCount = null } = {}) {
  const lines = [
    clog('pre.patchScope', {
      scope: p.scope, keeping: p.keeping, count: p.count, checked: p.checked,
      covered: p.covered, of: defsCount ? `/${defsCount}` : '',
    }),
  ];
  if (p.atRiskTicks.length) {
    lines.push(clog('pre.atRiskUnder', { n: p.atRiskTicks.length }));
    for (const t of p.atRiskTicks.slice(0, 5)) lines.push(`       ${t.text.split('\n')[0].slice(0, 50)}`);
    if (p.atRiskTicks.length > 5) lines.push(clog('pre.andMore', { n: p.atRiskTicks.length - 5 }));
  } else {
    lines.push(clog('pre.noTicksLost'));
  }
  // This line is the only concrete, quantifiable advantage a partial rewrite has over a whole one; do not omit it
  if (p.savedTicks > 0) {
    lines.push(clog('pre.ticksSaved', { n: p.savedTicks }));
  }
  return lines.join('\n');
}
...
export function formatPreflight(p, { defsCount = null } = {}) {
  const lines = [
    clog('pre.existing', {
      count: p.count, checked: p.checked, chars: p.chars,
      covered: p.covered, of: defsCount ? `/${defsCount}` : '',
    }),
  ];
  if (p.atRiskTicks.length) {
    lines.push(clog('pre.atRisk', { n: p.atRiskTicks.length }));
    for (const t of p.atRiskTicks.slice(0, 5)) lines.push(`       ${t.text.split('\n')[0].slice(0, 50)}`);
    if (p.atRiskTicks.length > 5) lines.push(clog('pre.andMore', { n: p.atRiskTicks.length - 5 }));
  } else {
    lines.push(clog('pre.noTicksLost'));
