...
export function resolveGuidePath(guidesDir, url) {
  ...
  if (!isInside(guidesDir, full)) {
    throw new Error(msg('guide.pathEscapes', { url }));
  }
  if (!existsSync(full)) throw new Error(msg('guide.fileMissing', { path: full }));
  return full;
}
...
/**
 * A **section** heading line → `[whole, hashes, title]`, or null.
 *
 * `##` and deeper only. `#` is the guide's own title, written by the program and never a section —
 * treating it as one would put the entire guide inside it. `guideOutline` above matches `#` as
 * well on purpose: that function describes the document, this one decides what a section is.
 */
export function headingOf(line) {
  return String(line ?? '').match(/^(#{2,6})\s+(.*\S)\s*$/);
}

/**
 * Two section titles name the same section when this agrees on them.
 *
 * Whitespace and case are ignored; **punctuation deliberately is not**. 「经典模式:通用挑战成就」
 * and 「经典模式·通用挑战」 are one topic to a reader and two different strings here — folding those
 * together is a guess about meaning, while folding two spellings of one title never is. The
 * classification pass is what reconciles differently-worded sections; this decides only what is
 * literally the same one.
 *
 * `mergeDuplicateSections` and the linter's `duplicate-heading` share it, so what one reports is
 * exactly what the other merges.
 */
export function normSectionTitle(title) {
  return String(title ?? '').replace(/\s+/g, '').toLowerCase();
}

/** A top-level achievement line. Indented ones are sub-steps and belong to the entry above them */
const TOP_ENTRY_RE = /^[-*] \[[ xX]\]/;

/**
 * The document as a tree of sections: each heading owns the lines under it and the deeper headings
 * that follow, up to the next heading at its own level or shallower.
 *
 * **One parser, two readers.** `mergeDuplicateSections` folds by it and `guidelint` reports the
 * shapes it exposes; a second copy is how the fold and the report end up disagreeing about what a
 * section contains. Everything before the first `##` — the guide's own `# 游戏名` and `appid:`
 * lines included — is the preamble.
 *
 * @returns {{preamble: string[], roots: any[]}} each node is
 *   `{level, title, line, own, entries, children}`, where `own` is its lines verbatim and `entries`
 *   counts only the top-level checkboxes among them.
 */
export function sectionTree(text) {
  const preamble = [];
  const roots = [];
  const open = [];
  for (const line of String(text ?? '').split(/\r?\n/)) {
    ...
    if (!h) {
      ...
      if (holder) {
        holder.own.push(line);
        if (TOP_ENTRY_RE.test(line)) holder.entries++;
      } else preamble.push(line);
      continue;
    }
    const node = { level: h[1].length, title: h[2], line, own: [], entries: 0, children: [] };
