...
export function parseArchiveId(config, id) {
  ...
  if (!ARCHIVE_DIRS.includes(dir)) throw new Error(msg('arc.badDir', { raw }));
  if (!ARCHIVE_FILE_RE.test(file)) throw new Error(msg('arc.badFile', { raw }));
  ...
  if (!path) throw new Error(msg('arc.outOfRange', { raw }));
...
async function restoreLocal(db, { config, dir, file, path, now }) {
  ...
  if (!appid) {
    throw new Error(msg('arc.noAppidLine', { file }));
  }
...
async function restoreNotion(db, { config, notion, path, now }) {
  ...
  if (!url) throw new Error(msg('arc.noPageUrl'));
  if (!notion?.configured) {
    throw new Error(msg('arc.notionMissing'));
  }

  const { blocks, dropped } = blocksForAppend(data.blocks ?? []);
  if (!blocks.length) throw new Error(msg('arc.noBlocks'));
  ...
  if (!fidelity.ok) {
    throw new Error(msg('arc.readbackMismatch', {
      url,
      problems: fidelity.problems.join('\n  '),
      path: backup.path,
    }));
  }
...
export async function restoreArchive(db, { config, notion = null, id, now = new Date() }) {
  const { dir, file, path } = parseArchiveId(config, id);
  if (!existsSync(path)) throw new Error(msg('arc.gone', { path }));
