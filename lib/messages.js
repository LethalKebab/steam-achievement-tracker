...
export const MESSAGES = {
  ...
  'ai.noKey':             ['{provider} 尚未填写 API Key', 'No API key for {provider} yet'],
  ...
  'ai.dsTooLong':         ['\n  (上下文或输出长度超了 —— DeepSeek 的上限比另外两家小,换一款成就少一点的游戏,或者换个供应商)', '\n  (the context or the output ran past its limit \u2014 DeepSeek\u2019s ceilings are lower than the other two. Try a game with fewer achievements, or another provider)'],
  'ai.keyEmpty':          ['(空)', '(empty)'],
  ...
  // ---- Notion --------------------------------------------------------------
  'notion.noToken':       ['尚未填写 Access token', 'No access token entered'],
  ...
  'gp.toolSearch':        ['正在联网搜索', 'searching the web'],
  'gp.toolFetch':         ['正在读取网页', 'reading a page'],
  'gp.toolFilter':        ['正在筛选结果', 'sifting the results'],
  ...
  'gp.mergedSections':    ['{n} 处重复的小节标题合到了一起', '{n} repeated section headings merged'],
  'gp.mergeFailed':       ['同名小节没能合并({reason}),同一个标题可能出现好几次', 'Same-titled sections could not be merged ({reason}); one title may appear several times'],
  'gp.regroupRetry':      ['分区统一没成,再试一次', 'grouping did not come back usable; asking once more'],
  'gp.regroupFill':       ['有 {n} 条没分到小节,单独再问一次', '{n} were left out of the grouping; asking about those'],
  'gp.regroupFilled':     ['补上了 {n}/{of} 条', '{n} of {of} filled in'],
  'gp.regroupFillFailed': ['补分没成({reason}),这几条留在原来的小节里', 'The follow-up did not come back usable ({reason}); those stay in the sections they were in'],
  'gp.regroupFailed':     ['分区统一失败({reason}),同类成就可能散在几个小节里', 'grouping failed ({reason}); related achievements may be spread across sections'],
  ...
  'lint.headingLevelJump': ['小节标题「{text}」比上一层深了不止一级,中间那一级没有标题,它挂在了上一个小节下面', 'The heading “{text}” is more than one level deeper than the heading above it, so the level in between has no heading and it hangs under the previous section'],
  'lint.mixedSectionDepth': ['小节「{text}」自己列了成就,下面又有带成就的子小节,同一节的成就分在了两个深度', 'The section “{text}” lists achievements of its own and also has subsections holding achievements, so one section’s achievements sit at two depths'],
  'lint.duplicateHeading': ['小节标题「{text}」出现了不止一次,同一个标题下的内容被拆到了几个地方', 'The section heading “{text}” appears more than once, so what belongs under one title is split across several places'],
  ...
  'schema.perfectSkipped': ['已经解锁全部成就,批量同步刻意不取它的成就详情(生成攻略时会自己去取)', 'Already at 100%, so the bulk sync deliberately does not fetch its achievement detail; generating a guide fetches it on the spot'],
  ...
  'status.whyComplete':   ['已解锁全部成就', 'every achievement is unlocked'],
  'status.whyIncomplete': ['成就总数变多,掉出 100%', 'the achievement total rose, dropping it below 100%'],
  'status.whyPlayed':     ['自上次检查后又玩过', 'played since this was last looked at'],
  ...
  'steam.recentFailed':   ['GetRecentlyPlayedGames 失败 HTTP {status},这次跳过家庭库检查', 'GetRecentlyPlayedGames failed, HTTP {status} — skipping the family-library check this run'],
  'family.tokenMissing':  ['没有填 access_token', 'No access_token was given'],
  'family.tokenBad':      ['access_token 无效或已过期,重新取一次(有效期约 24 小时)', 'That access_token is invalid or expired — fetch a new one (they last about 24 hours)'],
  'family.failed':        ['{method} 失败 HTTP {status}', '{method} failed, HTTP {status}'],
  'family.notMember':     ['这个账号不属于任何家庭组', 'This account is not in a family group'],
  ...
  'gen.mergeLostText':    ['合并同名小节丢了正文:「{text}」进去 {n} 次、出来 {got} 次。已停止,正文未改动。', 'Merging same-titled sections lost body text: “{text}” went in {n} times and came out {got}. Stopped; the text is unchanged.'],
  'gen.mergeBrokeToggles': ['合并同名小节把折叠块拆开了(进去 {in} 个、出来 {out} 个,内容对不上)。已停止,正文未改动。', 'Merging same-titled sections pulled the toggles apart ({in} went in, {out} came out, and the contents do not match). Stopped; the text is unchanged.'],
  ...
  'gen.unwrapRecheckFailed': ['拆折叠之后未通过校验({n} 条阻断)', 'The checks did not pass after unwrapping the toggles ({n} blocking)'],
  'gen.whereNotion':      ['Notion 页面', 'a Notion page'],
  'gen.whereLocal':       ['本地文件', 'a local file'],
  'gen.mergeRecheckFailed': ['合并同名小节之后未通过校验({n} 条阻断)', 'The checks did not pass after merging same-titled sections ({n} blocking)'],
  'gen.recheckFailed':    ['重排之后未通过校验({n} 条阻断)', 'The checks did not pass after regrouping ({n} blocking)'],
  ...
  'notion.noDbIdYet':     ['尚未填写攻略数据库 ID(也可以让程序帮你建一个)', 'No guide database ID entered yet — or have the program create one for you'],
