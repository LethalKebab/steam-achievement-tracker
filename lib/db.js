...
export function replaceAchievements(db, appid, rows) {
  ...
  const ins = db.prepare(
    `INSERT INTO achievements (appid, api_name, game_name, name_cn, name_en, description, description_en, hidden, icon)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
/**
 * The rarest achievement this account is still missing, per game.
 *
 * This is the number that says how hard finishing actually is. A game's remaining hours are
 * bounded below by its worst single achievement, not by how many are left, and a rarity of 1%
 * next to "4 left" is the difference between an evening and a project.
 *
 * Rows with no rarity on record are skipped rather than treated as easy — an absent figure is
 * not evidence of anything, and counting it as 100 would flatter exactly the games nothing is
 * known about.
 */
export function hardestRemaining(db) {
  return db
    .prepare(
      `SELECT appid,
              MIN(rarity)   AS min_rarity,
              COUNT(*)      AS locked_known
         FROM achievements
        WHERE unlocked = 0 AND rarity IS NOT NULL
        GROUP BY appid`
    )
    .all();
}
