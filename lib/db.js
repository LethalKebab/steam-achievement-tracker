...
-- AI 用量流水。**存 token 数,不存钱** —— 钱是事后按当时的价格表算出来的,
-- 而价格表会变、而且有些供应商我们根本没有可信单价。token 数是各家都测得准的事实,
-- 把它当账本的基础单位,美元只是它的一个视图。
-- usd 那一列是记账当时能算出来的金额,算不出来就是 NULL(不是 0)。
CREATE TABLE IF NOT EXISTS ai_usage (
  id       INTEGER PRIMARY KEY AUTOINCREMENT,
  ts       TEXT NOT NULL,
  kind     TEXT NOT NULL,   -- 'guide-gen' | 'ai-check' | 'verify'
  appid    TEXT,
  provider TEXT,
  model    TEXT,
 ...
 * 记一次 AI 调用的用量。
 *
 * **每次真花了钱的调用都要记**,不管成功还是失败 —— 失败的那次一样扣了额度,
 * 不记的话上限就会被"失败不算钱"这个错觉架空。
 */
export function recordAiUsage(db, { kind, appid = null, provider = '', model = '', usage, usd = null }) {
  db.prepare(
    `INSERT INTO ai_usage (ts, kind, appid, provider, model, requests, input, output, searches, usd)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    nowIso(), String(kind), appid === null ? null : String(appid), String(provider), String(model),
    usage?.requests ?? 0, usage?.inputTokens ?? 0, usage?.outputTokens ?? 0,
    usage?.webSearches ?? 0,
    typeof usd === 'number' && Number.isFinite(usd) ? usd : null
  );
}

/**
 * 从某个时刻起的用量合计。`usd` 是**能算出金额的那部分**的和,
 * `unpricedRequests` 是算不出金额的请求数 —— 两个都要报,否则
 * "今天花了 $0" 会被读成"没花钱",而实际可能是跑了 20 次没有价格表的模型。
 */
export function aiUsageSince(db, sinceIso) {
  const r = db.prepare(
    `SELECT COUNT(*) AS calls,
            ...
            COALESCE(SUM(CASE WHEN usd IS NULL THEN requests ELSE 0 END), 0) AS unpricedRequests
       FROM ai_usage WHERE ts >= ?`
  ).get(String(sinceIso));
  return {
    calls: r.calls, requests: r.requests, input: r.input, output: r.output,
    searches: r.searches, usd: r.usd, unpricedRequests: r.unpricedRequests,
  };
}

/** 最近几条流水,给 `ai-cost` 打明细用 */
export function recentAiUsage(db, limit = 20) {
  return db.prepare('SELECT * FROM ai_usage ORDER BY id DESC LIMIT ?').all(Number(limit));
}
