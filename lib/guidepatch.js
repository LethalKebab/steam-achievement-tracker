...
import { resolveScope, scopeEntries, classifyFindings, groupBySection, RARE_PCT } from './guidescope.js';
import {
...
export async function planPatch(db, { config, steam, appid, notion = null, selector, plan: given = null }) {
   ...
   * 并没有一个范围。以前那里传的是 `'all'`,借一个用户可见的选择器当内部信号;
   * 而 `all` 后来被删掉了(整篇重写有 `--overwrite`,不需要第二条路),那个借用
   * 当场就成了一个会抛错的调用。**内部需要的信号就明写成内部信号**,别去借一个
   * 将来可能被删的公开取值。
   */
  //
...
export const PATCH_PRESETS = [
   ...
   * **名字是名字,阈值是解释。**
   *
   * 上一版把阈值塞进了标签(`稀有 <15%`),理由是「悬停在触屏上不存在」。那个理由
   * 在这里不成立:这是个跑在本机、给桌面浏览器和 Electron 窗口看的面板,没有触屏
   * 这回事。而代价是实打实的 —— 一行按钮里混着一个数学表达式,读起来像参数不像选项。
   *
   * 所以标签回到「稀有成就」,阈值进 `title`。**判据仍然只有一个数**(`RARE_PCT`),
   * 说明是从它拼出来的,改阈值不会让这句话变成谎话。
   */
  {
    key: 'rare',
    label: '稀有成就',
    title: `全球解锁率低于 ${RARE_PCT}% 的成就。提示词里也是用这条线判断哪几条要写深`,
  },
  { key: 'locked', label: '未解锁', title: '你还没拿到的成就' },
];

/**
 * **`failing` 不在这一排里,也不在 CLI 上 —— 整个删掉了(owner 决定)。**
 *
 * 它选的是「当前校验没过、而且重写能修」的条目。真实语料里这个集合几乎永远是空的:
 * 攻略要么是这套流水线自己写的(闸门不过就不落地),要么是手写的(那种问题多半是
 * `missing-checkbox`,局部重写够不着)。一个常年显示 0 的按钮占着一格位置,还得
 * 让人每次判断一次"这 0 是什么意思"。
 *
 * 真需要的时候有更趁手的工具:`node tracker.js guide-lint <appid>` 逐条列出来,
 * 看完再决定点哪几条。
 */
 ...
 * **`thin` 不在这一排里,而且不该加回来。**
 *
 * 它选的是「去掉名字和官方描述之后剩不到 40 个字」的条目,而这件事没有一个诚实又
 * 简短的中文说法:「写得薄」是 thin content 直译,中文不这么讲;「没写打法」把
 * 「只写了一句」说成「没写」,是句假话。而按钮上的标签必须一眼看懂 —— 一个需要
 * 先解释阈值才成立的判据,做不成按钮。
 *
 * CLI 上它留着(`--only thin[:字数]`):那里可以写清楚判据、还能调阈值,
 * 而敲这个 flag 的人本来就在读说明。**界面和命令行的选项不必一一对应** ——
 * 硬要对齐的结果是界面上多一个说不清楚的按钮。
 */
 ...
 *
 * @returns {{key, label, title, count, keeping, atRisk, savedTicks, unavailable, reason}[]}
 */
export function patchPresets({ plan, oldText = null }) {
  const { defs, oldTodos, unlocked, rarity } = plan;

  return PATCH_PRESETS.map(({ key, label, title }) => {
    try {
      const scope = resolveScope({
        selector: key,
        defs,
      ...
      return {
        key, label, title, count: 0, keeping: 0, atRisk: 0, savedTicks: 0, unlocatable: 0,
        unavailable: true,
 ...
 *
 * 界面上的「挑几条…」用它,而它同时解决了两件事:选一整个小节(点标题)和选单条
