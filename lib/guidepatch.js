...
export async function planPatch(db, { config, steam, appid, notion = null, selector, plan: given = null }) {
  ...
  if (!plan.existing) {
    const err = new Error(msg('patch.noGuide', { game }));
  ...
  if (scope.unresolved.length) {
    const err = new Error(msg('patch.namesNotFound', { game, list: scope.unresolved.join('、') }));
    err.code = 'unknown-achievements';
    err.detail = { unresolved: scope.unresolved };
    throw err;
  }
  if (!scope.apiNames.length) {
    const err = new Error(msg('patch.selectorEmpty', { selector }));
  ...
  if (!entries.length) {
    const err = new Error(msg('patch.noCheckboxes', { n: scope.apiNames.length }));
...
export async function patchGuide(db, {
  ...
  const ask = async (prompt, round) => {
    ...
    if (!verdict.ok) {
      const err = new Error(msg('gen.roundFailed', { round, reason: verdict.reason }));
  ...
  while (round < rounds) {
    ...
    if (caused.length && !caused.some((f) => MODEL_FIXABLE.has(f.code))) {
      throw new Error(msg('patch.nothingModelCanFix', { problems: caused.map((f) => f.message).join('\n  ') }));
    }
  }
...
export async function landPatchNotion(
  ...
  for (const e of entries) {
    ...
    if (!top) {
      ...
      throw new Error(msg('patch.noCheckboxBlock', { game: plan.game, apiName: e.apiName }));
    }
  ...
  if (bad.length) {
    throw new Error(msg('patch.notionRecheckFailed', {
      url: plan.existing.url,
      problems: bad.map((f) => f.message).join('; '),
    }));
  }
