...
import {
  ...
  buildSystemPrompt, buildPatchMessage, collapseEmptyBreaks, extractMarkdown, splitFindings, planGuide,
} from './guidegen.js';
...
export async function planPatch(db, { config, steam, appid, notion = null, selector, plan: given = null }) {
  ...
  /**
   * **`selector` 可以是 null:那表示"只要 plan 和基线,不要解析范围"。**
   *
   * Dashboard 的预检就是这么用的 —— 它要拿这份 plan 去把四个预设各算一遍,自己
   * 并没有一个范围。**内部需要的信号就明写成内部信号**,别去借一个公开选择器
   * (比如传 `'all'`)当内部标记:那个取值一旦被删,借用当场变成一个会抛错的调用。
   */
  //
  // **判的是 `== null`,不是假值。** 写成 `if (!selector)` 的话,用户敲的
  // `--only ""` 也会走进这条内部捷径 —— 于是 `planPatch` 交回一个 `scope: null`
  // 的结果,而调用方紧接着读 `pp.scope.apiNames`,当场 TypeError。空选择器是一个
  // **用户错误**,它有自己的 code(`empty-scope`)和终端建议,该照常抛出去。
  // 「调用方没给」和「用户给了个空的」是两件事,而假值判断把它们混成一件
...
/**
 * **Dashboard 上不做「范围」预设(稀有 / 未解锁 / …),只做挑选列表上方的快捷按钮。**
 *
 * 区别是:快捷按钮点一下把符合的那批**勾上**,而预设会把它们变成一个不可见的集合
 * 直接拿去跑。后者有一个真缺陷:
 *
 * 预设选中时,用户**从没见过那 27 条是什么**,却要在这个集合上确认一次要花钱、
 * 且不可逆的操作;而且选中之后增删不了 —— 想去掉其中两条都做不到。
 *
 * 搬进列表之后:点一下 → 那 27 个框当场勾上 → 看得见是哪 27 条 → 还能改。
 * 「范围」于是回到一个真正的二选一:整篇 vs 自选。**预设本来就不是"范围",
 * 是"选择的起点"** —— 把它们建模成范围是个分类错误。
 *
 * 跟着一起没了的还有:一次 plan 算四个预设的那套机器、`unavailable` 和 `0` 的区分、
 * 每个预设各自的 preflight。客户端手里 `pickableEntries` 每条都带着解锁率和解锁状态,
 * 自己算就行,而且没有往返延迟。
 *
 * **CLI 的 `--only rare` / `--only locked` 照旧**,那边没有列表可看,只能一次点名。
 * 能力一样、语义一样,只是交互形态不同 —— 这不算两个界面各说各话。
 */

/**
 * 挑选列表:攻略里**能被局部重写改到**的成就,按小节分组。
 *
 * 界面上的「自选…」用它,而它同时解决了两件事:选一整个小节(点标题)和选单条
 * (点条目)。**所以界面不需要 `section:` 这个选择器** —— 分组只是呈现,发出去的
 * 请求永远是一串点名的 api_name,而那条路 `resolveScope` 早就支持了。
 *
 * 只列 `scopeEntries` 定位得到的那些:攻略里根本没写的成就局部重写够不着,列在
 * 挑选列表里等于摆一个点了没用的勾。
 *
 * @param {{heading:string|null, apiNames:string[]}[]} groups  groupBySection 的结果
 */
export function pickableEntries({ plan, groups }) {
  ...
  const item = (apiName) => {
    ...
    return {
      apiName,
      name: d?.name_cn || d?.name_en || apiName,
      // 稀有度和解锁状态跟着一起给:挑的时候最想知道的就是「这条难不难」「我打过没」,
      // 而这两个数服务端本来就有,前端再问一次是白跑一趟
  ...
  // 大纲里没出现、但攻略里确实有框的成就(标题结构没读到、或者归属没认出来)。
  // **兜住它们,不能让它们从列表里消失** —— 从"选不了"变成"看不见"是更坏的失败
...
export async function patchGuide(db, {
  ...
  while (round < rounds) {
    ...
    const msg = round === 1
      ? buildPatchMessage(entries, { instruction })
    ...
    /**
     * **勾选状态要在校验之前算出来,不能让校验看见一份「重写过的条目全没勾」的攻略。**
     *
     * `applyPatchToTodos` 把每条重写过的成就标成 `checked: false`,那是对的 ——
     * 模型交回来的永远是 `- [ ]`,它不许写勾。落地时两个后端都会照
     * `computeCheckedKeys` 重新勾一遍,所以**写进去的那份是勾好的**。
     * 但校验跑在落地之前,看到的是没勾的那一份,于是每一条**已解锁**的成就
     * 都会报一条 `checked-mismatch` —— 而那条错是 `MODEL_FIXABLE` 之外的,
     * 下面那个 throw 于是把整次改动判死。
     *
     * 后果是这条路对**任何已解锁的成就都不能用**,也就是一份打了大半的攻略里
     * 的绝大多数条目。实测:罗曼圣诞探案集(926340,46/50)重写「初入酒馆」
     * 一条,直接失败,报的正是这一条。
     *
     * 算出来的这份**必须传给落地**,不能让它自己再算一次:`computeCheckedKeys`
     * 会跳过 `checked` 已经为真的条目(「已经勾上的不用管」),所以对着这份
     * 勾好的再算一遍会得到空集,Notion 那条路就会拿 `checked: false` 把
     * 刚勾上的框**取消勾选** —— 同一个 bug 翻个面。
     */
...
export async function landPatchNotion(
  ...
  // 勾选状态按**改完之后的整份**算,和本地那条路同一个函数 —— 但那一步在**校验之前**
  // 就做完了(见上面那段长注释),这里只是把算好的结果用掉。**不要在这里重算**:
  // 传进来的 `patched` 已经是勾好的,而 `computeCheckedKeys` 跳过勾好的条目,
  // 重算会得到空集,然后把刚勾上的框取消勾选。
  // 合成 key 的那些(新子步骤)拿不到 block id,勾选状态在 append 时写进 payload
