    ...
    fetch('/api/syncStatus', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' })
      ...
      .then(({ result: s }) => {
        ...
        if (s.running) {
          ...
          // user ever gets
          if (s.error) {
            show('error', line('error', `同步失败:${esc(s.error)}` + reloadLink), { sticky: true });
            return;
          }
          ...
          if (typeof window.reloadDashboard === 'function') {
            ...
            if (forbidden.length) {
              notices.push(line('warn',
                forbidden.length + ' 款游戏 Steam 不再提供你的进度:' +
                  esc(forbidden.slice(0, 3).join('、')) + (forbidden.length > 3 ? ' 等' : '') +
                  '<br><span class="rpc-sub">请切换成手动更新成就数据</span>'
              ));
            }
