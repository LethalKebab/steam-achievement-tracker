...
export class DeepseekProvider {
  ...
  constructor(ai, { fetchImpl = globalThis.fetch, log = () => {} } = {}) {
    ...
    // 和 `provider: 'deepseek'` 那个预设区分开 —— 那个走 Anthropic 兼容端点、有联网
   ...
   * 有没有联网能力,**由配置推导,不写死**。
   *
   * 写这个文件时的认知是"DeepSeek 的 API 不提供服务端搜索"(聊天网站有,API 没有),
   * 所以 `deepseekTools` 默认空 ⇒ canSearch 为 false。但这是个**会过期的事实**,
   * 而且是在拿不到文档的情况下下的判断 —— 同一个会话里已经因为凭记忆断言
   * 栽过两次(Gemini 的模型名、2.5 系列停售)。
   *
   * 所以做成:真有服务端搜索的话,把它的声明填进 `ai.deepseekTools`,
   * canSearch 自动变 true、提示词自动换回联网那版、guide-gen 的闸门自动放行。
   * 一行配置,不改代码。
   */
  get canSearch() {
  ...
  /** `ai-check --models`:OpenAI 兼容的 GET /models。问 API 比翻记忆可靠 */
  async listModels() {
...
/**
 * 描述一个 key 的**形状**,不泄露内容。
 * 401 时最有用的信息是"我到底发出去了个什么",而这几项就够定位绝大多数粘贴事故。
 */
export function describeKey(k) {
  const s = String(k ?? '');
  if (!s) return '(空)';
  const bits = [`长度 ${s.length}`];
  bits.push(s.startsWith('sk-') ? '以 sk- 开头' : `以 "${s.slice(0, 3)}" 开头(不是 sk-)`);
  if (s !== s.trim()) bits.push('**首尾有空白**');
  if (/^["']|["']$/.test(s)) bits.push('**首尾有引号**');
  return bits.join(',');
}
...
async function errorFromResponse(res, model, apiKey = '') {
  ...
  // **诊断留下,命令行拿掉。** 这几句会原样出现在 Dashboard 的浮窗上,而那边
  // (尤其打包版)没有终端可敲、也不编辑 config.json。"发出去的 key 长什么样"
  // 这类事实两个界面都用得上,所以留着;"清掉环境变量该敲什么"只对终端有意义,
  // 按 code 挂给 tracker.js 的 CLI_HINTS(见那张表)
  let hint = '';
  let code = null;
  if (res.status === 402) {
    ...
    // 401 的时候人最想知道的是"我到底把什么发出去了"。key 不能打印,但**形状**可以,
    // 而形状恰好能指出最常见的两种错法:粘错了别家的 key、或者粘漏了一截
    code = 'bad-api-key';
    hint =
      `\n  (API key 无效。实际发出去的 key:${describeKey(apiKey)}\n` +
  ...
  const err = new AiError(`DeepSeek API HTTP ${res.status}${type ? ` ${type}` : ''}:${detail}${hint}`, {
    status: res.status,
    type,
    // 402 是余额问题,重试永远不会好
