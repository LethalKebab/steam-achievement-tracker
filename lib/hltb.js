...
export class HltbClient {
  ...
  #get(url, init = {}) {
    return fetch(url, { signal: AbortSignal.timeout(HLTB_TIMEOUT_MS), headers: baseHeaders(), ...init });
  }
  ...
  async openSession() {
    ...
    if (!res.ok) {
      this.log(`HLTB init -> HTTP ${res.status}`);
      this.session = null;
      return null;
    }
  ...
  async detail(hltbId) {
    ...
    if (!res.ok) {
      this.log(`HLTB 详情 ${hltbId} -> HTTP ${res.status}`);
      return null;
    }
