 ...
 */
import { achievementsFor, allGames, allGuides, getGame, upsertGuide, getGuide, appendSyncLog, nowIso, setGuideStatusSeen } from './db.js';
...
/**
 * How much of an entry counts as its head.
 *
 * SKILL.md rule 3.1 writes an entry as `**名字**<br>官方描述<br>心得`, and `<br>` is a newline
 * inside the one block, so the name and the official description are lines one and two. Anything
 * below that is the writer's own notes, where a description can only be a citation.
 */
const HEAD_LINES = 2;
 ...
 *
 * Three tiers, each requiring **uniqueness** — an audit either gives a definite answer or none, and
 * never guesses:
 *   1. The **head of the entry** quotes the achievement's complete description, and that description
 *      is unique within the game
 *   2. A candidate segment is exactly equal to an achievement name, and that name maps to exactly
 *      one achievement
 *   3. The description is quoted anywhere else in the entry, same uniqueness rule
 *
 * **Why the head comes first and the rest of the entry comes last.** By the writing convention the
 * first two lines are the name and the official description, so a description quoted there is what
 * this entry is *about*; one quoted further down is a citation — 「前置:先购买…(见成就「购买了愿望
 * 之夜-女巫」)」 — and used to win anyway, because it was longer than the entry's own description.
 * Measured over the whole corpus: **7 boxes of 4,133 were filed under an achievement the entry
 * merely mentioned**, across 神界：原罪2 (4), 月圆之夜 (2) and KINGDOM HEARTS (1). All seven name
 * themselves unambiguously on their own first line.
 *
 * **Tier 3 is what keeps the ambiguous-name rescue alive.** `matchAchievements` resolves a
 * same-named achievement only through a box that resolves to it *by description*, so a rule that
 * simply preferred the name would have taken that path away — and the cost of losing it is a
 * checkbox that can never be ticked. Keeping the whole-entry search as the last tier also costs
 * nothing: the one box in the corpus whose description sits below the head, and which has no
 * unique name of its own, still resolves exactly as it did.
 *
...
export function resolveTodoToAchievement(text, defs) {
  ...
  // **In either language.** An English guide quotes the English description and a Chinese one
  // quotes the Chinese, so matching one language only means this stage silently never fires on half
  // the guides. It is not resolved by asking which language the guide is in: the answer would have
  // to be right, and rows written before that was recorded carry an assumed value.
  //
  ...
  /**
   * The longest complete description contained in `haystack`, or null.
   *
   ...
   * ambiguity, and this answers those with nothing at all.
   */
  const longestQuoted = (haystack) => {
    const flatHay = flat(haystack);
    let best = null;
    let bestLen = 0;
    let ambiguous = false;
    for (const d of defs) {
      for (const raw of [d.description, d.description_en]) {
        if (!isQuotableText(raw)) continue;
        const k = flat(raw);
        if (descOwners.get(k)?.size !== 1 || !flatHay.includes(k)) continue;
        if (k.length > bestLen) {
          best = d;
          bestLen = k.length;
          ambiguous = false;
        } else if (k.length === bestLen && d.api_name !== best?.api_name) {
  ...
  /** The first candidate segment in `str` that is exactly one achievement's name */
  const uniqueName = (str) => {
    for (const cand of extractTitleCandidates(normalizeText(str))) {
      const hit = byName.get(cand);
      if (hit?.size === 1) return [...hit.values()][0];
    }
    return null;
  };
  const lines = String(text ?? '').split(/\r?\n/);

  // 1. The description quoted at the head — by the writing convention lines one and two are the
  //    name and the official description, so a description quoted there is what this entry is about
  const byHead = longestQuoted(lines.slice(0, HEAD_LINES).join('\n'));
  const byTitle = uniqueName(lines[0] ?? '');
  // **The title breaks a tie against a description that points somewhere else.** An achievement
  // Steam ships with no description at all cannot be quoted, so a guide writing one quotes a
  // sibling's — measured on KINGDOM HEARTS, where 「Record Keeper Sora」 has an empty description and
  // its entry quotes 「Record Keeper」's. Where the two agree, and where the title names nothing
  // unique (a same-named achievement), this changes nothing and the description still answers
  if (byTitle && byHead && byTitle.api_name !== byHead.api_name) return { def: byTitle, via: 'name' };
  if (byHead) return { def: byHead, via: 'description' };

  // 2. A name anywhere in the entry, same uniqueness rule
  const byAnyName = uniqueName(text);
  if (byAnyName) return { def: byAnyName, via: 'name' };
  // 3. The description quoted anywhere else in the entry
  const byBody = longestQuoted(text);
  if (byBody) return { def: byBody, via: 'description' };
...
export const GUIDE_STATUS_IN_PROGRESS = 'In progress';
export const GUIDE_STATUS_NOT_STARTED = 'Not started';

/**
 * Should this page move to `In progress`? Two questions, and which one applies depends on whether
 * the page has ever been looked at.
 *
 * **A page with no stamp is judged on the claim its status makes.** `Not started` is an assertion
 * about the game, and a single unlocked achievement falsifies it — no timing is involved, and none
 * should be: a game can carry unlocks and no `last_played` at all, since a family-shared row is
 * dated only by its playtime moving between two syncs and one never seen twice has no date.
 * Requiring play evidence here would strand those pages at `Not started` permanently. Anything
 * other than `Not started` on an unstamped page was chosen by the reader and is adopted as it is.
 *
 * **A page with a stamp is judged on play since that stamp** — never on a fixed recency window.
 * "Played in the last five days" reads as the obvious test and is wrong twice over: it fights a
 * reader who sets statuses by hand, and it cannot clear a backlog, since a guide stuck at
 * `Not started` for a game last played months ago falls outside every window worth having.
 *
 * Setting a status by hand does not move `last_played`, so nothing re-triggers; playing the game
 * again does, which is what lifts a page back out of `Paused`.
 */
export function shouldMarkInProgress(game, guide, pageStatus, notStartedName = GUIDE_STATUS_NOT_STARTED) {
  if (!guide.status_seen_at) return pageStatus === notStartedName && game.achieved > 0;
  // `last_played` is seconds (Steam's rtime), the stamp is an ISO string
  return game.last_played > 0 && game.last_played * 1000 > new Date(guide.status_seen_at).getTime();
}
 ...
 */
/**
 * The pages this function can say anything about, paired with their guide row and game row.
 *
 * Extracted because two callers need the same set and must not disagree about it: the selector
 * decides which of them change, and `syncGuideStatuses` stamps **all** of them — including the ones
 * it left alone. Without stamping those, a page the reader set to `Paused` never acquires a
 * timestamp, so it stays in the first-sight branch of `shouldMarkInProgress` forever and playing
 * the game can never lift it.
 */
export function guideStatusCandidates(db, pages) {
...
export function selectGuideStatusUpdates(
  ...
  { doneName = GUIDE_STATUS_DONE, stagedName = GUIDE_STATUS_STAGED,
    inProgressName = GUIDE_STATUS_IN_PROGRESS, notStartedName = GUIDE_STATUS_NOT_STARTED } = {}
) {
  const updates = [];
  for (const { page, guide, game } of guideStatusCandidates(db, pages)) {
    ...
    if (perfect && page.status !== doneName) {
      ...
      // **Playing is evidence, the same class as completing.** Without this a guide created before
      // its game was touched sits at `Not started` until the day it hits 100%, then jumps straight
      // to `Done` — the two states in between are written once at creation and never again.
      //
      // Last of the three on purpose: completion is a fact about the game and outranks a fact
      // about the session. A page demoted to `Staged` this run keeps that label rather than being
      // overwritten by play in the same pass; the next stretch of play moves it on, by which point
      // 「有补丁加了成就」 is no longer the most useful thing the page could be saying.
      !perfect
      // A null name is how the caller says the database has no such option. Without this the
      // branch still fires and writes a null status, which Notion answers with a 400
      && inProgressName
      && page.status !== inProgressName
      && shouldMarkInProgress(game, guide, page.status, notStartedName)
    ) {
      updates.push({ ...common, to: inProgressName, reason: 'played' });
    }
  }
  return updates;
}
...
export async function syncGuideStatuses(db, { notion, dryRun = false, onProgress = () => {} }) {
  ...
  // **The promotion is only offered when the option exists.** A database without `In progress`
  // would take a 400 on every played game — and unlike Done and Staged this one is not required,
  // since a reader who never added it is not using that state
  const canPromote = !schema.options.length || schema.options.includes(GUIDE_STATUS_IN_PROGRESS);
  const updates = selectGuideStatusUpdates(db, pages, canPromote ? {} : { inProgressName: null });
  // Every page this run could speak about, whether or not it changed. What is stamped is decided
  // after the writes: a page whose write failed keeps its old stamp, so the next run tries again
  const candidates = guideStatusCandidates(db, pages);
  const logs = [];
  const applied = [];
  ...
  // **Keyed on the reason, not on a two-way branch.** With three reasons a boolean would report a
  // promotion for play as 「掉出 100%」 — a sentence the reader would go looking for a cause of
  const WHY = { complete: 'status.whyComplete', incomplete: 'status.whyIncomplete', played: 'status.whyPlayed' };
  const why = (u) => msg(WHY[u.reason] ?? 'status.whyIncomplete');
  ...
  // **Stamped after the writes, and never on a dry run.** A page is settled if it needed nothing or
  // if its change landed; one whose write failed keeps its old stamp so the next run tries again.
  // Stamping the untouched ones is what lets a page the reader set by hand ever leave the
  // first-sight branch — without it, `Paused` could never be lifted by playing the game.
  if (!dryRun) {
    for (const { guide } of candidates) {
      if (!failed.has(guide.appid)) setGuideStatusSeen(db, guide.appid);
    }
  }
