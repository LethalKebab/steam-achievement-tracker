...
export function resolveTodoToAchievement(text, defs) {
  ...
  // 1. The complete description, unique within the game
...
export async function syncGameCheckboxes(db, steam, guide, gameName, { notion, config, dryRun = false, cascade = true }) {
  ...
  for (const a of matches.skippedAmbiguous ?? []) {
    push(achName(a), msg('sync.ambiguousName'));
  }
  ...
  if (dryRun) {
    for (const m of matches) {
      push(achName(m.achievement), msg('sync.wouldTick', { text: m.text.slice(0, 60) }));
    }
  ...
  try {
    ...
    for (const m of matches) {
      push(achName(m.achievement), msg('sync.ticked', { text: m.text.slice(0, 60) }));
    }
