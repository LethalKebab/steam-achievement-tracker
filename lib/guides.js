...
export async function checkboxSync(
  db,
  steam,
  { notion, config, appid = null, dryRun = false, cascade = true, onProgress = () => {} }
) {
  const guideByAppid = Object.fromEntries(allGuides(db).map((g) => [g.appid, g]));
  const candidates = allGames(db).filter((g) => {
    ...
    // 已经 100% 的游戏原本直接跳过(没有新成就可勾)。开了子步骤联动之后它们理论上
    // 不再是 no-op:成就全解锁、底下的子步骤却可能还空着。
    //
    // 但**必须同时有成就详情**才可能联动:认父成就靠 resolveTodoToAchievement,
    // 它要 achievements 表里的名字/描述,而 syncAchievementSchema 故意跳过正好 100% 的游戏
    // ——实测这类游戏 55/55 都没有成就详情。不加这个条件的话,每次跑都会白拉 55 个
    // 页面 + 55 次 Steam 请求,一个框也勾不上。哪天真给某个满成就游戏同步了详情,自动生效。
    if (g.achieved >= g.total) return cascade && achievementsFor(db, g.appid).length > 0;
    return true;
  });
