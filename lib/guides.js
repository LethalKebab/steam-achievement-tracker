...
/**
 * Has this game been played since the last time its status was looked at?
 *
 * **The comparison is against our own last look, not against a fixed recency window.** "Played in
 * the last five days" reads as the obvious test and fights the reader: park a game you played
 * yesterday at `Paused`, and every Dashboard open for the next five days moves it back. Comparing
 * against the stamp makes the promotion happen once per stretch of play — setting a status by hand
 * afterwards does not move `last_played`, so nothing re-triggers until the game is genuinely
 * played again. Playing it again is then exactly what lifts it back out of `Paused`, which is the
 * behaviour asked for.
 *
 * **A page never looked at before is a special case, deliberately narrow.** With no stamp there is
 * nothing to compare, and adopting silently would leave every guide created before this existed
 * sitting at `Not started` until its game is next played. So a first sight promotes — but **only
 * from `Not started`**, which is the value this program itself writes at creation. Anything else
 * on an unstamped page was chosen by the reader and is adopted as-is.
 */
export function playedSinceStatusLook(game, guide, pageStatus, notStartedName = GUIDE_STATUS_NOT_STARTED) {
  if (!(game.last_played > 0)) return false;
  // `last_played` is seconds (Steam's rtime), the stamp is an ISO string
  if (guide.status_seen_at) return game.last_played * 1000 > new Date(guide.status_seen_at).getTime();
  return pageStatus === notStartedName;
}
 ...
 * timestamp, so it stays in the first-sight branch of `playedSinceStatusLook` forever and playing
...
export function selectGuideStatusUpdates(
  ...
  for (const { page, guide, game } of guideStatusCandidates(db, pages)) {
    ...
    if (perfect && page.status !== doneName) {
      ...
      && playedSinceStatusLook(game, guide, page.status, notStartedName)
    ) {
