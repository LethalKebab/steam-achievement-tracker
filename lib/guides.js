...
export function normalizeText(s) {
  ...
  return String(s)
    .toLowerCase()
    // 下划线标注和 `**` 同类:是标记不是正文,而且 Notion 那边只剩里面的字。
    // 不削的话同一条攻略在两种后端上归一化的结果不一样,匹配就会时灵时不灵
...
/**
 * 「这两段文字是不是同一段」用的归一化。**只折叠同一个字符的不同写法,不碰内容。**
 *
 * 两个地方问这个问题:`resolveTodoToAchievement`(audit 反查、Dashboard 的
 * 「还差哪些成就」面板)和 `guidelint` 的「描述有没有原文照抄」。两边**必须是同一个
 * 判据** —— 各写一份的话迟早只改一份,而那时候 linter 会说「audit 反查不了这个框」,
 * 指着一个 audit 其实反查得动的框。原来正是各写一份(两处一模一样的私有 `flat`)。
 *
 * 折叠印刷体引号的理由是它**不改变内容**:Steam 给的 `“幸福的孩子”` 和攻略里抄的
 * `"幸福的孩子"` 是同一句话,而我们因为字形不同判成了「改写过」,于是 audit 反查不动
 * 那个框。实测全库 791 条不匹配里有 14 条是这一种,其余 777 条是作者真的改写了,
 * 不该动。
 *
 * **这不是放宽匹配。** 它不会让两段**不同**的描述相等,只会让同一段的两种写法相等 ——
 * 和已经在做的「折叠空格」同一类。真出现只差引号的两个成就,描述唯一性一判就变成 2,
 * 两个都不匹配,落在这个项目一贯选的那一侧(宁可勾不上,不可勾错)。
 *
 * 破折号和省略号**故意不折**:没有证据说需要,而它们在不同语境里含义确实不同。
 */
const TYPO_QUOTES = /[\u201C\u201D\u201E\u201F\uFF02]/g;
const TYPO_APOS = /[\u2018\u2019\u201A\u201B\uFF07]/g;
...
export function extractTitleCandidates(text) {
  ...
  //  用 guidelint 跑全量攻略时才暴露出来,实测影响 17 个框(全是文明 6)。
  ...
  //    Steam 上是「秘密食材」),精确相等因此不成立。和上面破折号那条是同类问题。
  //
...
/**
 * 反查一整份攻略:成就 → 攻略里写它的那一条(正文 + 挂在它下面的子步骤)。
 *
 * 用途和 auditGuideTicks 不一样(那个问"勾对了吗",这个问"这条该怎么打"),但方向
 * 和证据标准完全一样,共用同一个 resolveTodoToAchievement。**别因为"这次只是显示"
 * 就放松匹配** —— 同一个函数还担着往用户笔记里写勾的活,松一次是两边一起松。
 * 宁可卡片留白,也不能把 A 成就的打法贴到 B 成就下面。
 *
 * 按 SKILL.md 的写法约定,成就名 / 官方描述 / 你的打法在 Notion 里是**同一个块**
 * (`<br>` 是块内换行,notionblocks.js 专门保证过这件事),所以匹配上的那条 `text`
 * 本身就是解法全文,不需要去抓周围的段落——Dashboard 能直接把它显示出来。
 *
 * 同一个成就在攻略里出现多次时只认第一条:重复通常是别处顺带提了一句而不是第二种
 * 解法,两条都塞进卡片只会让人分不清哪条是正文。
 *
...
export function mapAchievementGuides(todos, defs) {
  ...
  // 子步骤 = 挂在成就框下面、自己又不对应任何成就的框。子步骤自己还能再挂子步骤,
  // 所以往下走到底(和 collectSubStepTicks 的 walk 同样的形状),depth 留给前端缩进。
  // claimed 的要跳过:那是它自己那条成就的正文,不是上面这条的子步骤
  const seen = new Set();
...
/**
 * 去掉攻略正文开头对成就名 / 官方描述的复述。
 *
 * Dashboard 的卡片上方已经印着 Steam 给的成就名和描述,而按 SKILL.md 的写法约定,
 * checkbox 的头两行正好是「成就名」和「逐字抄的官方描述」—— 同一份信息在一张卡片上
 * 出现两遍,把真正有用的打法挤到窗口外面去。
 *
 * **只删确凿的复述,而且只从开头删。** 两条判据都要求整行**恰好**是那个东西:
 *
 *   - 名字:整行归一化后等于中文名或英文名,或者是 `中文名(English Name)` 这种写法。
 *     **不能用 extractTitleCandidates 来判**——那个函数会从
 *     `知识(Rationality) — "……" 集齐全部百科全书条目` 里切出「知识」,照着删就把
 *     整条打法删没了。它是给"找匹配"用的,不是给"确认整行是复述"用的。
 *   - 描述:整行**逐字**等于 Steam 的描述。改写过的一律留着 —— 那是用户自己的话,
 *     而且隐藏成就的这一行往往是**全卡片唯一**写着达成条件的地方(Steam 那边根本
 *     不给描述)。删错一行拿不回来,留一行只是啰嗦,这个不对称决定了判据的松紧。
 *
 * **删到什么都不剩就返回空串**,调用方据此干脆不画攻略窗口。这不是"信息丢了":
 * 那种条目本来就只抄了官方名字和描述、没写任何打法,而这两样卡片上方已经有了。
 * 早先这里是 `rest || raw`(空了就原样吐回来),理由写的是"不能给出一张空卡片"——
 * 那个理由是错的,卡片上还有名字、描述和跳转按钮,空的只是窗口。它的实际效果是:
 * **恰好只抄了官方文案的条目,反而是重复得最彻底的那一张。**
 */
...
export function backendFor(guide, { notion, config }) {
  if (guide.kind === 'local') {
    ...
    return {
      ...
      // 校验器要全文才能查 `# 游戏名`/标题统计数字/数据来源说明这几条。
      // Notion 那边给不出整页原文,返回 null,lintGuide 会自动跳过依赖全文的规则
...
export async function syncGameCheckboxes(db, steam, guide, gameName, { notion, config, dryRun = false, cascade = true }) {
  ...
  try {
    ...
    push('', `跳过 - 无法读取${backend.label}攻略(Notion 需检查 connection 是否连接到该页面): ` + err.message);
    return logs;
  }
...
/**
 * 一份本地攻略的文件头:`appid:` 那行和 `# 游戏名` 那行。
 *
 * **只看前 15 行**,和这个函数被抽出来之前一样 —— 正文里出现 `appid:` 字样
 * (比如攻略在讲怎么找 appid)不该把登记的对象换掉。
 *
 * 抽出来是因为存档面板要拿同一个答案:`.migrated/` 和 `.drafts/` 里的文件名不带 appid,
 * 只能从正文读。两处各写一份正则,迟早会出现"存档面板认得这个文件、攻略发现认不得"。
 */
