...
export function chunkDefs(defs, size) {
  ...
  const out = [];
  for (let i = 0; i < defs.length; i += n) out.push(defs.slice(i, i + n));
  return out;
}
...
export async function planGuide(db, {
  ...
  if (defs.length > max) {
    ...
    throw new Error(
      `这个游戏有 ${defs.length} 个成就,超过上限 ${max}。分段写是支持的` +
        `(每段 ${size} 个,这一份要 ${Math.ceil(defs.length / size)} 段),但段数越多跑得越久、` +
        '花得越多。真要写就把 config.json 的 ai.maxAchievements 调大。'
    );
  }
  ...
  if (existing && !overwrite) {
    ...
    throw new Error(
      `《${game}》已经有攻略了(${where}:${existing.url})。要重新生成并覆盖它就加 --overwrite:` +
        '会先把原文备份进 guides/.backups/,再给你一份差异预览,确认之后才写。'
    );
  }
  ...
  if (target === 'local' && !existing && existsSync(finalPath)) {
    throw new Error(
      `${finalPath} 已经存在(但没登记在 guides 表里)。想覆盖它就加 --overwrite,` +
        '或者用 --file 换个名字写成新的一份。'
    );
  }
