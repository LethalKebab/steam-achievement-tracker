...
import { createSession, checkResult, checkBudget, estimateCost } from './ai.js';
...
export async function generateGuide(db, {
  ...
  for (round = 1; round <= rounds; round++) {
    ...
    // **每一轮跑完就查一次上限。** 查在轮与轮之间是有意的:回灌重写是这个功能里
    // 唯一会成倍放大花费的地方(3 轮 = 3 份完整上下文),而一轮跑完时钱已经花了、
    // 下一轮还没开始 —— 这是唯一既拦得住、又不会白白丢掉已付出成本的位置
    const budget = checkBudget(session.usage, {
      model: provider.model,
      pricing: config.ai?.pricing,
      maxTokens: config.ai?.maxTokensPerRun ?? 0,
      maxUsd: config.ai?.maxSpendPerRunUsd ?? 0,
      label: `《${game}》这一次生成`,
    });
    if (budget.note) onProgress({ phase: 'budget-note', round, note: budget.note });
    if (budget.over) {
      throw new Error(
        `${budget.reason}。已经跑完 ${round} 轮,草稿留在 ${draftPath}。
` +
          '  要放宽就改 config.json 的 ai.maxTokensPerRun / ai.maxSpendPerRunUsd。'
      );
    }
  ...
  return {
    ...
    expected,
    usage: session.usage,
    cost: estimateCost(session.usage, provider.model, config.ai?.pricing),
    model: provider.model,
    registered,
  };
}
...
export async function planGuide(db, { config, steam, appid, fileName = null }) {
  ...
  return {
    defs, game, unlocked, rarity, finalPath, draftPath,
    fileName: name,
...
/**
 * 起跑之前查一次**累计**上限(今天已经花了多少)。
 *
 * 和 per-run 上限是两件事:per-run 拦的是"这一次跑飞了",累计拦的是"点了二十次"。
 * 只有前者的话,一个每次都乖乖收在预算内的功能,点一天照样能花光。
 *
 * @returns {{over: boolean, reason: string|null, note: string|null, spent: object}}
 */
export function checkDailyBudget(db, config, { aiUsageSince, now = new Date() } = {}) {
  const maxTokens = config.ai?.maxTokensPerDay ?? 0;
  const maxUsd = config.ai?.maxSpendPerDayUsd ?? 0;
  const start = new Date(now.getFullYear(), now.getMonth(), now.getDate()).toISOString();
  const spent = aiUsageSince(db, start);

  const tokens = spent.input + spent.output;
  if (maxTokens > 0 && tokens >= maxTokens) {
    return {
      over: true,
      spent,
      reason: `今天已经用掉 ${tokens} 个 token,到达上限 ${maxTokens}(config.json 的 ai.maxTokensPerDay)`,
      note: null,
    };
  }
  if (maxUsd > 0 && spent.usd >= maxUsd) {
    return {
      over: true,
      spent,
      reason: `今天已经花掉 $${spent.usd.toFixed(4)},到达上限 $${maxUsd}(config.json 的 ai.maxSpendPerDayUsd)`,
      note: null,
    };
  }
  // 设了美元上限、但今天跑的全是算不出金额的模型 —— 这条上限其实没在拦任何东西,
  // 而账面上会显示 "$0.00 / 上限 $5",看起来一切正常。必须说出来
  const note =
    maxUsd > 0 && spent.unpricedRequests > 0
      ? `注意:今天有 ${spent.unpricedRequests} 次请求算不出金额(模型没有价格表),` +
        '美元上限对它们不起作用。把单价填进 ai.pricing,或者改用 ai.maxTokensPerDay。'
      : null;
  return { over: false, spent, reason: null, note };
}
