...
import {
  ...
  headingOf, normSectionTitle, sectionTree,
} from './markdown.js';
...
export const SKILL_RULE_DISPOSITION = {
  ...
  '4.2': '进了 —— 按游戏自身分类分节、小节标题一律 `##`、引言写在它那一节里面、节标题不带统计数字',
  '4.3': '进了 —— 写完就停',
...
**小节标题一律用 \`##\`。** 只有在一个小节内部还要再分一层时才用 \`###\`,而且那一层属于你正在写的这个小节。整份攻略的小节必须在同一层,否则读的人会看到一串没有归属的子标题。
...
- **一节的引言写在那一节里面。** 开一个小节、下面只放一段「这些成就都在 X 模式里做」然后把成就列在别的标题下,读的人看到的是一个标题带一段话、底下什么都没有,而且没法知道它说的是哪几条。机制速查是例外:那是给整份攻略查的,不是给某一节做引子的
...
**Section headings are \`##\`.** Use \`###\` only where a section needs dividing one level further, and that level belongs to the section you are writing. Every section of the guide sits at the same level; otherwise the reader is handed a run of subheadings belonging to nothing.
...
- **A section's intro goes inside that section.** Opening a section that holds one paragraph saying "these are all done in X mode" and then listing those achievements under other headings leaves the reader a heading with a paragraph and nothing under it, and no way to tell which achievements it was about. The quick reference above is the exception: it serves the whole guide rather than introducing one section
...
/**
 * How many times the classification pass is asked before the run degrades to "no rearrangement".
 *
 * **Two, and not more.** Every failure there but a cancellation is about what came back, so the
 * second ask is a real second chance rather than the same sentence thrown at the wall again; a
 * third would be, and this pass runs after the guide is already paid for and landable.
 */
export const REGROUP_ATTEMPTS = 2;

/**
 * The thinking depth an **aside** pass runs at, whatever depth the run itself was given.
 *
 * The classification pass writes no prose. The guide is already written, ticked and through the
 * gates; all this pass does is put 162 numbers under a dozen section names. Inheriting a 「深度模式」
 * run's `high` had it spend **31,998 of the 32,000 token budget** thinking and get truncated before
 * the answer — twice in a row on 月圆之夜, losing the entire sectioning both times, and the reason
 * only became visible once the degradation started carrying it. `max_tokens` caps thinking plus
 * prose, so the cap cannot separate the two; the depth is the only knob that can.
 *
 * The same measurement was taken on the spoiler pass before it was removed: inheriting `high` cost
 * **24,594** output tokens against **2,636** at `low`, on writing that itself cost 40,615 — the
 * difference between an aside costing 60% extra and costing 6%.
 *
 * **`low`, never `off`.** `off` stops the field being sent at all, which on this endpoint was
 * measured at 337 s and 145,955 characters of thinking: the uncapped case, the opposite of what is
 * wanted here.
 *
 * **The writing rounds are untouched.** 「深度模式」 still governs everything that decides what the
 * guide says; this is the one pass that decides nothing about content.
 */
export const ASIDE_EFFORT = 'low';
...
export function buildChunkFeedback(findings, chunks, i, unnameable = new Set(), lang = 'zh') {
  ...
  return (
    `第 ${i + 1}/${chunks.length} 段(第 ${before + 1}–${before + chunks[i].length} 个成就)未通过校验:\n\n` +
...
/**
 * The run's steps, decided **before** it starts.
 *
 * Issue #78 asked for "step N of M", and the reason the pipeline looks unable to answer that is one
 * variable, not many: how many rewrite rounds a guide needs is not knowable until the linter has
 * seen it. Everything else is settled by the time the plan exists — how many shards there are,
 * whether the classification pass will run (`chunks > 1`), whether there is an existing guide to
 * back up first.
 *
 * So that one variable is **kept out of the count**: rewrite rounds are progress *within* the
 * writing step, never extra steps. A total that grew as the run went would be worse than no total,
 * since the thing a person is watching for is whether it is advancing.
 *
 * **One step per shard**, for the same reason the total exists at all. Writing is nearly all of a
 * run's wall time, so a single step covering it leaves the count reading `1 / 2` for twenty
 * minutes — a number that does not move answers nothing that "is it stuck" was asking. Which shard
 * is *current* is deliberately not what a write step means: shards are written concurrently, so
 * that jumps around (see `askChunk`). It is **how many are finished**, which is monotonic.
 *
 * A shard that splits mid-run pushes the live count past this list, and the step is capped rather
 * than the total grown — the 「第 n/of 段」 note carries the live number, and a total that moved
 * while somebody watched would be worse than none.
 *
 * `unwrap` is deliberately absent. Whether it runs depends on what the model wrote, so it cannot be
 * listed in advance, and it is mechanical and near-instant — a step nobody would see.
 */
export function generationSteps({ chunks = 1, existing = false } = {}) {
  const parts = Math.max(1, chunks);
  const steps = parts > 1
    ? Array.from({ length: parts }, (_, i) => `write:${i + 1}`)
    : ['write'];
  if (parts > 1) steps.push('regroup');
  // **`existing`, not `overwrite`.** The backup runs when there is a guide to back up, and that is
  // what the landing checks; keying the list off the flag instead would count a step that never runs
  if (existing) steps.push('backup');
  steps.push('land');
  return steps;
}

/** How many of `generationSteps`' entries are writing steps — one per shard, always at the front */
export function writeStepCount(steps) {
  return steps.filter((s) => s === 'write' || s.startsWith('write:')).length;
}

/**
 * Reports `phase: 'step'` against a step list settled before the run. **Both pipelines use this
 * one** — a partial rewrite has its own, shorter list (`patchSteps`) and the same two rules.
 *
 * **It never goes backwards.** On the generation side a later round re-asks only the shards that
 * failed, so the finished-shard count the step is derived from falls; a step number that goes down
 * reads as the run having lost work rather than as it retrying something. An unknown key indexes to
 * 0 and is dropped by the same test.
 *
 * `of` never changes during a run — see `generationSteps` for why the list is fixed up front.
 */
export function stepReporter(steps, onProgress) {
  let at = 0;
  return (key) => {
    const n = steps.indexOf(key) + 1;
    if (n <= at) return;
    at = n;
    onProgress({ phase: 'step', key, n, of: steps.length });
  };
}

/**
 * How many achievement entries the prose streaming back so far has begun.
 *
 * **Top-level checkboxes only** (`^[-*] [ ]`), never the indented ones: sub-steps are lines inside
 * an entry, and counting them would report more written than the shard was asked for.
 *
 * It counts a line the moment its bracket arrives rather than when the line ends, so the number
 * means "started", and it only ever goes up — which is what a progress figure has to do. The text
 * is not parsed for anything else here; the guide is read from the finished reply as before.
 */
export function countStreamedEntries(text) {
  return (String(text ?? '').match(/^[-*] \[[ xX]\]/gm) ?? []).length;
}

/** The provider layer's tool vocabulary → what the progress bar says. See `toolLabel` in ai-anthropic.js */
const TOOL_NOTES = { search: 'gp.toolSearch', fetch: 'gp.toolFetch', filter: 'gp.toolFilter' };

/**
 * A tool event's vocabulary word → the sentence that goes on screen.
 *
 * **`guidepatch.js` calls this rather than keeping its own copy**, which is what it had: the word
 * itself reached the progress bar there — a bare English `search` under a Chinese interface — while
 * this path had been saying 「正在联网搜索」 since issue #78. Two copies of a mapping is how one of
 * them stays a year behind.
 *
 * An unknown word is passed through unchanged: a vendor adding a tool should read oddly rather than
 * vanish.
 */
export function toolNote(name) {
  return TOOL_NOTES[name] ? msg(TOOL_NOTES[name]) : name;
}
...
/**
 * The multiset of a document's real text: non-heading, non-blank lines, trimmed.
 *
 * **Shared by every pass that rearranges the prose** rather than written once per pass — each of
 * them makes the same promise (the words are the model's, the arrangement is ours) and a copy that
 * drifts is a pass whose promise is weaker than it reads.
 */
function textBag(lines) {
  const m = new Map();
  for (const l of lines) {
    const t = l.trim();
    if (!t || headingOf(l)) continue;
    m.set(t, (m.get(t) ?? 0) + 1);
  }
  return m;
}

/** The first key whose count differs between two `textBag`s, or null */
function bagDiff(inBag, outBag) {
  for (const [text, n] of inBag) {
    const got = outBag.get(text) ?? 0;
    if (got !== n) return { text, n, got };
  }
  return null;
}

/**
 * Every `<details>` block's contents, flattened and sorted — **structure, which counting text
 * cannot see**. Tearing a toggle into an empty shell plus loose siblings loses no character at all.
 */
function toggleBodies(arr) {
  const got = [];
 ...
 *
 * **Only "adjacent identical headings" are merged here.** The test is: this shard's first heading
 * line is exactly equal to the **last** heading in what has been joined so far. That is all this
 * function can decide — it runs on every round, over a document still being written, where a
 * heading appearing twice with others in between may simply be the next shard's first section.
 * Titles repeated further apart are `mergeDuplicateSections`' job, once at the end.
 *
...
/**
 * Folds sections carrying the same title into the one that used it first.
 *
 * **A repeated title is an index that does not work.** 月圆之夜 (162 achievements, four shards)
 * landed with 「购买内容」 three times holding 1, 2 and 1 entries, 「镜中的记忆」 four times, and two
 * separate 「职业通关 · 普通难度」 inside a single shard's own output. Nothing was missing and
 * nothing failed; a reader looking up one section simply finds it in three places. `joinBodies`
 * cannot see any of that — it compares each shard's first heading against the last one written so
 * far, which is the most a pass running mid-document can decide.
 *
 * **Identical titles only, and only between siblings.** Two rules, each protecting a real case:
 *
 * - The title must match **exactly** once whitespace and case are set aside (`normSectionTitle`).
 *   「经典模式:通用挑战成就」 and 「经典模式·通用挑战」 are one topic to a reader and stay two
 *   sections here — reconciling differently-worded ones is the classification pass's judgement to
 *   make, and a program that guessed at it would eventually merge two sections that differ for a
 *   reason.
 * - **Same parent, same level.** `### 角色通关` under 「镜中的记忆」 and `### 角色通关` under
 *   「愿望之夜」 are two different sections that happen to share a subtitle; merging those would
 *   move entries into the wrong game mode. A section owns its subsections, so folding one into an
 *   earlier twin carries its children with it.
 *
 * The document is otherwise untouched: with no duplicates the output is the input, byte for byte.
 *
 * @returns {{text: string, merged: {title: string, folded: number}[]}} `folded` counts the
 *   occurrences that were folded away, so the count reported is repeats rather than sections.
 */
export function mergeDuplicateSections(body) {
  const lines = String(body ?? '').split(/\r?\n/);
  const { preamble, roots } = sectionTree(body);

  const merged = [];
  const note = (title) => {
    const rec = merged.find((m) => m.title === title);
    if (rec) rec.folded++;
    else merged.push({ title, folded: 1 });
  };
  ...
  const fold = (siblings) => {
    ...
    for (const node of siblings) {
      // The level is part of the key: `### 收集` and `#### 收集` are two different things, and
      // merging them would lose the level — the same rule `joinBodies` holds to at a seam
      const key = `${node.level} ${normSectionTitle(node.title)}`;
      ...
      // **One blank line between the two bodies, never a run of them.** Both halves keep their own
      // spacing otherwise; the join is the only place this function decides anything about layout
      const tail = first.own;
      while (tail.length && !tail[tail.length - 1].trim()) tail.pop();
      const add = node.own.slice();
      while (add.length && !add[0].trim()) add.shift();
      if (tail.length && add.length) tail.push('');
      tail.push(...add);
      first.children.push(...node.children);
      note(first.title);
    }
    for (const node of kept) node.children = fold(node.children);
    return kept;
  };
  ...
  // ---- The prose is the model's and only the arrangement is ours: prove it
  const outLines = text.split('\n');
  const lost = bagDiff(textBag(lines), textBag(outLines)) ?? bagDiff(textBag(outLines), textBag(lines));
  if (lost) throw msgError('gen.mergeLostText', { text: lost.text.slice(0, 40), n: lost.n, got: lost.got });
  const tIn = toggleBodies(lines);
  const tOut = toggleBodies(outLines);
  if (tIn.length !== tOut.length || tIn.some((x, i) => x !== tOut[i])) {
    throw msgError('gen.mergeBrokeToggles', { in: tIn.length, out: tOut.length });
  }
...
export function regroupByAssignment(body, { defs, assignment, sections = [] } = {}) {
  ...
  const touch = (title, level = 2) => {
    const k = normSectionTitle(title);
    if (!bucket.has(k)) {
  ...
  for (const b of bucket.values()) {
    ...
    const target = bucket.get(normSectionTitle(best));
  ...
  /**
   * **Deduped, because one bucket emitted twice writes its achievements twice.**
   *
   * `parseRegroupReply` already drops a repeated `== 标题`, but only when the two are the same
   * string to the character. Two spellings of one title — a trailing space, a full-width space, a
   * difference in case — survive that and both resolve to this one key, so the section is emitted
   * once per spelling, its entries duplicated. Assertion 2 then catches the duplication and throws,
   * which costs the **whole** classification: the guide lands with each shard's own headings.
   *
   * Measured on 月圆之夜 twice in a row (162 achievements, four shards). Reproduced offline by
   * handing this function `['愿望之夜', '愿望之夜 ']`.
   */
  const wanted = [...new Set(sections.map(normSectionTitle))].filter((k) => bucket.has(k));
  ...
  const lost = bagDiff(textBag(lines), textBag(out.split('\n')));
  if (lost) throw msgError('gen.regroupLostText', { text: lost.text.slice(0, 40), n: lost.n, got: lost.got });
...
/**
 * The follow-up for the achievements the classification reply left out.
 *
 * **Measured: 9 of 162 on 月圆之夜**, and not at the tail — #57, #74–76, #131, #139–142, dropped in
 * topic-sized clumps (「第四章」+「梦魇乐章」 together, the four 狼人/巫师精通 tiers together). The
 * prompt already says every number appears exactly once; a model enumerating 162 of them misses a
 * few anyway, and each one left out keeps whichever section its shard happened to open, which is
 * how a reconciled guide still ends with four one-entry sections at the bottom.
 *
 * **The sections are closed.** This asks only "which of these", never "make one" — the count was
 * settled by the pass before it, and a follow-up inventing a twelfth section would undo that.
 *
 * It runs in the **same session**, so the model is looking at its own answer when told what it left
 * out; that is also what makes the ask small.
 */
export function buildRegroupFillPrompt(defs, missing, sections, lang = 'zh') {
  const rows = missing.map((i) => {
    const d = defs[i];
    const desc = lang === 'en'
      ? (d.description_en ?? '').trim() || (d.description ?? '').trim()
      : (d.description ?? '').trim();
    return lang === 'en'
      ? `${i + 1}. ${defNameIn(d, 'en')} — ${desc || '(no description)'}`
      : `${i + 1}. ${defName(d)} — ${desc || '(无描述)'}`;
  }).join('\n');
  const list = sections.map((x) => `- ${x}`).join('\n');
  return lang === 'en'
    ? 'These achievements were left out of the grouping above. Put each of them into one of the '
      + 'sections you just listed.\n\n'
      + `- **Use only these sections**, exactly as written; do not add one:\n${list}\n`
      + '- **Every number below appears exactly once**, and nothing else does\n\n'
      + 'Output format (exactly this, nothing else):\n'
      + `\`\`\`\n== Section title\n${missing[0] + 1}\n\`\`\`\n\n---\n\n${rows}`
    : '上面的分组把下面这些成就漏掉了。把每一个放进你刚才列出的小节里。\n\n'
      + `- **只能用这些小节**,照抄标题,不要新开:\n${list}\n`
      + '- **下面每个编号都要出现且只出现一次**,别的编号一个都不要写\n\n'
      + '输出格式(严格照这个,不要别的):\n'
      + `\`\`\`\n== 小节标题\n${missing[0] + 1}\n\`\`\`\n\n---\n\n${rows}`;
}
...
export async function generateGuide(db, {
  ...
  const steps = generationSteps({ chunks: chunks.length, existing: Boolean(plan.existing) });
  const step = stepReporter(steps, onProgress);
  /**
   * The writing step for a given number of **finished** shards — the shard being asked for next.
   * Capped at the last writing step, since a shard that split mid-run pushes the count past the
   * list settled before the run (see generationSteps).
   */
  const writeSteps = writeStepCount(steps);
  const writeStep = (done) => step(writeSteps > 1 ? `write:${Math.min(done, writeSteps - 1) + 1}` : 'write');
  onProgress({ phase: 'plan', chunks: chunks.length, achievements: defs.length, steps });
  writeStep(0);
  ...
  const ask = async (session, prompt, label, expected = 0) => {
    /**
     * **The one place a long run has anything to say per achievement.** A shard is written in a
     * single streaming response, so there is no per-achievement event to listen for — but the prose
     * arrives one `- [ ]` line at a time, in a format the linter enforces, so counting them is a
     * real figure rather than an estimate.
     *
     * Reported only when the count changes: the deltas are a few characters each and reporting per
     * delta would be thousands of events saying the same number.
     */
    let streamed = '';
    let counted = -1;
    const reply = await session.ask(prompt, {
      ...
      onEvent: (ev) => {
        if (ev.type === 'text') {
          streamed += ev.text ?? '';
          const done = countStreamedEntries(streamed);
          if (done !== counted) {
            counted = done;
            onProgress({ phase: 'written', round, label, done, of: expected });
          }
          return;
        }
        // Generic progress events; knows no vendor's raw format. The provider hands over a
        // vocabulary word and `toolNote` turns it into a sentence
        if (ev.type === 'tool') {
          const said = toolNote(ev.name);
          onProgress({ phase: 'tool', round, name: said, label });
        }
  ...
  const askChunk = async ({ session, chunkAt, setChunks, setBody, locate, prompt }) => {
    ...
    for (;;) {
      ...
      try {
        setBody(await ask(session, prompt(index, total), label, (chunkAt() ?? []).length));
        // **The step advances where a shard lands, not where one is asked.** Round 1 asks them
        // concurrently, so at ask time every shard reads the same figure and the count would sit at
        // its first value for the whole round. It is the same figure as `done` above — see
        // generationSteps for why it is finished-shards rather than which-shard
        writeStep(liveBodies().filter(Boolean).length);
        return null;
      } catch (err) {
  ...
  /**
   * ---- Fold sections that carry the same title ----------------------------
   *
   * **Mechanical, and it runs whether or not the pass below will.** Two sections with the same
   * title are the one part of a messy sectioning that needs no judgement at all, so this half keeps
   * working when the classification pass fails — which is exactly the case that produced 月圆之夜's
   * 「购买内容」 three times over.
   *
   * It runs **before** that pass so the classification prompt is shown the guide's sectioning
   * rather than the shards' seams, and it holds the same rollback rule as the passes on either
   * side: by now the draft is a landable finished product.
   */
  if (!blocking.length && !chunkFailures.length) {
    const before = readFileSync(draftPath, 'utf8');
    try {
      const { text, merged } = mergeDuplicateSections(before);
      if (merged.length) {
        ...
        if (recheck.blocking.length) {
          writeFileSync(draftPath, before);
          onProgress({ phase: 'merge-failed', reason: msg('gen.mergeRecheckFailed', { n: recheck.blocking.length }) });
        } else {
          lint = after;
          onProgress({
            phase: 'merged-sections',
            sections: merged.length,
            folded: merged.reduce((n, m) => n + m.folded, 0),
          });
        }
      }
    } catch (err) {
      writeFileSync(draftPath, before);
      onProgress({ phase: 'merge-failed', reason: String(err?.message ?? err) });
    }
  }
   ...
   *
   * **Asked twice before it degrades.** Every way this pass fails except a cancellation depends on
   * what came back — a reply judged unusable, a grouping nothing could be read out of, an
   * assignment that loses a line when the prose is rearranged by it. A second ask is one cheap
   * call with no research in it, against a failure that costs the whole sectioning of a guide that
   * already cost real money. Each attempt gets its own session: a rejected reply left in the
   * context is the model being shown its own bad answer and asked to do better.
   */
  if (!blocking.length && !chunkFailures.length && chunks.length > 1) {
    ...
    for (let attempt = 1; attempt <= REGROUP_ATTEMPTS; attempt++) {
      try {
        ...
        const grouper = createSession(provider, { system: regroupSystemFor(plan.lang), effort: ASIDE_EFFORT });
        ...
        /**
         * **What the reply left out gets one small follow-up.**
         *
         * An unassigned achievement keeps whichever section its own shard opened, which is the
         * right fallback and still leaves a reconciled guide ending in a row of one-entry sections:
         * measured at 9 of 162 on 月圆之夜, dropped in topic-sized clumps rather than at the tail.
         * The program knows exactly which ones, so asking about those is cheap and mechanical.
         *
         * **Failure here degrades to what the first reply gave** — every one of them already has a
         * home, so this costs tidiness and never a guide.
         */
        const missing = defs.map((d, i) => i).filter((i) => !parsed.assignment.has(defs[i].api_name));
        if (missing.length) {
          onProgress({ phase: 'regroup-fill', missing: missing.length });
          try {
            const allowed = new Set(finalSections.map(normSectionTitle));
            const more = await grouper.ask(buildRegroupFillPrompt(defs, missing, finalSections, plan.lang), { signal });
            const fillVerdict = checkResult(more);
            if (!fillVerdict.ok) throw new Error(fillVerdict.reason);
            let filled = 0;
            for (const [apiName, section] of parseRegroupReply(more.text, defs).assignment) {
              // **A closed list.** The section count was settled by the pass before this one, and a
              // follow-up inventing a twelfth would undo it. An answer naming a section that is not
              // on the list is dropped, and that achievement keeps the one its shard opened — the
              // same place it would have kept had this ask never been made
              if (parsed.assignment.has(apiName) || !allowed.has(normSectionTitle(section))) continue;
              parsed.assignment.set(apiName, section);
              filled++;
            }
            onProgress({ phase: 'regroup-filled', filled, of: missing.length });
          } catch (err) {
            if (err?.cancelled) throw err;
            onProgress({ phase: 'regroup-fill-failed', reason: String(err?.message ?? err) });
          }
        }
        ...
        // **It is also the one failure a retry must not swallow** — asking again is the opposite of
        // what a Cancel click means.
        if (err?.cancelled) {
          writeFileSync(draftPath, beforeText);
          throw err;
        }
        // **Roll back to the version before the rearrangement.** The draft is a finished product that
        // has been ticked and passed the gates; a half-done one must not be left behind. It happens
        // before the retry too: the next attempt has to rearrange the same prose this one saw
        writeFileSync(draftPath, beforeText);
        if (attempt < REGROUP_ATTEMPTS) {
          onProgress({ phase: 'regroup-retry', attempt, reason: String(err?.message ?? err) });
          continue;
        }
...
export async function planGuide(db, {
  ...
  // **Resolved before the object rather than inside it**, because `unnameable` below is a question
  // about this language and an object literal cannot read its own field
  ...
  return {
    ...
    notion: notionPlan,
    fileName: name,
    // **Which achievements ticking cannot reach depends on the language above**, because the names
    // on the page are that language's. See unnameableApiNames.
    unnameable: unnameableApiNames(defs, lang),
