...
import {
  spoilerSystemFor, buildSpoilerMessage, guideEntries, parseSpoilerReply, foldSpoilers, ASIDE_EFFORT,
...
export const SKILL_RULE_DISPOSITION = {
  ...
  'rule-5/spoiler': '进了 —— 剧透折叠,写在「标注用固定写法」那一节:固定标签、紧跟成就那一行、里面写段落、不受 10 行下限约束、要少。后端差异那一段**没进**(哪个后端是程序的事,提示词全篇不提落到哪里,同 8.0)',
...
- **剧透**:一句话如果**说出了故事本身的事实**(结局是什么、反转是什么、某个角色到底是谁、
  某段剧情实际发生了什么),把那句话折起来,不要直接写进心得:
      - [ ] **成就名**<br>官方描述<br>不剧透的那半做法
        <details>
        <summary>剧透</summary>
        具体内容写在这里,写成段落
        </details>
  - **只有这一类折。** 怎么操作、在哪、要刷几次、难不难、会不会错过,都不是剧透,照常写在心得里
  - **隐藏成就(清单里标着「(Steam 上是空的)」的那些)最常出现这种句子** —— Steam 什么都不给,
    你写的那一句是这个条件在任何地方唯一的出处。但不是每个隐藏成就都要折,只有那句话真的说出
    了故事事实才折
  - **\`<summary>\` 里只写「剧透」两个字。** 这个标签是读的人不点开也会看到的东西,
    写成「剧透:凶手是医生」等于没折
  - **成就那一行要留下不剧透的那半**,让人不点开也知道折叠里是哪一类东西、值不值得点
  - **折叠块紧跟在成就那一行后面、缩进更深,中间不能空行**
  - **折叠里写段落,不要放 checkbox。** 折叠里的 checkbox 会当成子步骤处理,父成就一解锁就
    连带勾上,变成一条假记录
  - **不受上面那个「10 行」下限的约束** —— 它折起来的理由是内容剧透,不是内容长
...
- **Spoiler**: when a sentence **states a fact about the story itself** (how it ends, what the twist
  is, who somebody turns out to be, what actually happened in some scene), fold that sentence away
  rather than writing it straight into your notes:
  ...
  - **That is the only thing folded.** How to do it, where it is, how many runs it takes, how hard it
    is, whether it can be missed — none of that spoils anything, and all of it stays in the notes
  - **Hidden achievements (the ones the list marks "(empty on Steam)") are where such a sentence
    turns up most often** — Steam gives nothing, so the line you write is the only place that
    condition appears anywhere. Not every hidden achievement needs a fold: only the ones whose line
    really does state a fact about the story
  ...
  - **The achievement's own line keeps the half that spoils nothing**, so a reader can tell what kind
    of thing is inside and whether it is worth opening, without opening it
 ...
 * whether the classification pass will run (`chunks > 1`), whether the spoiler pass was asked for,
 * whether there is an existing guide to back up first.
 *
 ...
 */
export function generationSteps({ chunks = 1, spoilerFold = false, existing = false } = {}) {
  const steps = ['write'];
  if (spoilerFold) steps.push('spoiler');
  if (chunks > 1) steps.push('regroup');
...
export async function generateGuide(db, {
  ...
  local = false,
  overwrite = false,
  // **Fold spoilers away** (lib/guidespoiler.js): one more request after the guide is written,
  // asking which sentences give away the story, then moving those into a fold.
  //
  // **An argument rather than a setting**, and off unless asked for. It is a decision about
  // spending, so it belongs where the spend is already confirmed — the dialog and the flag — and
  // not in a file somebody edited once and forgot. A guide is usually written for a game already
  // finished, where there is nothing left to spoil, so the common case should not be paying for it.
  spoilerFold = false,
  plan: given = null,
  onProgress = () => {},
  ...
  const steps = generationSteps({ chunks: chunks.length, spoilerFold, existing: Boolean(plan.existing) });
  /** Mark a step as started. `of` never changes during a run — see generationSteps */
  const step = (key) => onProgress({ phase: 'step', key, n: steps.indexOf(key) + 1, of: steps.length });
  ...
  const ask = async (session, prompt, label, expected = 0) => {
    ...
    const reply = await session.ask(prompt, {
      ...
      onEvent: (ev) => {
        ...
        // **The provider hands over a vocabulary word, and this turns it into a sentence.** The
        // word itself used to go on screen — `web_search`, and before the tool names were
        // normalised, `web_search_20260209`. An unknown word is still passed through: a vendor
        // adding a tool should read oddly rather than vanish
        if (ev.type === 'tool') {
          const said = TOOL_NOTES[ev.name] ? msg(TOOL_NOTES[ev.name]) : ev.name;
  ...
  /**
   * ---- Fold the spoilers away ---------------------------------------------
   *
   * **Also before the rearrangement**, for the same reason as unwrapping: this pass *creates*
   * toggles, and `regroupByAssignment`'s third assertion compares toggle contents, so run afterwards
   * it would be judged to have torn one open and roll the whole rearrangement back.
   *
   * Why this is a pass at all, rather than the prompt rule it started as, is in `guidespoiler.js`
   * and `docs/ai-guide-writing.md`: three live runs, two wordings, zero folds, and the same model
   * answering correctly the moment the question is asked on its own.
   *
   * **Any failure degrades to "nothing folded" and never aborts** — the same trade the classification
   * pass makes, for the same reason: the prose is written, ticked and through the gates by now, and
   * losing it over a cosmetic pass is a bad deal. Cancellation is the one exception, handled below.
   */
  if (spoilerFold && !blocking.length && !chunkFailures.length) {
    const beforeText = readFileSync(draftPath, 'utf8');
    const entries = guideEntries(beforeText);
    if (entries.length) {
      try {
        step('spoiler');
        onProgress({ phase: 'spoiler', entries: entries.length });
        const finder = createSession(provider, { system: spoilerSystemFor(plan.lang), effort: ASIDE_EFFORT });
        asideSessions.push(finder);
        const reply = await finder.ask(buildSpoilerMessage(entries.map((e) => e.text), plan.lang), { signal });
        const verdict = checkResult(reply);
        if (!verdict.ok) throw new Error(verdict.reason);

        const { picks, discarded } = parseSpoilerReply(reply.text, entries);
        const folded = foldSpoilers(beforeText, picks, defs, plan.lang);
        if (folded.applied.length) {
          writeFileSync(draftPath, folded.text);
          const after = lintGuide({
            todos: loadTodos(draftPath),
            defs,
            text: folded.text,
          ...
          const recheck = splitFindings(after.findings, plan.unnameable);
          if (recheck.blocking.length) throw msgError('gen.recheckFailed', { n: recheck.blocking.length });
          lint = after;
          onProgress({
            phase: 'spoiler-done',
            folded: folded.applied.length,
            skipped: discarded.length + folded.reverted.length,
          });
        }
      } catch (err) {
        // Same rule as the classification pass: a Cancel click must not come back as "finished
        // anyway, just without the fold"
