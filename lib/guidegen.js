 ...
 * 新生成的攻略页给什么状态。
 *
 * `Staged` = 攻略写好了、游戏还没打完 —— 正是 `syncGuideStatuses` 把一个跌破 100%
 * 的页面降回去的那个值。选它是因为**它会自我修正**:如果这个游戏其实已经满成就,
 * 下一次 `guide-status` 会自动把它提成 `Done`,不用在这里再判一次完成度。
 */
export const NOTION_NEW_STATUS = 'Staged';
...
export async function planGuide(db, { config, steam, appid, fileName = null, notion = null, local = false }) {
  ...
  const notionPlan = target === 'notion' ? await planNotionTarget(notion, game) : null;
 ...
 */
export async function planNotionTarget(notion, game) {
  const schema = await notion.fetchGuideDbSchema();
  if (schema.status && !schema.status.options.includes(NOTION_NEW_STATUS)) {
    throw new Error(
      `Notion 攻略库的「${schema.status.property}」属性里没有「${NOTION_NEW_STATUS}」这个选项。` +
  ...
  return {
    ...
    status: schema.status ? { ...schema.status, value: NOTION_NEW_STATUS } : null,
    existingPage,
  };
}
