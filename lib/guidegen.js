 ...
 *
 * `sections` 给一份封闭的小节名单就把标题钉死,不给就退回「标题照开、程序合」那套。
 * **生成路径上现在恒为空** —— 分类挪到了正文写完之后(见上面那段注释),写的时候
 * 各段照旧自己开标题。分岔留着是因为它和 `regroupBySections` 是同一套逻辑的两半,
 * 拆掉一半会让另一半的注释讲不通。
 */
...
export async function generateGuide(db, {
  ...
  /**
   * **写的时候没有分区表 —— 分类挪到了全篇写完之后**(见下面的 `phase: 'regroup'`)。
   *
   * 写之前那一趟手上**只有成就名**,而名字是梗的游戏就此失手:《马特的寻猫游戏》的
   * 「海拉鲁老流氓」其实是打碎 100 个罐子、「半条命4」是用撬棍撬容器,只看名字分出来
   * 的是「自然与美食」「囤积狂的自我修养」;补上描述再跑一次,依然丢掉了「难度模式」
   * 这种真正要紧的结构。**信息不够,不是提示词不够。**
   *
  ...
  // 拼接走 regroupBySections。`sections` 在这条路上恒为空,所以实际走的是 joinBodies 的接缝合并
