...
export function mergeDuplicateSections(body) {
  ...
  // ---- Parse into a forest: anything before the first heading is the preamble
  const preamble = [];
  const roots = [];
  const open = [];
  for (const line of lines) {
    ...
    if (!h) {
      (open.length ? open[open.length - 1].own : preamble).push(line);
      continue;
    }
    const node = { level: h[1].length, title: h[2], line, own: [], children: [] };
    while (open.length && open[open.length - 1].level >= node.level) open.pop();
