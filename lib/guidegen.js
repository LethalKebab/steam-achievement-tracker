 ...
 *
 * **Why this table is needed:** the `RULES` below are a **hand-copied summary** of SKILL.md (about a
...
export const SKILL_RULE_DISPOSITION = {
  ...
  // This batch was added later. The guard originally recognised only `## 规则N` and `### N.N`, so
  // adding an unnumbered subsection under some rule left the disposition table completely silent —
  // which is exactly how rule two had its conclusion changed with nobody noticing (see the rule two
  // entry below). The key is `规则N/子节标题`.
...
/** The achievement list. The ⚠️ 同名 marker tells the model outright which entries must copy the description, which beats making it count for itself */
...
function chunkFloorAdvice(err, round, chunk) {
  ...
  err.message += `\n这一段已经切到 ${chunk.length} 个成就仍未生成,换个模型或供应商可能更合适。`;
  return err;
}
...
export function regroupByAssignment(body, { defs, assignment, sections = [] } = {}) {
  ...
  if (before.length !== after.length || before.some((x, i) => x !== after[i])) {
    throw new Error(msg('gen.regroupLostAch', { before: before.length, after: after.length }));
  }
  ...
  for (const [text, n] of inBag) {
    ...
    if (got !== n) {
      throw new Error(msg('gen.regroupLostText', { text: text.slice(0, 40), n, got }));
    }
  }
  ...
  if (tIn.length !== tOut.length || tIn.some((x, i) => x !== tOut[i])) {
    throw new Error(msg('gen.regroupBrokeToggles', { in: tIn.length, out: tOut.length }));
  }
...
export async function generateGuide(db, {
  ...
  if (!Number.isInteger(rounds) || rounds < 1) {
    throw new Error(msg('gen.badRounds', { rounds }));
  }
  ...
  for (round = 1; round <= rounds; round++) {
    ...
    if (!blocking.some((f) => MODEL_FIXABLE.has(f.code))) {
      throw new Error(msg('gen.nothingModelCanFix', {
        path: draftPath,
  ...
  if (!blocking.length && !chunkFailures.length && chunks.length > 1) {
    ...
    try {
      ...
      if (!finalSections.length) throw new Error(msg('gen.noGroups'));
      ...
      const recheck = splitFindings(after.findings, plan.unnameable);
      if (recheck.blocking.length) {
        throw new Error(msg('gen.recheckFailed', { n: recheck.blocking.length }));
      }
      lint = after;
      onProgress({
  ...
  if (ok && plan.target === 'notion') {
    ...
    const recheck = splitFindings(after.findings, plan.unnameable);
    if (recheck.blocking.length) {
      throw new Error(msg('gen.finalRecheckFailed', {
        path: finalPath,
...
export async function landToNotion(
  ...
  if (plan.existing?.kind === 'notion') {
    if (!backup?.blocks) throw new Error(msg('gen.noBackupBeforeOverwrite'));
...
async function finishNotionLanding(
  ...
  const recheck = splitFindings(after.findings, plan.unnameable);
  if (recheck.blocking.length) {
    throw new Error(msg('gen.notionRecheckFailed', {
      url: page.url,
      problems: recheck.blocking.map((f) => f.message).join('; '),
    }));
  }
  ...
  const registered =
    ...
    ((await notion.extractAppIdFromPageContent(page.id)) === String(appid)
      ...
      : null);
  if (!registered) {
    throw new Error(msg('gen.appidNotFound', { url: page.url, appid }));
  }
...
export async function planGuide(db, {
  ...
  if (defs.length > max) {
    const err = new Error(msg('gen.tooManyAch', { n: defs.length, max }));
  ...
  if (existing && !overwrite) {
    ...
    const err = new Error(msg('gen.alreadyHasGuide', { game, where, url: existing.url }));
  ...
  if (target === 'local' && !existing && existsSync(finalPath)) {
    const err = new Error(msg('gen.fileExists', { path: finalPath }));
  ...
  if (existing) {
    if (existing.kind === 'local') {
      ...
      if (!existsSync(path)) throw new Error(msg('gen.guideFileMissing', { path }));
  ...
  if (raw.retry) throw new Error(msg('gen.noUnlockState', { id }));
  if (raw.noAchievementSystem) throw new Error(msg('gen.noAchData', { id }));
