 ...
 *    时是同一条(见文件头 1.)。
 *
 * `fresh` 是给"我不要你参考,重新查一遍写"准备的出口:原文会被抽掉,只留下清单。
 * 默认**给**原文 —— 「写详细点 / 补上前置条件 / 把这段改成表格」这类要求占绝大多数,
 * 而它们全都以看得见原文为前提。
 */
export function buildPatchMessage(entries, { instruction = null, fresh = false } = {}) {
  const lines = entries.map((e, i) => {
    ...
    if (!fresh) body.push(`   现在写的:${String(e.text ?? '').replace(/\n/g, ' ') || '(空)'}`);
    return body.join('\n');
  });
  ...
  return (
    ...
    `${ask}\n\n` +
    `要重写的是这 ${entries.length} 条${fresh ? '' : ',连同它们现在的写法'}:\n\n` +
    lines.join('\n\n') +
    '\n\n输出要求:\n' +
