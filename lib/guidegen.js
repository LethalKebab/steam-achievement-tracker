 ...
 * 五个成就的正文两三千字,撑不满任何一家的单次上限,还被截断说明吃掉额度的是
 ...
 * 这次失败能不能靠"切小再问"补救。
 *
 * **只有截断能。** 拒答、RECITATION、搜索报错都不是长度问题,切小了照样撞;
 * 而且它们各自的正确处置完全不同(换问法 / 换游戏 / 查网络),当成长度问题
 * 重试一遍只是把同一个错误再犯一次,还多花一次联网的钱。
 *
 * 只在第一轮切。之后几轮是拿校验结果定点重写,段长已经是第一轮跑通过的值,
 * 这时候再截断多半不是长度问题;而且重写轮的下标和 lint.findings 是绑定的,
 * 中途改段数会让那份对应关系失效 —— 那是另一个坑,不在这次的范围里。
 */
function canSplit(err, round, chunk) {
  return err?.code === 'max_tokens' && round === 1 && chunk.length > MIN_CHUNK;
}

/** 切不动了的时候,把"为什么调大 maxTokens 多半没用"当场说清楚 */
function truncationAdvice(err, chunk) {
  if (err?.code !== 'max_tokens') return err;
  err.message +=
    `\n这一段已经切到 ${chunk.length} 个成就(下限 ${MIN_CHUNK})还是写不完。` +
    '\n**先别急着调大 ai.maxTokens** —— 它管的是 thinking + 正文的总额,' +
    '而正文这么短还超,说明吃掉额度的是思考,加大只会让它想得更久。' +
    '\n可查的方向:换一个能压住思考的端点(官方 Anthropic / Gemini 才发得出 ' +
    'thinking 参数,兼容端点发过去是 400,见 ai.anthropicExtras),或者换个模型。';
  return err;
}
...
export async function generateGuide(db, {
  ...
  for (round = 1; round <= rounds; round++) {
    // **第一轮用 while,因为段数会在循环里变** —— 见下面 max_tokens 的处理。
    // 之后几轮是按校验结果定点重写,`targets` 是那时算好的下标,不能中途变
    const order = round === 1 ? null : targets;
    for (let i = 0, k = 0; order ? k < order.length : i < chunks.length; ) {
      if (order) i = order[k];
      ...
      try {
        ...
        if (!canSplit(err, round, chunks[i])) throw truncationAdvice(err, chunks[i]);
        // **被 max_tokens 截断 ⇒ 这一段太长了,一分为二只重问这两半。**
        //
        // 这里用的是**已经发生的事实**,不是开跑前的估算 —— 装进单次请求的是
        // thinking + 正文,而 thinking 随游戏、模型、端点变,还在兼容端点上根本
        // 发不出参数去压(见 ai-anthropic.js 的 extras)。任何"先算出该切多大"的
        // 做法都是在预测那个量;截断本身则是量到的,它直接说明这一段越了界。
        // 每次至少减半,所以必然在几步内收敛到 MIN_CHUNK,不需要再加一个次数上限。
        //
        // **摘掉刚才那一轮是这件事的前提,不是收尾。** 半份废稿留在上下文里,而
        // 重问的提示词写着"不要重复前面已经写过的成就" —— 模型会跳过它写了一半的
        // 那几个,产出看着正常但是缺条目
        session.dropLastTurn();
        const half = Math.ceil(chunks[i].length / 2);
        const [a, b] = [chunks[i].slice(0, half), chunks[i].slice(half)];
        chunks.splice(i, 1, a, b);
        onProgress({
          phase: 'resplit', round, chunk: i + 1, chunks: chunks.length,
          from: a.length + b.length, to: a.length,
        });
        continue; // 同一个下标重来,现在它是前一半
      }
      if (order) k++;
      else i++;
    }
