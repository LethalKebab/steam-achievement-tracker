...
export const SKILL_RULE_DISPOSITION = {
  ...
  '规则二': '**部分** —— 截图整块没进(模型给不出可靠的游戏内截图,而且它看不见图,没法核对贴的对不对);「贴不了截图的时候」那一节**进了**,细则见下面的子节',
  ...
  '规则五': '进了 —— <details> 折叠(带 10 行下限、成就本身不进折叠)、表格取舍。删除线没进(生成新攻略用不上)',
  '规则六': '进了 —— 中英文混用',
  '规则七': '进了 —— 不写数据来源',
  ...
  // ---- 不带编号的 `###` 子节 -------------------------------------------
  // 这一批是补出来的。守卫原来只认 `## 规则N` 和 `### N.N`,于是往某条规则底下
  // 加一个不带编号的子节,处置表一个字都不会响 —— 规则二就是这么被改了结论
  // 而没人发现的(见下面规则二那一条)。key 是 `规则N/子节标题`。
  '规则一/心得段:前置、步骤、警告分行写': '进了 —— 前置/步骤/警告分行写,判据是「有没有把两类东西挤进一行」,不是字数',
  '规则一/子 checkbox 的分组标签': '进了 —— 但**按后端分岔**(见 groupLabelRule),是整份提示词里唯一一处分岔',
  '规则一/子任务/子收集品:嵌套 checkbox 要满足三个条件': '进了 —— 嵌套的三个条件(有身份、写得出做法、都要做)逐字同口径,测试钉着',
  '规则一/重名成就的处理': '进了 —— 硬规则 5:清单里标了 ⚠️ 同名 的,描述原文必须抄进去',
  '规则一/写后验证': '**没进** —— 程序做:落盘后重新读一遍再过一次校验器,不指望模型自查',
  '规则二/值得贴的截图': '**没进** —— 模型给不出可靠的游戏内截图,而且它看不见图,没法核对贴的对不对',
  '规则二/贴不了截图的时候:带时间点的视频链接': '**进了** —— 位置写不出来时给带时间点的视频链接,并点名挡掉「留意角落」那类万能话。规则二整条是「没进」,唯独这一节进了 —— 早先处置表只记到规则一级,这一节加进 SKILL.md 时没人发现结论变了',
  '规则二/不要贴的截图': '**没进** —— 根本不贴图,这一节没有约束对象',
  '规则二/来源攻略有图时的处理': '**没进** —— 同上。「跟着配等价的图」对一个贴不了图的写手是空话',
  '规则五/表格的使用': '进了 —— 列数多、纯文字对不齐的可以用 <table>,能用 checkbox 列表说清的优先用列表',
  '规则五/删除线的使用': '**没进** —— 删除线是改已有攻略时用的,生成器每次写全新一份',
...
/**
 * 分类**不在写正文之前做**,在写完之后做(见 `buildRegroupPrompt`)。
 *
 * 写之前那一趟手上只有成就名,而成就名常常是梗:《马特的寻猫游戏》的「海拉鲁老流氓」
 * 其实是打碎 100 个罐子。只看名字分出来的是「自然与美食」「囤积狂的自我修养」这种
 * 主题化标题,而且会把同类成就劈进两个小节。补上描述再跑一次也一样 ——
 * **信息不够,不是提示词不够**:分类要的是"查完资料之后知道这条到底要干什么",
 * 那份信息只有正文写完才存在。
 */
 ...
 * 就是他还会用到的全部。省下来的是这几条的查资料和正文,而那正是这个功能唯一花钱的
 * 地方(见 CLAUDE.md 里那次 A/B:时间几乎全在模型思考和联网上,不在字数上)。
 *
 * **全解锁的游戏一条都不省。** 那时候"省下来的"就是整篇攻略 —— 剩下一串只有名字和
 * 官方描述的行,而那份东西 Steam 页面上本来就有。会给一个 100% 的游戏生成攻略的人,
 * 要的恰恰是内容。
 *
 * 这里只管**新写的那一篇**。覆盖重写不走这条路:那时候攻略里已经有花钱写出来的正文,
 * 而"他后来解锁了"不是把那段字删掉的理由 —— 见 generateGuide 里传参那一处。
 */
...
/**
 * 这一段里哪几条只写一行。**点名少的那一半** —— 大部分游戏是"已经解锁了一大半",
 * 那时候列出"要写详细的这几个"比列出"要略写的那四十个"短得多,而且读起来正好是
 * 模型这一段真正要干的活。
 */
function briefInstruction(chunk, brief) {
  const all = chunk ?? [];
...
/**
 * 这一段该写什么。只有一段、又没有要略写的,就是原来那句话,行为一个字都没变。
 */
export function buildChunkMessage(chunks, i, brief = new Set()) {
  ...
  return (
    ...
    // 重了程序合 —— 接缝上的重复由 `joinBodies` 合掉,真正的归类是全篇写完之后
    // 那一趟的事(见 `buildRegroupPrompt`)
 ...
 * 把**装着成就本身**的顶层折叠拆开。
 *
 * **规则五的折叠是给长内容用的,不是给成就列表用的。** 但规则五只说了"内容到 10 行才折",
 * 没说"成就本身永远不折" —— 实测《马特的寻猫游戏》整节 `## 世界全清` 的 13 条成就被
 * 塞进了一个叫「世界 1~12 全清与通关」的折叠里,于是那一节在 Notion 上直接显示 0 条。
 * 功能没坏(同步照样认得出),但一节点开是空的,读起来就是个 bug。
 *
 * 两条同时成立才拆,少一条都不动:
 *
 * 1. **折叠在顶层(不缩进)。** 分组标签(前置/步骤/注意)那种折叠一定是缩进在某条成就
 *    底下的(见 `groupLabelRule` 和 SKILL.md 的例子),不缩进的只可能是小节级长清单。
 * 2. **里面有 checkbox 能反查到真成就。** 子步骤反查不到 —— `resolveTodoToAchievement`
 *    要么描述全文命中、要么成就名精确相等,两条都要求唯一,不做前缀匹配。
 *
 * `<summary>` 的文字不丢,降成一行加粗:它是这一组的标签,拆了外壳还得有人说这是什么。
 *
 * @param {string} text 正文
 * @param {any[]} defs
 * @returns {{text:string, unwrapped:string[]}} unwrapped 是被拆开的那几个折叠的标题
 */
export function unwrapAchievementToggles(text, defs) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // 顶层才管。缩进的折叠是挂在某条成就下面的分组标签,那是规则一要的,不能碰
    ...
    /**
     * `<summary>` 拆标签,**按标签剥、不按整行匹配**:模型两种都写过,一行写完的和
     * 开合标签各占一行的。整行正则只认前者,遇到后者会把裸的 `<summary>` 留在正文里。
     */
    let label = null;
    const body = [];
    let inSummary = false;
    ...
    // 外壳没了,里面统一提一级 —— 留着缩进会让 parseTodos 把它们当成子步骤挂到别人名下
 ...
 *
 * 和 `joinBodies` 的分工:那个只合掉**接缝上**同名的标题,搬不动条目 —— 它跑在
 * 各段刚写完的时候,那时还没有任何人给出过「这条该去哪一节」。这个反过来:正文已经
 * 写完了,分类那一趟看着全文给映射,于是可以真的把条目搬到该去的小节。
 *
...
export function regroupByAssignment(body, { defs, assignment, sections = [] } = {}) {
  ...
  /** 原文里已经见过成就没有 —— 决定纯说明小节排在列表前面还是后面,见下面 head/tail */
  let sawEntry = false;
  let cur = null;
  for (let i = 0; i < lines.length; i++) {
  ...
  /**
   * **分类名单里没提到的小节,留在成就列表的原来那一侧。**
   *
   * 分类那一趟只列装成就的小节,纯说明小节(规则 3.5 的「机制速查」)它一个字都不会提。
   * 而没被提到的一律接在后面 —— 于是一段本该在列表**前面**的速查会被搬到全篇末尾,
   * 吊在最后一条成就下面。**那是给人在读列表之前看的东西,挪到末尾等于没写。**
   *
   * 判据取自原文,不用猜:标题出现时还一条成就都没见过 ⇒ 它在列表前面,还排前面。
   * 这样两个方向都对 —— 收尾的「备注」原来在后面,重排完还在后面。
   */
 ...
 * 对每条成就的理解,而写正文之前那一趟(只有成就名)根本拿不到这个信息。
 */
...
/**
 * 分类那一趟的提示词。**跑在正文写完之后**,这是它和「写之前只看名字分一趟」的全部区别:
 *
 ...
 * 让它失手),编号不会。
 */
export function buildRegroupPrompt(game, defs, current, clusters = []) {
...
export async function generateGuide(db, {
  ...
  /**
   * 他已经解锁的那几条只写一行 —— 见 `briefApiNames`。
   *
   * **覆盖重写一条都不省。** 那时候攻略里已经有花过钱写出来的正文,而"他后来把这条
   * 解锁了"不是把那段字删掉的理由 —— 那是他付过账的东西,而且删掉之后没有任何地方
   * 找得回来。所以只有新写的那一篇会略写。
   */
  const brief = overwrite ? new Set() : briefApiNames(defs, unlocked);
  /**
   * **问某一段该写什么,只有这一个出口。** 两个调用点(第一轮的分片路径、后面几轮
   * 补写没写出来的那段)各写一次 `buildChunkMessage(..., brief)` 的话,漏传的那一处
   * 不会报错、不会少内容 —— 只是那一段的略写名单静静地没了,而它恰好是最难被测到的
   * 那条路(要先有一段整段失败才走得到)。绑一次,两边都拿不到漏传的机会
   */
  const chunkMessage = (cs, at) => buildChunkMessage(cs, at, brief);
  let bodies = [];
  ...
  /**
   * **各段自己开小节标题,写的时候不给它们一份统一名单**(分类是全篇写完之后那一趟
   * 的事,见下面的 `phase: 'regroup'`)。
   *
   * 写之前定名单的话,那一趟手上**只有成就名**,而名字是梗的游戏就此失手:
   * 《马特的寻猫游戏》的「海拉鲁老流氓」其实是打碎 100 个罐子、「半条命4」是用撬棍
   * 撬容器,只看名字分出来的是「自然与美食」「囤积狂的自我修养」;补上描述再跑一次,
   * 依然丢掉了「难度模式」这种真正要紧的结构。**信息不够,不是提示词不够。**
   *
   * 所以接缝上的重复交给 `joinBodies` 合掉,真正的归类留到最后一趟 —— 那时名字、
   * 描述、各段自己的判断、以及联网查过的正文全都在手上。
   */
   ...
   * ---- 把藏进折叠里的成就掏出来 -------------------------------------------
   *
   * **在重排之前做,不是之后。** `regroupByAssignment` 的第三条断言比对的是折叠块的
   * 内容,掏空一个折叠会当场把它判成"折叠被拆了"而整趟回滚 —— 那条断言是对的,
   * 它防的是重排自己拆折叠。所以顺序上先拆完,重排看到的就是已经摊平的正文。
   *
   * 不分段的攻略也会犯这个毛病,所以这一步**不看 `chunks.length`**。
   *
   * **改完要重新过一遍闸门,改不动就退回去。** 草稿在这一步之前已经打过勾、过了校验,
   * 是一份能落地的成品;这一趟只是让它更好看,没有资格把它弄成不合格的。重排那一趟
   * 是同一条规矩 —— 区别只在于重排在后面,靠它兜不住这里(它回滚回的是拆完之后那一份)。
   */
  if (!blocking.length && !chunkFailures.length) {
    ...
    if (unwrapped.length) {
      ...
      if (recheck.blocking.length) {
        ...
        onProgress({ phase: 'unwrap-failed', reason: `拆折叠之后校验没过(${recheck.blocking.length} 条阻断)` });
      } else {
        lint = after;
   ...
   * 各段并发写的时候才发生的 —— 写正文之前那一趟在结构上就看不见它,这一趟看得见。
   *
   * **只在真的分了段时跑。** 一段写完的没有跨段口径问题,而那一趟模型手上本来就有
   * 描述和稀有度,分得比任何事后一趟都准。
   *
   * **失败一律降级成「不重排」,不中断。** 正文已经写好、打过勾、过了闸门,为一次
   * 锦上添花的归类把它丢掉不划算 —— 和别的锦上添花的趟次同一条规矩。降级要出声。
   */
  if (!blocking.length && !chunkFailures.length && chunks.length > 1) {
    ...
    try {
       ...
       * **同类成就簇：先告诉模型，再自己兜底。**
       *
       * 告诉它是因为它选得比 plurality 准 —— 它看得见正文，知道这一簇更该跟哪个
       * 小节；兜底是因为封闭名单拦不住「名单里有两个都说得通的去处时二选一」 ——
       * 《马特的寻猫游戏》四条替换吉祥物被劈成「宝石与商店」和「吉祥物替换」两处，
       * 就是这么发生的。提示词纠不过一个说得通的编辑判断，得有个程序判据。
       */
      const clusters = sameKindClusters(defs);

      const grouper = createSession(provider, { system: REGROUP_SYSTEM });
      const reply = await grouper.ask(buildRegroupPrompt(game, defs, current, clusters));
      const verdict = checkResult(reply);
