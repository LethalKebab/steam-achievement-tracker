...
import { createSession, buildWebTools, checkResult } from './ai.js';
...
先用 web_search 找这个游戏的成就攻略,再用 web_fetch 把最有用的那一两页正文抓回来读完。中文攻略站(游民星空、3DM、NGA、B站)、Steam 社区攻略、TrueAchievements、Fandom wiki 都可以。**理解机制之后用自己的话重写**,不要照搬原文。
...
export async function generateGuide(db, {
  ...
  const session = createSession(provider, { system, tools: buildWebTools(config.ai) });
    ...
    const reply = await session.ask(message, {
      onEvent: (ev) => {
        if (ev.type === 'content_block_start' && ev.content_block?.type === 'server_tool_use') {
          onProgress({ phase: 'tool', round, name: ev.content_block.name });
        }
      },
    });
