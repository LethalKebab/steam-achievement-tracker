 ...
 * 这次失败能不能「原样再问一次」。
 *
 * **只有 empty。** 别的每一种都有具体原因:截断是段太长(该切小)、拒答和
 * RECITATION 是内容判定(再问一遍是同一个结果)、搜索报错是网络或额度
 * (要么等要么改配置)。把它们塞进重试,只是花钱把同一个错误再犯一次。
 */
function canRetry(err, attempt) {
  return err?.code === 'empty' && attempt < EMPTY_RETRIES;
}
 ...
 */
const SPLITTABLE = new Set(['max_tokens', 'empty']);
 ...
 */
const CHUNK_LOCAL = new Set(['empty', 'max_tokens', 'refusal', 'recitation', 'tool-error', 'other']);
...
export async function generateGuide(db, {
  ...
  for (round = 1; round <= rounds; round++) {
    ...
    // **第一轮的段数会在循环里变** —— 见下面切小的处理。之后几轮是按校验结果
    // 定点重写,`targets` 是那时算好的下标,不能中途变
    ...
    for (let i = 0, k = 0; order ? k < order.length : i < chunks.length; ) {
      ...
      // 一段自己的重试阶梯:原样再问 → 切成两半 → 记下来放过。**三级都在这个内层循环里**,
      // 而不是靠外层 `continue` 绕回来 —— 那样"这一段问了第几次"就得挂在外面,
      // 而切小之后那个计数该归零,两件事缠在一个变量上迟早错
      let attempt = 0;
      for (;;) {
        try {
          // **这一段还什么都没有 ⇒ 用原来那句"写这一段",不管第几轮。**
          // 拿 buildChunkFeedback 去问一段从没写出来的内容,等于甩过去五十条
          // "缺 checkbox" —— 而模型缺的不是修正意见,是这一段本身
          bodies[i] = bodies[i]
          ...
          if (canRetry(err, attempt)) {
            attempt++;
            onProgress({
              phase: 'retry', round, chunk: i + 1, chunks: chunks.length,
          ...
          if (canSplit(err, round, chunks[i])) {
            ...
            onProgress({
              ...
              from: a.length + b.length, to: a.length, reason: err.code,
            });
          ...
          //
          // 记的下标是**当下**的下标,而它不会再被后面的 splice 挪动:切分只发生在
          // 当前下标 i 上(splice(i, 1, a, b) 只影响 i 及其后),而循环只往前走,
          // 所以已经记下来的都在 i 之前
          const failed = chunkFloorAdvice(err, round, chunks[i]);
          chunkFailures.push({
            chunk: i + 1,
            of: chunks.length,
            count: chunks[i].length,
            first: defName(chunks[i][0]),
            last: defName(chunks[i][chunks[i].length - 1]),
            code: failed.code ?? null,
            reason: failed.message,
          });
          onProgress({
            phase: 'chunk-failed', round, chunk: i + 1, chunks: chunks.length,
            count: chunks[i].length, reason: failed.message,
          });
          break;
        }
      }
      // 拿到了就立刻落盘。**放在 try 外面** —— 写盘失败(磁盘满、权限)是我们这边的
      // 故障,不是"这一段模型没写出来",不该被上面那套重试阶梯当成后者处理
