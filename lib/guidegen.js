...
- **分组的标签单独占一行,不要在每一条前面重复。** 十四条各写一遍「前置:」「步骤:」,
  ...
  **标签行必须也是 \`- [ ]\`,不能写成普通 bullet(\`- 前置:\`)。** 这是硬性的:
  ...
  分组只在**条目多到要分**的时候用;五六条以内直接平铺,不用套一层
...
export async function generateGuide(db, {
  ...
  let sections = [];
  if (chunks.length > 1) {
    onProgress({ phase: 'sections', chunks: chunks.length });
    try {
      // 单独一条会话,**不带联网工具**:这一趟不查资料,而且不该污染任何一段的上下文
      const planner = createSession(provider, { system: SECTION_PLAN_SYSTEM });
      const reply = await planner.ask(buildSectionPlanPrompt(game, defs));
      const verdict = checkResult(reply);
      if (!verdict.ok) throw new Error(verdict.reason);
      sections = parseSectionPlan(reply.text);
      if (!sections.length) throw new Error('回复里挑不出成形的分区表');
      onProgress({ phase: 'sections-done', count: sections.length, names: sections });
    } catch (err) {
      sections = [];
      onProgress({ phase: 'sections-failed', reason: String(err?.message ?? err) });
    }
  }
