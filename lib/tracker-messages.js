...
export const TRACKER_MESSAGES = {
  ...
  'phase.libraryFamily':  ['检查家庭库游戏', 'Checking family-shared games'],
  ...
  'sync.familyCleared':   [',清除家庭标记 {n} 款', ', {n} family flags cleared'],
  'sync.added':           ['     新增:{names}', '     Added: {names}'],
  'sync.familyAdded':     [',家庭库新发现 {n} 款', ', {n} picked up from the family library'],
  'sync.familySkipped':   [',家庭库这次没查成', ', family-library check did not run'],
  'sync.familyNames':     ['     家庭库新增:{names}', '     From the family library: {names}'],
  'sync.forbidden':       ['  {n} 款游戏 Steam 不再提供你的进度,请切换成手动更新:{names}', '  Steam no longer reports your progress for {n} game(s) — switch them to manual: {names}'],
  'family.how':           ['登录 Steam 后打开 https://store.steampowered.com/pointssummary/ajaxgetasyncconfig,取 data.webapi_token', 'Open https://store.steampowered.com/pointssummary/ajaxgetasyncconfig while signed in to Steam, and copy data.webapi_token'],
  'family.prompt':        ['粘贴 access_token(不回显): ', 'Paste the access_token (input is hidden): '],
  'family.noToken':       ['❌ 没有输入 token', '❌ No token was entered'],
  'family.failedLine':    ['❌ 家庭库导入失败:{reason}', '❌ Family-library import failed: {reason}'],
  'family.scanned':       ['家庭组 {members} 人,共享库 {n} 款(其中 {excluded} 款无法共享)', 'Family group of {members}, {n} apps in the shared library ({excluded} of them not shareable)'],
  'family.addedN':        ['  新增 {n} 款', '  {n} added'],
  'family.filledN':       ['  补上游玩记录 {n} 款', '  {n} rows given their play history'],
  'family.nothing':       ['  没有需要导入的 —— 你玩过的家庭组游戏都已经在追踪里了', '  Nothing to import — every family game you have played is already tracked'],
  ...
  // ---- guide-icons ------------------------------------------------------------
  'gi.noToken':           ['Notion:没配 token(config.json 的 notion.token)', 'Notion: no token configured (notion.token in config.json)'],
  'gi.dryRun':            ['预演模式:只看会换哪些,不写\n', 'Dry run: it shows what would be replaced and writes nothing\n'],
  'gi.summary':           ['{pages} 个 Notion 攻略页:{replaced} 个换成了更清晰的图标', '{pages} Notion guide pages: {replaced} given a sharper icon'],
  'gi.nothing':           ['  (没有要换的。自己设过的图标一律不动)', '  (nothing to replace — an icon you chose yourself is never touched)'],
  'gi.rerun':             ['\n确认没问题就去掉 --dry-run 再跑一次。', '\nIf that looks right, drop --dry-run and run it again.'],
  'gi.res.replaced':      ['换成 460×215 商店头图', 'replaced with the 460×215 store header'],
  'gi.res.wouldReplace':  ['会换成 460×215 商店头图', 'would be replaced with the 460×215 store header'],
  'gi.res.noSource':      ['Steam 没有可用的商店图,保持原样', 'Steam has no usable store art — left as it is'],
  'gi.res.noBetter':      ['只有 32×32 那一张,保持原样', 'the 32×32 is the only art there is — left as it is'],
  'gi.res.unreadable':    ['页面读不到(可能没授权给 integration)', 'the page cannot be read (the integration may not have access)'],
  'gi.res.badUrl':        ['数据库里的地址不是 Notion 页面', 'the address in the database is not a Notion page'],
  'gi.res.writeFailed':   ['写入失败', 'the write failed'],
  ...
  'in.hasDbWarn':         ['   新建一个会把配置改指到新库 —— 现有攻略一篇都不会丢失,但工具会看不到它们。', '   Creating another repoints the configuration at it. No existing guide is lost, but this tool stops seeing them.'],
  ...
  'nc.noStatus2':         ['   guide-status 那套(解锁全部成就→Done、掉出 100%→Staged)没东西可写。', '   that guide-status (100% → Done, dropping below → Staged) has nothing to write to.'],
  ...
  'gg.noResearchAlt':     ['  想要经过调研的攻略,换一家有联网的:--provider anthropic 或 --provider deepseek。', '  For a researched guide, switch to a provider that has web access: --provider anthropic or --provider deepseek.'],
  ...
  'gg.partialRewrite':    ['  未通过校验,第 {round} 轮只重写其中 {chunks}/{of} 段', '  The checks did not pass; round {round} rewrites only {chunks} of {of} shards'],
  ...
  'gg.notResearched':     ['    这一份未联网,内容是模型凭已有知识写的。', '    This one was not researched online; the content is what the model already knew.'],
  ...
  'gp.noResearch':        ['{name} 没有服务端联网搜索,重写出来的内容是模型**凭已有知识写的**,不是查来的。\n  真要这么跑,加 --no-research 明说;想要经过调研的,换一家有联网的(--provider anthropic 或 --provider deepseek)。', '{name} has no server-side web search, so the rewritten content would come **from what the model already knows** rather than from research.\n  To run it anyway, add --no-research to say so; for researched content, switch to a provider with web access (--provider anthropic or --provider deepseek).'],
  ...
  'gp.notResearched':     ['    这几条未联网,内容是模型凭已有知识写的。', '    These were not researched online; the content is what the model already knew.'],
...
Steam 成就追踪器(本地版)—— 零依赖,不需要 Google 账号
  ...
  node tracker.js guide-status            攻略页状态对齐完成度(解锁全部成就→Done,掉出100%→Staged)
              guide-status --dry-run      只算不写,先看会改哪些
  node tracker.js audit [appid]           反查有没有勾上了但其实没解锁的 checkbox(只读)
  node tracker.js guide-icons             把还在用 32×32 小图标的攻略页换成商店头图
              guide-icons --dry-run       只看会换哪些,不写
...
Steam achievement tracker (local) — zero dependencies, no Google account
  ...
  node tracker.js guide-icons             swap guide pages still on the 32x32 icon to the store header
              guide-icons --dry-run       show what would be replaced, write nothing
