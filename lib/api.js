...
export function createApi({
  ...
  const api = {
    ...
    async addGame(appid, name, family = false) {
      ...
      // **不要一律设 `status:'Manual', syncLocked:1`。** 「手动加的多半是 Steam 查不到
      // 数据的」这个理由对家庭共享的游戏正好是反的:家庭标记的含义就是**你自己在玩、
      // Steam 照常返回真实进度**,锁住等于让它永远停在加进来那一刻的数字上。
      // 默认保持自动同步;真的拿不到数据,行上那把锁一点就停。
     ...
     * 在系统文件管理器里定位这份**本地**攻略。
     *
     * 存在的理由是本地攻略经 `/guide/` 出来的是**纯文本 markdown 源码**(那是防 XSS
     * 的有意选择,见 server.js),要好好读还得用编辑器打开文件 —— 而"文件在哪"
     * 正是生成完那一刻最容易丢的信息。
     *
     * **路径不接受调用方给。** 只收 appid,自己去 guides 表查,再过一道
     * `resolveGuidePath`(不许越出 guidesDir)。这是本项目唯一一处拉起外部进程的地方,
     * 让页面传路径进来等于把那个进程的参数交给页面
     */
     ...
     *
     * 这条是给 launcher 用的,不是给 Dashboard 的:收进托盘后进程能连着跑几天,而
     * `startupJobs` 里那次 `maybeAutoSync` 只在服务器启动时响一次,所以「开一次 app =
     * 查一次是否过期」在后台常驻下就不成立了。launcher 在窗口显示时打这里补上触发点。
     *
     ...
     *
     * **「先验证再写盘」那条顺序不能简化掉** —— 一个非空但打不通的 key 会让 `/` 的
     * 重定向判断放行,用户就卡在一个看起来配好了、实际永远同步不了的 Dashboard 上。
     */
    ...
    getSettings: () => ({
      ...
      // **每家各自的状态。** 一家一套之后,「换一家」不该再要求重打一遍 key,而页面
      // 只有知道这个才画得出来:哪几家配好了、各自 pin 的是哪个模型。
      // **key 仍然只回答"配没配"**,绝不回传原文;model 不是密钥,原样给。
     ...
     * 顺手提交一下就会把 key 抹掉。要停用就去 config.json 清空那一家的槽位。
     *
     * **"已经存着的那个"是按家算的。** 这一行原来写作
     * `provider === config.ai?.provider ? config.ai?.apiKey : ''` —— 它已经识破了
     * "别沿用上一家的 key"这件事,但当时没有地方存第二把,所以换一家就只能重打一遍。
     * 现在 `ai.keys` 一家一个槽位,同一条规则由 `resolveAiKey` 统一执行,换家不用重打。
     *
    ...
    async saveAiConfig(provider, apiKey, model) {
      ...
      // 按**归一后**的名字存:google 和 gemini 是一家,deepseek 的两个端点共用一套。
      // 按原样存会让同一家出现两套,而"我明明填过了"是最难查的那种状态
      ...
      config.ai = ai;
      // **只写这一家那一套。** `saveConfig` 的 merge 是递归的,所以传一个单键的
      // `providers` 会并进去而不是替换掉 —— 别家的原样留着。整份写回去就等于
      // 「保存 Gemini 顺手把 Anthropic 抹了」,而那种丢失不会报错
    ...
    // effort 不在这里校验取值:各家的档位名不一样,写死白名单会在供应商加档位的
    // 那天把合法值拒掉。填错就是供应商回 400,而那条路上有专门的提示
    // `scope` 非空 = 局部重写(`{selector, note}`)。**服务端不替它补 overwrite** ——
    // 局部重写在 planPatch 里本来就要求攻略已存在,而那条检查比这里的布尔更严
    ...
    /**
     * 局部重写的预检:每个预设范围各会改几条、其余几个框不动。
     *
     * 和上面那个是**两次不同的问题**,所以是两个方法而不是一个带参数的:整篇重写
     * 问「你会失去什么」,局部重写问「什么会留下」。共用的是计算,不是措辞 ——
     * 这条规矩在 `inspectGuideDb` 和 `previewGuideRewrite` 上各得出过一次
     */
    ...
    /**
     * 攻略备份:`guides/` 底下那三个归档目录的列表、预览、恢复、删除。
     *
     * **列表不带正文。** 一份 Notion 备份是一百多 KB 的 block dump,列表里要的只是
     * "哪个游戏、什么时候、多大";正文等点开「看」再单独要一次。
     *
     * 传 appid = 只要这个游戏的,那是 Dashboard 上 ⋯ 菜单的用法;不传 = 全量,
     * 设置页拿它算总占地和挑孤儿。
     */
    ...
    /**
     * 恢复。**同步返回,不走后台任务** —— 和搬去 Notion 同一个理由:没有模型调用,
     * 最慢的一种(整页删完重写)也就是几十次 Notion 请求。
     */
    ...
    /**
     * 设置页「全部删除」。**收的是编号表,不是"把目录清了"** —— 理由写在
     * `deleteArchives` 头上:页面画完到你点下去之间,后台重写能再存进一份。
     *
     * 外层还得包一层 try:`ids` 不是可迭代的东西时 `for...of` 就地抛,
     * 那一抛在循环内部的 try 外面。
     */
     ...
     * ——token 不对要回开发者页面重新复制,数据库访问不了是没在页面上加
    ...
    async saveNotionConfig(token, dbId) {
      ...
      if (dbId) {
        ...
        if (unreadable) {
          // **两条都要说出来。** 「填的是页面 ID 而不是数据库 ID」和「忘了加 Connections」
          // 在 Notion 那边报的一模一样,只提其中一个会把填错 ID 的人赶去反复检查权限。
          return {
            error:
     ...
     * 备份写到盘上,**返回路径,不返回文件本身**。
     *
     * 界面上不做「点一下下载」:`launcher/main.js` 没有挂 `will-download`,
     * Electron 会退回它自己的原生保存对话框 —— 而原生对话框在这个程序里是死的
     * (见 docs/self-update.md,`showMessageBox` 秒返回 420)。一个点了没反应的
     * 下载按钮,比一条能复制的路径糟糕得多。配一个 revealBackup 就够了。
     */
    ...
    /** 在文件管理器里定位刚做好的备份。**路径必须落在 backups/ 里** —— 这个方法
     *  会被 /api/ 直接调到,参数是外面给的,和 revealGuideFolder 同一条纪律 */
    revealBackup(path) {
      const dir = join(DATA_ROOT, 'backups');
    ...
    /**
     * 恢复。写完 config.json 之后**必须把这个进程手里的 config/steam 也改掉** ——
     * 和 completeSetup 同一个坑:两个对象是启动时建好的,SteamClient 更是把
     * key/steamId 复制成了自己的实例字段(见 lib/steam.js)。不当场改,
     * 用户就得到一个"恢复成功"但所有 Steam 请求都用着旧凭据(或者没凭据)的程序。
     */
