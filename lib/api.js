...
export function createApi({
  ...
  const api = {
    ...
    startGuideGen: (appid) => startGuideGen(String(appid)),
    ...
    async migrateGuideToNotion(appid) {
      try {
        const r = await migrate(db, { notion: new NotionClient(config), config, appid });
    ...
    async saveNotionConfig(token, dbId) {
      ...
      if (dbId) {
        try {
          ...
          return {
            error:
              `token 没问题,但这个数据库访问不了:${err.message}。` +
              `在 Notion 里打开该数据库(或它的父页面)→ 右上角 ••• → Connections → 加上这个 integration。`,
          };
        }
      }
