...
import { saveConfig, loadConfig, CONFIG_PATH, DATA_ROOT } from './config.js';
...
export function createApi({
  ...
  const api = {
     ...
     *
     * 第三个参数曾经是 csvFolder(表格导入),2026-08-19 随导入一起删掉。删的时候
     * 注意**别把上面那条「先验证再写盘」的顺序也一起简化掉** —— 它和导入无关,
     * 挡的是另一件事:一个非空但打不通的 key 会让 `/` 的重定向判断放行。
     */
