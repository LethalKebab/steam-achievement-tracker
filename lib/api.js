...
import { runFamilyImport } from './family.js';
import {
...
/**
 * Seconds → hours, one decimal. null stays null — an absent figure must not render as 0.0h, which
 * reads as "instant" rather than as "unknown"
 */
const hours = (sec) => (typeof sec === 'number' && sec > 0 ? Math.round(sec / 360) / 10 : null);

/**
 * The two numbers the 「预计还需」 column shows, and nothing else.
 *
 * **`remaining` is hours still to go, not the game's length.** The naive apportionment of a
 * completionist figure is `(1 − rate) × hours`, which assumes every achievement costs the same;
 * what is left in a part-finished game is the tail nobody else finished either, so that spelling
 * understates every unfinished game — measured, all 60 of them, by a median of 1.2×. The share
 * comes from `remainingDifficulty`, and falls back to `1 − rate` only when the game has no rarity
 * on record: with nothing to weight by there is no correction to make, and a rough number beats
 * none.
 *
 * **`gain` is what finishing it does to the average completion rate**, in percentage points, and
 * the two cases are not the same formula. An eligible game already sits in that mean, so
 * completing it moves the mean by `(1 − rate) / N`. A game with nothing unlocked is **not in the
 * mean at all** — Steam's method counts only games with at least one achievement — so finishing
 * it both adds a term and grows the divisor, giving `(1 − avg) / (N + 1)`. Using the first
 * formula for both overstates an untouched game by about four times.
 */
function costOf(row, h, costs) {
  const total = hours(h?.comp_100_med ?? h?.comp_100);
  if (total === null) return { remaining: null, gain: null, lo: null, hi: null, sample: 0 };
  const rate = typeof row.rate === 'number' ? row.rate : 0;
  const w = costs?.difficulty.get(row.appid);
  const share = w && w.all_weight > 0 ? w.left_weight / w.all_weight : 1 - rate;
  const round = (n) => Math.round(n * 10) / 10;

  const { eligibleCount: n, avg } = costs.agcr;
  const eligible = row.status !== 'Unvetted' && row.total > 0 && row.achieved > 0;
  const gain = row.status === 'Unvetted' || !row.total
    ? null
    : eligible
      ? ((1 - rate) / n) * 100
      : ((1 - avg) / (n + 1)) * 100;
  ...
  return {
    remaining: round(share * total),
    gain: gain === null ? null : Math.round(gain * 1000) / 1000,
    // The spread scaled by the same share, so it describes the same quantity as `remaining`
    lo: hours(h?.comp_100_lo) === null ? null : round(share * hours(h.comp_100_lo)),
    hi: hours(h?.comp_100_hi) === null ? null : round(share * hours(h.comp_100_hi)),
    // Single digits mean the figure is one stranger's playthrough. The table dims it rather than
    // ranking on it as though it were a measurement
    sample: h?.comp_100_count ?? 0,
  };
}

/** One row of the games table → the game object the Dashboard frontend expects */
function toDashboardGame(row, guideUrls, guideKinds, lang, costs) {
  const { shown, alt } = gameNamePair(row, lang);
  const h = costs?.hltb.get(row.appid);
  return {
    appid: row.appid || '',
    ...
    // Hours still to go and what finishing does to the average completion rate. Both null when
    // nothing is known — see costOf, which carries the reasoning for each
    cost: costOf(row, h, costs),
    ...
    newAchDaysAgo: row.new_ach_date
      ...
      : null,
    // The notification events. **The moment the transition happened**, stamped by
    ...
    newAchAt: row.new_ach_date ?? null,
    // last_played is in seconds, like Steam's rtime_last_played, but it has two sources. An owned
    // row takes that field straight from GetOwnedGames. A shared row has no such field anywhere in
    // the Web API, so it is **derived** — from its playtime moving between two syncs, or written
    // once by the family-library import. A delisted row has neither and stays null
    playedDaysAgo:
...
/**
 * The achievements a patch added after a game's original set, as api_name → the moment each one
 * appeared.
 *
 * **"After the original set", not "recently seen".** A game's first schema sync writes every row
 * with one timestamp, so the earliest stamp is the set it shipped with and anything later arrived
 * with an update. A row that predates the first_seen column is NULL, which counts as earlier than
 * any stamp: its moment was never recorded, and an addition written since is still later than it.
 * A game with no stamps at all has nothing to report.
 */
function addedAfterOriginalSet(rows) {
  const stamps = rows.map((r) => r.first_seen).filter(Boolean);
  if (!stamps.length) return new Map();
  const earliest = rows.some((r) => !r.first_seen) ? '' : stamps.reduce((a, b) => (b < a ? b : a));
  return new Map(rows.filter((r) => r.first_seen && r.first_seen > earliest).map((r) => [r.api_name, r.first_seen]));
}
...
export function createApi({
  ...
  const api = {
    getDashboardData() {
      ...
      // Both read whole tables once rather than per game — 300-odd rows either way, but per game
      // it would be 600 statements on every page open
      const agcr = computeAgcrStats(db);
      const costs = {
        hltb: new Map(allHltb(db).map((r) => [r.appid, r])),
        difficulty: new Map(remainingDifficulty(db).map((r) => [r.appid, r])),
        // The gain half of the column needs both the divisor and the current mean, and they are
        // already computed a few lines down for the readout — same call, one set of numbers
        agcr,
      };
      const games = allGames(db).map((r) => toDashboardGame(r, guideUrls, guideKinds, lang, costs));
    ...
    async addGame(appid, name, family = false) {
      ...
      const result = {
        ...
        perfectLostDaysAgo: null, achAddedDaysAgo: null, perfectLostAt: null, achAddedAt: null, newAchAt: null,
      };
    ...
    /**
     * The one-time family-library import.
     *
     * **What the sync cannot do.** `syncLibrary` sees a shared game through
     * `GetRecentlyPlayedGames`, whose window is a fixed fortnight — so it recovers what you are
     * playing now and nothing older. Measured on a live account, that is 2 games out of 18 ever
     * played. This is the other 16, and it is a one-off because the family interface takes a
     * browser token good for about a day (see `lib/family.js`).
     *
     * **The token is used and dropped.** It is never written to `config.json`, so this method
     * takes it per call and the page asks for it every time. A day-old secret in the backup zip
     * would be worse than no feature.
     *
     * Two kinds of write, and the second is the one worth reading twice:
     * - rows that are **not here yet** are inserted, flagged `family`, with the localised name
     *   fetched the same way both other insert paths fetch it;
     * - rows that **are** here get `playtime_forever` and `last_played` filled in. Nothing else is
     *   touched — not the name, not ♥/★, not the lock, not the counts. Those two columns are the
     *   ones no other code path can fill for a shared row, and they are what the 🎮 badge and the
     *   recently-played sort read. Without this, such a row needs **two** further play sessions
     *   before it can carry a date: the first only records a baseline for `recordUnownedPlaytime`
     *   to compare against.
     */
    importFamilyLibrary(token) {
      return runFamilyImport({ db, steam, config, token });
    },
    ...
    async getMissingAchievements(appid) {
      ...
      const added = addedAfterOriginalSet(cached);
      let defs;
      if (cached.length > 0) {
        const lang = normalizeUiLanguage(config.uiLanguage);
        defs = cached.map((a) => ({
      ...
      return {
        ...
        missing: missing.map((a) => {
          ...
          return {
            ...
            icon: a.icon,
            // How many days ago a patch added it, or null for the set the game shipped with. The page
            // applies its own notification window, so that number has one copy
            addedDaysAgo: added.has(a.name)
              ? Math.floor((Date.now() - Date.parse(added.get(a.name))) / DAY_MS)
              : null,
            guide: g
              ? {
     ...
     * across every vendor (`lib/ai-anthropic.js` and `ai-deepseek.js` read `ai.requestTimeoutMs`
