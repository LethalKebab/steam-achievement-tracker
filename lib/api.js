...
function toDashboardGame(row, guideUrls, guideKinds) {
  return {
    appid: row.appid || '',
    name: row.name || '(未命名)',
    // The English store title, '' when there is none on record. **Search only** — the row still
    // displays `name`. Both of the Dashboard's search tests match either, which is what lets an
    // English term find a game stored under its Chinese title
    nameEn: row.name_en || '',
...
export function createApi({
  ...
  const api = {
    ...
    async addGame(appid, name, family = false) {
      ...
      const result = {
        appid, name: resolved, nameEn, achieved: null, total: null, rate: null,
