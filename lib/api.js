...
function toDashboardGame(row, guideUrls, guideKinds, lang, costs) {
  ...
  const wall = costs?.walls.get(row.appid);
  return {
    appid: row.appid || '',
    ...
    /**
     * What finishing this game would cost. Absent for a game nobody has reported a completionist
     * time for, and for one HowLongToBeat has never heard of — both are `null`, and the table
     * renders that as unknown rather than as free.
     *
     * **`mid` is the median, not the site's headline figure.** The headline is neither the mean nor
     * the median but a trimmed statistic of its own, and the median is the one that survives a
     * handful of people who left the game running for a week.
     *
     * `lo`/`hi` are the site's own rushed and leisurely ends. They are sent because a point estimate
     * lands within ±30% of the truth about seven times in ten, and a reader deciding how to spend
     * a weekend is entitled to see that rather than a number pretending to be exact.
     */
    cost: {
      mid: hours(h?.comp_100_med ?? h?.comp_100),
      lo: hours(h?.comp_100_lo),
      hi: hours(h?.comp_100_hi),
      // Single digits mean the figure is one stranger's playthrough. The table says so instead of
      // ranking on it as though it were a measurement
      sample: h?.comp_100_count ?? 0,
      // The global unlock rate of the rarest achievement still locked here, 0-100. This is the
      // number an hour figure cannot contain: a game can be short and still hold one achievement
      // that almost nobody has
      wall: typeof wall === 'number' ? wall : null,
    },
  ...
  const api = {
    getDashboardData() {
      ...
      const costs = {
        ...
        walls: new Map(hardestRemaining(db).map((r) => [r.appid, r.min_rarity])),
      };
