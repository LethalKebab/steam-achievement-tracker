  ...
  const api = {
    getDashboardData() {
      ...
      return {
        ...
        // **原始比值,给顶栏那条完成度线用。** 前面两个是给人读的字符串,页面要画一条
        // 宽度等于完成度的线就得有个数。让前端去 parseFloat('77.123%') 也能跑,但那样
        // 这条线的正确性就挂在一个显示格式上 —— 哪天给它加个千分位或者空格,线会**静默**
        // 画错而不是报错。数值和它的格式化结果各走各的,是这里唯一不会烂掉的形状
        avg: agcr.avg,
