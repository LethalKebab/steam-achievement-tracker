...
export function lintGuide({ todos, defs, text = null, unlockedApiNames = null, kind = 'notion' }) {
  ...
  for (const [name, apiNames] of byName) {
    ...
    for (const apiName of apiNames) {
      ...
      const quotes = (raw) => Boolean(raw) && hits.some((t) => flat(t.text).includes(flat(raw)));
      ...
      // Steam entry with only an English one is still fixable by quoting that
      if (d.description || d.description_en) {
  ...
  for (const d of defs) {
    if (!d.description && !d.description_en) continue;
    ...
    // one nobody reads
    const quoted = (raw) => raw && hits.some((t) => flat(t.text).includes(flat(raw)));
  ...
  if (text !== null) {
    ...
    if (DATA_SOURCE_RE.test(text)) {
      add('warn', 'data-source-note', '攻略里写了勾选状态的数据来源,SKILL.md rule-7要求去掉');
    }
  }
