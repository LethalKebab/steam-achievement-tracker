...
import { headingOf, normSectionTitle, sectionTree } from './markdown.js';
...
export function lintGuide({ todos, defs, text = null, unlockedApiNames = null, kind = 'notion' }) {
  ...
  if (text !== null) {
    ...
    /**
     * **The heading structure, which a guide written in several concurrent passes gets wrong
     * without anything erroring.** Measured on 月圆之夜: shard 1 opened its sections with `##`,
     * shard 2 with `###`, shard 4 with `##` again — so twenty top-level topics rendered as
     * subsections of whichever `##` happened to precede them. Every achievement was present and
     * every checkbox worked.
     *
     * Two shapes, both decidable from the tree and neither a matter of taste:
     *
     * - **A level skipped.** `##` straight to `####`, or a `###` opening the document: the level in
     *   between has no heading, so the deeper one hangs off nothing.
     * - **One section's achievements at two depths.** A section that lists achievements of its own
     *   *and* has subsections listing more. A section deliberately divided into subsections holds
     *   none of its own, which is why this can tell the two apart.
     */
    const walkDepth = (nodes, parentLevel) => {
      for (const n of nodes) {
        if (n.level > parentLevel + 1) {
          add('warn', 'heading-level-jump', msg('lint.headingLevelJump', { text: n.title.slice(0, 50) }));
        }
        const deeperEntries = (kids) => kids.some((k) => k.entries > 0 || deeperEntries(k.children));
        if (n.entries > 0 && deeperEntries(n.children)) {
          add('warn', 'mixed-section-depth', msg('lint.mixedSectionDepth', { text: n.title.slice(0, 50) }));
        }
        walkDepth(n.children, n.level);
      }
    };
    walkDepth(sectionTree(text).roots, 1);
...
/**
 * Achievements mechanical ticking can **never reach**: the name a guide in `lang` would write is
 * not unique within this game.
 *
 ...
 *
 * **The language is not optional, because a guide is written in one of them.** The question is not
 * "does this achievement have a unique name somewhere", it is "is the name that will be **on the
 * page** unique" — and the other language's name is not on the page, so its uniqueness cannot help
 * ticking. The two readings differ on the most common collision there is: a Steam localisation
 * duplication in one language only. 文明VI ships 亦敌亦友 twice, under `Frenemy` and `Frenemies`;
 * a Chinese guide carries 亦敌亦友 on both entries and can tick neither, while an English one ticks
 * both. Measured 18 such achievements across 9 games of a 318-game library.
 *
 * The consequence of answering "it has a unique name somewhere": the unlocked one's box stays
 * empty, `checked-mismatch` is reported, this set does not exempt it, and nothing in `blocking` is
 * model-fixable — so a **complete** draft is thrown away with 「没有一条是模型能改的」. Warframe,
 * on 格斗精通II and 特工 (issue #81).
 *
 * The fallback matters as much as the rule: a game whose achievement has no name in `lang` at all
 * has to write the other one, so that is the name that decides. Reading a missing name as "not
 * unique" would exempt every achievement in such a game and hide a genuinely broken ticker.
 *
 * `byName` stays indexed over **both** languages, because `computeCheckedKeys` looks candidates up
 * in that same combined index — a Chinese name equal to another achievement's English name really
 * does collide there, and a per-language index would disagree with the code this predicts.
 *
 * @param {Array}  defs   this game's achievement definitions
 * @param {string} [lang] 'zh' | 'en' — the language the guide is written in
 */
export function unnameableApiNames(defs, lang = 'zh') {
  const byName = buildNameIndex(defs);
  const out = new Set();
  for (const d of defs) {
    const own = lang === 'en' ? d.name_en : d.name_cn;
    const other = lang === 'en' ? d.name_cn : d.name_en;
    const written = normalizeText(own) ? own : other;
    const k = normalizeText(written);
    if (!k || byName.get(k)?.size !== 1) out.add(d.api_name);
  }
  return out;
}
