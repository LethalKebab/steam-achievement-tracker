...
// **和 audit 用同一个判据。** 这一条 lint 的措辞是「audit 反查不了这个框」,
// 所以它必须拿 audit 真正用的那个函数去判 —— 自己再写一份,迟早只改一份
 ...
 */
/**
 * 本地 markdown 里,哪些**行号**落在 `<details>…</details>` 内部。
 *
 * Notion 那边这件事由 `fetchAllToDoBlocks` 的 `container` 标记回答;本地攻略只有文本,
 * 所以在这里按行算一遍。两个后端要给同一个答案 —— 同一份攻略换个后端结论不一样,
 * 是 bug 不是设计。
 */
...
export function lintGuide({ todos, defs, text = null, unlockedApiNames = null, kind = 'notion' }) {
  ...
  /**
   * **反过来的那一半:顶层 checkbox 认不出是哪个成就。**
   *
   * 上面那条查的是「每个成就都有框」,查不到「每个顶层框都是成就」。两者不是一回事:
   * 子步骤写成了顶层(该缩进的没缩进)时,成就一个不缺、覆盖率 100%,而那些框
   * **永远勾不上** —— `checkbox-sync` 按成就名精确匹配,匹配不到就不勾,一直挂在那儿。
   *
   * **实测漏过一次**:《破晓传奇》生成完报的是「覆盖 58/58,0 条 warn」,而页面上有
   * 70 个顶层 checkbox —— 多出来的 12 个是「12 个个人支线一览」里该嵌套的子步骤。
   *
   * 只报 `warn` 不报 `error`:这是模型的书写违规,不是数据错误,而 error 会驱动重写轮次,
   * 模型改不动的话就是白烧三轮。看得见就够了。
   */
  const inDetails = detailLineSet(text);
  for (const t of todos) {
    if (t.parent !== null && t.parent !== undefined) continue;
    // 装在折叠里的不算孤儿:规则五明说长清单可以用 `<details>` 包一串 checkbox,
    // 那些条目本来就不是成就。Notion 那边靠 `container` 标记,本地 md 靠行号
    if (t.container || inDetails.has(t.key)) continue;
    const hits = extractTitleCandidates(normalizeText(t.text)).some((c) => byName.has(c));
    if (hits) continue;
    add('warn', 'orphan-todo', `顶层 checkbox 认不出是哪个成就,永远勾不上:${t.text.split('\n')[0].slice(0, 40)}`, {
      key: t.key,
    });
  }
  ...
  for (const [name, apiNames] of byName) {
    ...
    for (const apiName of apiNames) {
      ...
      if (hasDesc) continue;
      // **两种情况用两个 code,不是一个 code 加一个布尔字段。**
      //
      // 它们的补救办法根本不同:有描述的那种,重写攻略把描述抄上去就好了;描述是空的
      // 那种,谁都修不了 —— Steam 那边就没有可抄的字符串。而这个项目里"这种失败能不能
      // 补救"一律是按 `code` 分流的(`MODEL_FIXABLE`、`splitFindings`、`CLI_HINTS` 都是),
      // 所以把区别塞进一个只有这条规则会设的 `fixable` 布尔值,等于给同一个问题开第二套
      // 机制 —— 而实际结果是那个字段**算出来了却没有任何生产代码读它**(只有一条测试
      // 断言它被设上了),于是一份 197 条全写对的攻略被 15 条谁都改不动的错误拦下来,
      // 还先花三轮让模型去抄不存在的描述。实测撞到过:
      // KINGDOM HEARTS -HD 1.5+2.5 ReMIX-(四合一合集,16 个名字撞车)。
      //
      // 名字用 `d.name_cn || d.name_en` 而不是 `name`:`name` 是 `normalizeText` 之后的
      // 索引键(小写、去标点),照着念是 `proud player`,而用户在 Steam 上看到的是
      // `Proud Player`。报给人看的东西要用人看得到的那个写法
      const shown = d.name_cn || d.name_en || name;
      if (d.description) {
        add('error', 'ambiguous-no-description',
          `同名成就「${shown}」没抄描述原文,靠名字分不出是哪一个,永远同步不上`,
          { apiName, name: shown });
      } else {
        add('error', 'ambiguous-empty-description',
          `同名成就「${shown}」在 Steam 上的描述是空的,无法区分——它注定同步不上,` +
            '而这不是攻略能修的',
          { apiName, name: shown });
      }
    }
 ...
 *
 * **按名字算,不是按成就算**——中文名撞车但英文名唯一的照样勾得上,和匹配那边
 * 「闸门按名字关」是同一条规则。绝大多数同名其实只在一种语言下撞车。
 */
