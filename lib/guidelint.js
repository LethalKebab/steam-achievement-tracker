...
import { todoSpansWithToggles, detailsBlockEnd } from './markdown.js';
...
/** The `<summary>` of a `<details>`, wherever in the block it was written */
const SUMMARY_RE = /<summary\b[^>]*>([\s\S]*?)<\/summary\s*>/i;
/** A summary that is trying to be the spoiler notation */
const SPOILER_WORD_RE = /剧透|spoiler/i;
/** …and the fixed form it has to be. Both languages are accepted — see `spoilerFolds` */
const SPOILER_LABEL_RE = /^(?:剧透|spoiler)$/i;
/** A checkbox line, in the shape both backends read (markdown.js's `/^\s*[-*]\s*\[/`) */
const FOLD_CHECKBOX_RE = /^\s*[-*]\s*\[[ xX]\]/;

/**
 * The `<details>` blocks carrying the **spoiler notation**, and what is wrong with each.
 *
 * Three of the notation's properties are pure form and so belong here. **What is inside is not**:
 * whether the folded content really spoils anything, and whether something left unfolded should have
 * been folded, are content judgements this validator cannot make and never claims to. There can
 * therefore be no completeness check on this notation — an absent fold and "nothing here spoils" are
 * the same text — which is why the count below is *reported* and never compared against a threshold.
 *
 * **Both languages' labels are accepted whatever language the guide is in.** Threading `lang` in to
 * reject the other one buys nothing: a label in the wrong language is a content fault, and content
 * is what this file does not check. Accepting both can only ever refuse less.
 *
 * **Attachment is decided by `todoSpansWithToggles`** rather than by a lookalike "is the line above a
 * checkbox" test — the question is whether the fold is adjacent to the achievement and indented under
 * it, and that function already answers it. A blank line between the two is enough for it not to be.
 *
 * **It is not the same question as "will a partial rewrite carry it along", and only Notion's answer
 * is yes.** `parsePatchReply` picks `todoSpansWithToggles` for Notion and plain `todoSpans` for local
 * markdown, and `spliceIntoText` — the local write — uses `todoSpans` unconditionally, whose range
 * ends at the first non-checkbox line and therefore never contains a fold. So on a local guide
 * `--only` replaces the entry's line and **leaves the old fold sitting where it was**: stale rather
 * than lost, which is why that is not reported here. Widening `todoSpans` to fix it is not available
 * — its conservatism is what stops a splice deleting text silently.
 *
 * An **unclosed** `<details>` is skipped rather than reported here. markdown.js deliberately refuses
 * to guess where such a block ends, and reading one as a spoiler fold would mean guessing the same
 * thing in a second place.
 */
function spoilerFolds(text) {
  const src = String(text ?? '');
  const lines = src.split(/\r?\n/);

  const attached = new Set();
  for (const span of todoSpansWithToggles(src).values()) {
    for (let i = span.start; i <= span.end; i++) attached.add(i);
  }

  const out = [];
  for (let i = 0; i < lines.length; i++) {
    if (!/^<details\b/i.test(lines[i].trim())) continue;
    const close = detailsBlockEnd(lines, i);
    if (close === null) continue;
    const hit = lines.slice(i, close + 1).map((l) => SUMMARY_RE.exec(l)).find(Boolean);
    const label = String(hit?.[1] ?? '').replace(/\*\*/g, '').trim();
    if (!SPOILER_WORD_RE.test(label)) continue;
    out.push({
      line: i + 1,
      label,
      labelled: SPOILER_LABEL_RE.test(label),
      // The opening and closing tags themselves are not content
      hasCheckbox: lines.slice(i + 1, close).some((l) => FOLD_CHECKBOX_RE.test(l)),
      attached: attached.has(i),
    });
  }
  return out;
}
...
export function lintGuide({ todos, defs, text = null, unlockedApiNames = null, kind = 'notion' }) {
  ...
  if (text !== null) {
    ...
    // ---- The spoiler notation. Form only; see spoilerFolds ----
    const folds = spoilerFolds(text);
    spoilerFoldCount = folds.length;
    for (const f of folds) {
      // **A warn, not an error.** A label carrying more than the fixed word defeats the notation,
      // but it breaks neither the sync nor a rewrite — the two things an error here is spent on —
      // and every error is fed back to the model and paid for as another rewrite round. Promote it
      // if the model turns out to do this often; that is a one-word change.
      if (!f.labelled) {
        add('warn', 'spoiler-summary-form', msg('lint.spoilerSummaryForm', { line: f.line, label: f.label.slice(0, 40) }));
      }
      // Errors, both: a checkbox in there becomes a sub-step and is cascade-ticked into a record of
      // something the player never did, and a detached fold is text a partial rewrite leaves behind
      if (f.hasCheckbox) {
        add('error', 'spoiler-fold-checkbox', msg('lint.spoilerFoldCheckbox', { line: f.line }));
      }
      if (!f.attached) {
        add('error', 'spoiler-fold-detached', msg('lint.spoilerFoldDetached', { line: f.line }));
      }
    }
  }
  ...
  return {
    ...
    stats: {
      ...
      // **`null`, not 0, when there is no text.** A Notion guide's folds are toggles, and toggles
      // are not `to_do` blocks, so nothing in this function's inputs can see them. Reporting 0
      // there would state "this guide folds nothing" on the strength of not having looked
      spoilerFolds: spoilerFoldCount,
    },
  };
}
