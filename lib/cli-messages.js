...
import { TRACKER_MESSAGES } from './tracker-messages.js';

export const CLI_MESSAGES = {
  // ---- serve's own log --------------------------------------------------------
  'ai.retrying':          ['  第 {attempt} 次失败({reason}),{wait}ms 后重试', '  attempt {attempt} failed ({reason}); retrying in {wait}ms'],
  ...
  'srv.patchFailed':      ['⚠️ 局部重写未通过校验,原攻略未改动:{url}', '⚠️ The partial rewrite did not pass the checks; the guide is untouched: {url}'],
  'srv.genDone':          ['✅ 攻略写完:{url}', '✅ Guide written: {url}'],
  'srv.genDraft':         ['⚠️ 攻略未通过校验,草稿留在 {path}', '⚠️ The guide did not pass the checks. The draft is at {path}'],
  'srv.patchError':       ['❌ 局部重写失败:{reason}', '❌ The partial rewrite failed: {reason}'],
  'srv.genError':         ['❌ 攻略生成失败:{reason}', '❌ Guide generation failed: {reason}'],
  'srv.patchCancelled':   ['🚫 局部重写已取消:{game}({appid})', '🚫 Partial rewrite cancelled: {game} ({appid})'],
  'srv.genCancelled':     ['🚫 攻略生成已取消:{game}({appid})', '🚫 Guide generation cancelled: {game} ({appid})'],
 ...
 *
 * **One lookup over both terminal tables.** They are split by size rather than by audience — see the
 * header of `tracker-messages.js` — so splitting the lookup as well would put the language in two
 * places, and two places holding a language is where they start to disagree.
 */
export function clog(key, values) {
  const pair = CLI_MESSAGES[key] ?? TRACKER_MESSAGES[key];
  if (!pair) return key;
