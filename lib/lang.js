...
export function achievementName(row, lang) {
  // **Both shapes.** Rows straight out of the achievements table are snake_case; the normalised
  // objects guides.js builds are camelCase, and both reach this function. Accepting one and
  // silently returning '' for the other is the shape of bug that shows as a name going missing in
  // half the messages and nowhere else
  return pick(row?.name_cn ?? row?.nameCn, row?.name_en ?? row?.nameEn, lang)
    || row?.api_name || row?.apiName || '';
}
...
export function achievementDescription(row, lang) {
  return pick(row?.description, row?.description_en ?? row?.descriptionEn, lang);
}
