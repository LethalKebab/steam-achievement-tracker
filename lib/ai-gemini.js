...
/**
 * The first wait for a 503, longer than the ordinary one.
 *
 * The general ladder is 1s + 2s + 4s ≈ **7 seconds**, sized for a hiccup. A Gemini 「high demand」
 * 503 is not a hiccup: measured, it persisted across repeated attempts over several minutes on two
 * different models. Seven seconds against that is the same as not retrying at all, which is how a
 * run that had already spent minutes on research died at the first request.
 *
 * 5s + 10s + 20s ≈ 35 seconds covers a genuine short spike and is invisible inside a run that takes
 * minutes anyway. **It does not rescue a long one, and the hint says so** rather than implying that
 * a higher `ai.maxRetries` would.
 */
const UNAVAILABLE_RETRY_BASE_MS = 5000;
/**
 * How many times a 503 is re-asked, **independent of `ai.maxRetries`**.
 *
 * Measured, 10 **bare** requests per model during a spike: `gemini-flash-latest` answered 6 of 10,
 * `gemini-3.8-flash` 4 of 10, and among small requests neither size nor declared tools changed the
 * odds — all three shapes both succeeded and failed inside one loop.
 *
 * **Four things were tried and none of them moves the odds**: request size (13k system prompt),
 * declared tools, the `maxOutputTokens: 32000` reservation, and streaming versus not — each pair
 * measured head to head, interleaved, and each came out level. What does change is **the moment**:
 * early in one session every request failed for minutes on end, and later the same bare request
 * succeeded about half the time. So the rate is not a property of Gemini, or of our request; it is
 * a property of the spike, and it ranges from zero to roughly half.
 *
 * Which is exactly why the answer is more draws rather than a different request.
 *
 * That is what decides the number. At 40% a request wins, `maxRetries: 3` still loses one attempt
 * in eight (0.6⁴), and **a guide run is many requests** — shards, rewrite rounds, the aside passes —
 * each an independent draw, so one in eight per request is a run that usually dies. Five retries
 * put it near one in a hundred. The wait that buys is ~95s against a run measured in minutes.
 *
 * `ai.maxRetries` stays what it is for every other failure: it is a budget for hiccups, and this is
 * not one.
 */
const UNAVAILABLE_MAX_RETRIES = 5;
...
export class GeminiProvider {
  ...
  constructor(ai, { fetchImpl = globalThis.fetch, log = () => {} } = {}) {
    if (!ai?.apiKey) {
      throw new AiError(
        msg('ai.noKeyConfigured', { vendor: 'Gemini', envVar: 'GEMINI_API_KEY' })
          + msg('ai.geminiKeyWhere')
      );
    }
    this.name = 'gemini';
    this.ai = ai;
  ...
  #retryDelayFor(err, attempt) {
    ...
    const base = err.status === 503 ? UNAVAILABLE_RETRY_BASE_MS : RETRY_BASE_MS;
    return Math.min(base * 2 ** attempt, RETRY_MAX_MS);
  }

  async #withRetry(body, opts) {
    let lastErr;
    // A 503 gets its own budget — see UNAVAILABLE_MAX_RETRIES. Resolved per attempt rather than up
    // front because which failure this is only becomes known once one has happened
    const limitFor = (err) => (err?.status === 503 ? UNAVAILABLE_MAX_RETRIES : this.maxRetries);
    for (let attempt = 0; ; attempt++) {
      try {
        return await this.#once(body, opts);
      } catch (err) {
        lastErr = err;
        if (!(err instanceof AiError) || !err.retryable || attempt >= limitFor(err)) throw err;
...
async function errorFromResponse(res, model) {
  ...
  if (/no longer available|deprecated|retired/i.test(detail)) {
    ...
    hint = msg('ai.gemRetired', { model });
  } else if (res.status === 404 || /not found|not supported/i.test(detail)) {
    code = 'gemini-model-unknown';
    hint = msg('ai.gemUnknownModel', { model });
  } else if (noAllowance) {
    code = 'gemini-no-allowance';
    hint = msg('ai.gemNoAllowance', { model });
  } else if (res.status === 429) {
    hint = msg('ai.gemQuota');
    if (retryAfterMs) hint += msg('ai.gemRetryAfter', { seconds: Math.ceil(retryAfterMs / 1000) });
    if (!violations.length) {
      ...
      hint += msg('ai.gemNoQuotaDetail');
    }
  } else if (res.status === 400 && /tool|function/i.test(detail)) {
    code = 'gemini-tool-rejected';
    hint = msg('ai.gemToolRejected');
  } else if (res.status === 403) {
    hint = msg('ai.gemForbidden');
  } else if (res.status === 503) {
    /**
     * **Capacity, not configuration** — and saying so is the whole point of this branch.
     *
     * Measured on one key within a few minutes: `gemini-flash-latest` and `gemini-3.6-flash` both
     * 503, `gemini-3.8-flash` answered 200 and then 503 for the byte-identical request, and
     * `gemini-2.5-flash` gave 404 「no longer available to new users」. Three different faults, and
     * without a branch here the middle one arrived as the vendor's raw English sentence — which
     * reads exactly like a misconfigured model and sends people to change a setting that was right.
     */
    code = 'gemini-unavailable';
    hint = msg('ai.gemUnavailable', { model });
  }

  const detailLines = violations.length
    ? msg('ai.gemQuotaDetail', {
      lines: violations.map((v) => msg('ai.gemQuotaLine', {
        metric: v.metric, id: v.id ? ` (${v.id})` : '', value: v.value ?? '?',
      })).join('\n'),
    })
    : '';
  ...
  const err = new AiError(
    msg('ai.httpError', {
      vendor: 'Gemini', status: res.status, type: status ? ` ${status}` : '',
      detail, hint: `${detailLines}${hint}`,
    }),
    {
      status: res.status,
      type: status,
