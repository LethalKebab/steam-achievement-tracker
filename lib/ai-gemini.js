...
async function errorFromResponse(res, model) {
  ...
  // 对新申请的 key 已经不给用,但它照样出现在 --models 列表里 —— 列出来 ≠ 能用
  if (/no longer available|deprecated|retired/i.test(detail)) {
    hint = `\n  ⚠️  ${model} 对新 key 已经停止提供了(不是名字写错)。\n` +
      '      注意 `--models` 列出来的模型**不一定能用**,那个接口不反映这件事。\n' +
      '      换新一代:--model gemini-3.5-flash-lite 或 --model gemini-flash-latest';
    ...
    hint = `\n  ⚠️  这一档对 ${model} 的额度是 **0** —— 不是用完了,是这个模型不在你这一档里。\n` +
      ...
      '      改 config.json 的 ai.model;有哪些可用跑 `node tracker.js ai-check --models`。';
