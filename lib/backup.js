 ...
 *
 * 为什么不是"导出 CSV 再导回来"(2026-08-19 把那条路整个删了,见 lib/csv.js):
 ...
 *    也不需要重启子进程。
 *
 * 3. **列是按交集搬的,不是 SELECT *。** 老版本的备份少几列(cover_url 这种
 *    后加的),INSERT INTO games SELECT * FROM bak.games 会直接报列数不匹配;
...
/**
 * zip 里的路径不能直接拿去写盘:一个 ../ 就能把文件写到 guides/ 外面去
 * (zip-slip)。这里只接受留在 guides/ 里面的相对路径。
 */
function safeGuidePath(guidesDir, entryName) {
  ...
  return join(guidesDir, ...rel.split('/'));
}
