...
/**
 * Which tables to move, and the list is **checked against the schema by a test** rather than
 * kept in step by memory.
 *
 * `sqlite_sequence` is not among them: it is AUTOINCREMENT's internal bookkeeping and follows
 * the rows.
 *
 * **`hltb` was missing from this list for a month and nothing said so.** The list was written
 * on 2026-08-19; the table arrived on 2026-09-08 and this line was never revisited, so a restore
 * silently kept whatever the live database held and dropped every HowLongToBeat resolution the
 * backup carried. The header above calls moving the database "structurally lossless" — true of
 * the *zip*, which is a whole-file VACUUM INTO, and not true of this path, which is a
 * hand-maintained list. That is the same discipline dependency the header rejects CSV for, one
 * level up: CSV had to keep column order, this has to keep table membership.
 *
 * The cost is not symmetrical across the two, which is why both are here:
 * - `hltb` is expensive to lose — re-resolving is a search plus up to three page fetches per
 *   game against somebody's website — but it *can* be re-derived.
 * - `hltb_history` **cannot**. It is a record of readings taken at moments that do not come
 *   back, so a restore that drops it destroys the measurement outright rather than costing
 *   requests.
 *
 * Old backups already contain both: the zip has always held the whole database file, so this
 * fix also recovers the rows from backups taken before it.
 */
export const TABLES = ['games', 'achievements', 'guides', 'hltb', 'hltb_history', 'sync_log', 'meta'];
