...
export class SteamClient {
  constructor(cfg, { log = () => {} } = {}) {
    ...
    // **两条路的限流宽严差得远,所以是两个旋钮。**
    // `delay` 管 api.steampowered.com 的官方 Web API。实测过一次:GetPlayerAchievements
    // 连打 400 个、间隔 0ms、持续 11 次/秒 36 秒,**一个 429 都没有**(另有一个
    // 300→0ms 的阶梯,每档 30 个,同样全绿)。所以这里给 100ms 已经留了一半余量。
    // `storeDelay` 管 store.steampowered.com —— 抓的是商店网页 HTML,限流严得多,
    // 而且撞上去是 IP 级的,不是 key 级的。**这一条故意没有实测**:为了一个数字去高频
    // 刷商店页,风险和收益完全不成比例。它保持 300ms,调它之前先想清楚上面这句话。
  ...
  async fetchAppName(appid) {
    ...
    // 两边都是 store.steampowered.com(appdetails 和商店页 HTML),所以用 storeDelay
