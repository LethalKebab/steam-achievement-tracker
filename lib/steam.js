...
export class SteamClient {
  ...
  /**
   * `unlocked` is the api_names this account holds, carried out alongside the counts.
   *
   * It is **free**: the response already lists every achievement with its own flag, and until now
   * the array was counted and dropped. Naming which ones are held is what lets the interface say
   * more than a number — and asking a second time would be a second request per game.
   */
  async fetchAchievementStats(appid) {
    ...
    const unlocked = raw.achievements.filter((a) => a.achieved === 1).map((a) => a.apiname);
    return { total, achieved: unlocked.length, unlocked };
  }
