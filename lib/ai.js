 ...
 */
export function assertModelMatchesProvider(provider, model) {
  if (!model) return;
...
export async function createProvider(config, opts = {}) {
  ...
  assertModelMatchesProvider(provider, ai.model);
  ...
  throw new AiError(`还没接入的供应商:${provider}(目前支持 anthropic、gemini、deepseek)`);
}
