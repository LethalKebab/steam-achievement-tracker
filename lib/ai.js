 ...
 * ------------------------------------------------
 * Each vendor's differences are confined to its own file (`ai-anthropic.js` / `ai-gemini.js`);
 ...
 *
 * The vendors phrase it differently (Anthropic uses `end_turn`/`max_tokens`/`refusal`, Gemini uses
 * `STOP`/`MAX_TOKENS`/`SAFETY`/`RECITATION`), and it is **translated to one set at the provider
 * boundary** so that `checkResult` and `guidegen` are not littered with "if it is this vendor then…".
 *
 ...
 * - `recitation` blocked for reproducing copyrighted content at length (only Gemini has this today)
 ...
 * environment variable AI_PROVIDER still said gemini, so deepseek-chat was requested from Gemini's
 * endpoint.
 */
...
export function createSession(provider, { system = null, tools = null } = {}) {
  ...
  return {
    ...
    async ask(userText, { onEvent, signal } = {}) {
      ...
      const r = await provider.send({ system, messages, tools, onEvent, signal });
...
export function checkResult(r) {
  ...
  if (r.stopReason === 'recitation') {
    ...
    return bad(
      'recitation',
      '因大段复述受版权保护的内容(RECITATION)被中断。这类任务容易撞上——' +
