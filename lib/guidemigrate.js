...
import { planNotionTarget } from './notion.js';
...
export async function planMigration(db, { notion, config, appid }) {
  ...
  const game = getGame(db, id)?.name || guide.name || id;
  const target = await planNotionTarget(notion, game);
...
export async function migrateGuideToNotion(
  db,
  { notion, config, appid, plan = null, onProgress = () => {} }
) {
  ...
  if (page) {
    ...
    // 图标不在这里补:本地攻略搬家是"换个地方放",不是"重做一份"。
    // 想要图标,Notion 页面上自己拖一个更快,也不用为此多打一次 Steam 接口
