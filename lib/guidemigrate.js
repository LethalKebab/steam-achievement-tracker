...
export function checkFidelity(sourceTodos, notionTodos) {
  ...
  if (a.length !== b.length) {
    problems.push(msg('migrate.countMismatch', { a: a.length, b: b.length }));
  }

  for (let i = 0; i < Math.min(a.length, b.length); i++) {
    if (a[i].text !== b[i].text) {
      problems.push(msg('migrate.textMismatch', {
        n: i + 1, a: a[i].text.slice(0, 70), b: b[i].text.slice(0, 70),
      }));
      ...
      const tick = (on) => msg(on ? 'migrate.ticked' : 'migrate.unticked');
      problems.push(msg('migrate.tickMismatch', {
        n: i + 1, from: tick(a[i].checked), to: tick(b[i].checked), text: a[i].text.slice(0, 40),
      }));
    }
    if (problems.length >= 10) {
      problems.push(msg('migrate.andMore'));
      break;
    }
  }
