...
export class NotionClient {
      ...
      for (const block of data.results ?? []) {
        if (block.type === 'to_do') {
          results.push({
            ...
            parent,
            // **装在 toggle / column 里的 checkbox 要标出来。** `parent` 分不出这件事:
            // 顶层折叠(规则五的长清单)里的条目,`parent` 原样往下传就是 `null`,
            // 和「该缩进却写成了顶层的孤儿子步骤」长得一模一样。校验器要区分这两者。
            container,
          });
        ...
        if (block.has_children && !['child_database', 'link_to_page'].includes(block.type)) {
          ...
          // 但**要记下「它在容器里」** —— 见上面 `container` 那段注释
  ...
  async replaceTodoChildren(blockId, blocks) {
    for (const k of await this.childBlockStubs(blockId)) {
      // **只删正文块。** `to_do` 是子步骤,`toggle` 是分组标签(前置/步骤/注意,见
      // guidegen 的 `groupLabelRule`)—— 两者都是模型每次重写时整份交回来的东西。
      // 图片、表格、段落这些**不删**:找物类游戏的位置只能靠用户自己贴图说清楚
      // (规则二明说模型给不出可靠的游戏内截图),顺手删掉它们是不可接受的,
      // 而且删了不报错,用户下次打开才发现,还对不上是哪一步弄的。
  ...
  /**
   * 一个块的**直接**子块,只要 `{id, type}`。
   *
   * 不用 `fetchAllBlocks`:那个把整棵子树都拉下来,而这里只需要知道第一层每个块是
   * 什么类型。删父块时 Notion 会把整棵子树一起归档,所以更深的层不需要枚举。
   */
  ...
  async appendBlocks(pageId, blocks, { after = null, atStart = false } = {}) {
    let written = 0;
    /**
     * **定位插入只能给第一批用,后面每一批要接在上一批的最后一块后面。**
     * 不接的话第 2 批会掉到整页末尾,于是一份攻略被拦腰劈成两段 —— 而且不报错。
     * (`after` 在 2022-06-28 上实测可用;新版叫 `position.after_block`,是同一件事。)
     */
    ...
    for (const chunk of chunkBlocks(blocks)) {
      ...
      try {
        const res = await this.request('patch', `/blocks/${pageId}/children`, {
          children: shallow,
          // 第一批可能要插到页首(前面没有任何保留块可以当锚点);之后一律接在
          // 上一批最后一块后面。两种定位在 2022-06-28 上都实测可用
