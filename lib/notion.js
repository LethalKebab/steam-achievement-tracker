...
export class NotionClient {
  ...
  async createGuideBoardView(databaseId) {
    ...
    if (!dataSourceId) throw new Error('这个库没有 data source,建不了看板视图');
    ...
    if (!status) throw new Error('读不到状态属性的 id,建不了看板视图');
  ...
  if (!schema.status) {
    ...
    if (missing.length) {
      ...
      if (wrongGroup.length) {
        add(DB_PROBLEM.OUTDATED_FORMAT, 'warn', '这个库是旧格式:状态没有按看板分组,也没有看板视图', {
          property: schema.status.property,
          outdated: wrongGroup,
          colours: wrongColour,
        });
      } else if (wrongColour.length) {
        const want = wrongColour.map((n) => `${n} → ${GUIDE_STATUS_STYLE[n].color}`).join(',');
        add(DB_PROBLEM.COLOUR_BY_HAND, 'warn', `这几个状态的颜色只能你自己改:${want}`, {
          property: schema.status.property,
          colours: wrongColour,
          hint: 'Notion 不允许改已有选项的颜色。打开那个库 → 点状态属性 → 逐个挑颜色。不影响使用,只是看着分不清',
        });
      }
    }
  }
