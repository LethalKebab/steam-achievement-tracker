...
/**
 * 颜色的中文名。**这是给人看的,不是给 API 看的** —— 提示里要说的是他在 Notion 的取色盘上点哪一个,
 * 而那个盘子在中文工作区里写的是「蓝」「紫」,不是 `blue` `purple`。
 */
...
export class NotionClient {
  ...
  async createGuideBoardView(databaseId) {
    ...
    if (!dataSourceId) throw new Error('这个库读不出数据源,建不了看板视图');
    ...
    if (!status) throw new Error('这个库读不出状态属性,建不了看板视图');
...
export async function inspectGuideDb(notion, dbId, { probeWrite = false } = {}) {
  ...
  if (!schema.status) {
    ...
    if (missing.length) {
      ...
      /*
       * **Two problems, because one of them has a button and the other cannot.** Reporting both as
       * one meant the warning survived its own repair: groups get fixed, colours never can, so the
       * flag stayed lit and the button stayed on screen doing nothing on every further press —
       * exactly the "button that does nothing when pressed" the `fixable` comment below rejects.
       *
       * So: anything the program can act on is OUTDATED_FORMAT and carries the button. Once only
       * the colours are left, it becomes a different problem that says what to click and offers no
       * button, which is a state the user can actually finish.
       */
      if (wrongGroup.length) {
        add(DB_PROBLEM.OUTDATED_FORMAT, 'warn', '这个库是早先建的:四个状态还堆在一栏里,也没有看板视图', {
          property: schema.status.property,
          outdated: wrongGroup,
          colours: wrongColour,
        });
      } else if (wrongColour.length) {
        const want = wrongColour.map((n) => `${n} ${COLOUR_ZH[GUIDE_STATUS_STYLE[n].color]}`).join(',');
        add(DB_PROBLEM.COLOUR_BY_HAND, 'warn', `这几个状态的颜色要你自己去 Notion 里挑:${want}`, {
          property: schema.status.property,
          colours: wrongColour,
          hint: '已经建好的选项,Notion 只让人在界面里改颜色。打开那个库 → 点状态属性 → 逐个挑一次。不改也能用,只是看板上一片灰。',
        });
      }
    }
  }
...
export async function repairGuideDb(notion, dbId) {
  ...
  if (!missing.length && !wrongGroup.length) {
    return {
      ...
      regrouped: [], wrongColour, boardView, reason: 'nothing-to-do',
    };
  }
