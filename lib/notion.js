 ...
 */
/**
 * A Notion option colour, as a word. Keys rather than words now — the CLI prints these when it
 * reports what `notion-check --fix` recoloured, so they follow the interface language like the
 * sentence around them
 */
const COLOUR_ZH = { default: 'colour.default', blue: 'colour.blue', purple: 'colour.purple', green: 'colour.green' };
...
export class NotionClient {
  ...
  async searchPages({ maxPages = 5 } = {}) {
    ...
    for (let i = 0; i < maxPages; i++) {
      ...
      for (const page of data.results ?? []) {
        ...
        results.push({
          id: page.id,
          title: richTextToPlain(titleProp?.title) || msg('notion.untitled'),
          url: page.url,
   ...
   */
  async createGuideDatabase({ parentPageId, title = msg('notion.defaultDbTitle') }) {
...
export async function inspectGuideDb(notion, dbId, { probeWrite = false } = {}) {
  ...
  try {
    ...
    workspace = me.name || me.bot?.workspace_name || msg('notion.unnamedWorkspace');
  } catch (err) {
    // **The reason travels beside the sentence.** `api.js` used to recover it by stripping the
    // Chinese prefix off `message` with a regex, which is treating prose as an interface: the
    // moment this composes in English that strip matches nothing and the prefix comes out doubled
    add(DB_PROBLEM.BAD_TOKEN, 'error', msg('notion.tokenBad', { reason: err.message }),
      { reason: err.message });
    return { ok: false, fixable: false, workspace, database: null, schema: null, problems };
  }

  if (!dbId) {
    add(DB_PROBLEM.NO_DB_ID, 'error', msg('notion.noDbIdYet'));
    return { ok: false, fixable: false, workspace, database: null, schema: null, problems };
  }
  ...
  try {
    ...
    add(DB_PROBLEM.DB_UNREADABLE, 'error', msg('notion.dbIdUnreadable', { reason: err.message }), {
      causes: [msg('notion.causeNotADb'), msg('notion.causeNotShared')],
    });
    return { ok: false, fixable: false, workspace, database: null, schema: null, problems };
  }
  ...
  const database = {
    ...
    title: (raw.title ?? []).map((t) => t.plain_text).join('') || msg('notion.untitled'),
    url: raw.url ?? null,
  };
  ...
  if (!hasTitle) {
    add(DB_PROBLEM.NO_TITLE_PROP, 'error', msg('notion.noTitleProp'));
  }
  ...
  if (!schema.status) {
    ...
    add(DB_PROBLEM.NO_STATUS_PROP, 'warn', msg('notion.statusPropAbsent'), {
      wanted: [...GUIDE_STATUS_OPTIONS],
    });
  } else {
    const missing = GUIDE_STATUS_OPTIONS.filter((o) => !schema.status.options.includes(o));
    if (missing.length) {
      add(DB_PROBLEM.MISSING_OPTIONS, 'error', msg('notion.missingOptions', { missing: missing.join(' / ') }), {
        property: schema.status.property,
      ...
      if (wrongGroup.length || wrongColour.length) {
        ...
        add(DB_PROBLEM.OUTDATED_FORMAT, 'warn', msg('notion.outdatedFormat'), {
          property: schema.status.property,
          outdated: wrongGroup,
          colours: wrongColour,
        });
  ...
  if (probeWrite && !problems.some((p) => p.severity === 'error')) {
    ...
    if (!probe.ok) {
      add(DB_PROBLEM.NO_WRITE, 'error', msg('notion.writeProbeFailed', { reason: probe.error }), {
        hint: msg('notion.writeProbeHint'),
      });
    }
    if (probe.strandedUrl) {
      add(DB_PROBLEM.STRANDED_PROBE_PAGE, 'warn', msg('notion.strandedProbePage'), {
...
export async function probeGuideDbWrite(notion, dbId, schema) {
  ...
  try {
    page = await notion.createGuidePage({
      ...
      title: msg('notion.probePageTitle', { when: new Date().toISOString() }),
      status,
    });
  } catch (err) {
