...
export class NotionClient {
   ...
   *
   * "The call succeeded ≠ the content is right" is not a slogan on this path, it was measured:
   * **the status property's groups (`groups`) are ignored by Notion whether passed at creation or
   * PATCHed afterwards — HTTP 200 every time, nothing changed.** All three payload shapes were
   * tried, all silently ineffective. So whether the four options actually landed can only be
   * answered by reading it back and asking `pickGuideDbProperties` — and asking *it*, not parsing
   * the response here, because it is the real downstream consumer.
   *
   * The conclusion on groups: all four options in the created database land in the `To-do` group,
   * with zero functional impact (the code only reads `options` and never looks at `groups`); it
   * only looks odd in the board view, and tidying it up means dragging them by hand.
   */
...
export async function inspectGuideDb(notion, dbId, { probeWrite = false } = {}) {
  ...
  return {
    ...
    // Missing options is the only fault the program can possibly repair. The rest are either a few
    // clicks in Notion or a different ID — marking those fixable too would just produce a button
