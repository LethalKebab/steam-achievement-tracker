 ...
 * - Local but absent from the owned list (family-shared / delisted / hand-added) → left
 *   completely untouched. The test is "is it in the owned list", not what status says
 */
...
/**
 * Phase three: achievement detail (Chinese/English names, descriptions, icons) → the achievements table.
 * Refreshed when this appid has no rows yet, or when the achievement total rose within the last
 * 7 days (meaning a game update added achievements).
 * The skips are consistent with that: games confirmed to have no achievement system, and games
 * at exactly 100% (already complete, so no checklist is needed).
 */
/**
