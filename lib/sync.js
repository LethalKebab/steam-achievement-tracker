...
/**
 * How many already-resolved games may have their hours re-read in one run.
 *
 * **The cap exists because the due dates herd.** Everything resolved in the first sync is stamped
 * within minutes of everything else, so thirty days later the whole library falls due on the same
 * run — the identical spike `selectStatsTargets`'s sweep budget exists to prevent, and the reason
 * that one is documented as "do not remove the cap". Spreading the work also de-herds it
 * permanently: each run stamps only the rows it actually read, so after one cycle the due dates
 * are scattered across the month by themselves.
 */
export function selectHltbTargets(db, { refreshBudget = 40 } = {}) {
  ...
  // Stalest first, so a capped run always spends its budget on the rows furthest out of date
  refresh.sort((a, b) => String(a.row.checked_at ?? '').localeCompare(String(b.row.checked_at ?? '')));
  return { resolve, refresh: refresh.slice(0, refreshBudget), refreshPending: Math.max(0, refresh.length - refreshBudget) };
}

export async function syncHltb(db, hltb, { onProgress = () => {}, refreshBudget } = {}) {
  const { resolve, refresh, refreshPending } = selectHltbTargets(db, { refreshBudget });
  ...
  for (const g of resolve) {
    // The breaker has decided the service is not answering. Continuing would spend the rest of
    // the run on requests that cannot succeed, and — worse — would write a **recorded miss** for
    // every one of them, which is permanent: those games would never be searched for again
    if (hltb.stopped) break;
  ...
  return { resolved, notFound, refreshed, candidates: total, refreshPending, stopped: hltb.stopped ?? null };
}
 ...
 */
export async function fullSync(db, steam, { onProgress = () => {}, selection = null, hltb = null, hltbRefreshBudget } = {}) {
  ...
  //
  // **And it can never fail the sync.** Same rule the Notion tick pass follows: a third party
  // being down is not this program's sync failing, and the four phases above it have already
  // landed their data by the time this runs. `HltbClient` catches its own network errors, so
  // this is a backstop against anything else in the phase throwing — without it a single
  // unhandled shape would surface as 「同步失败」 on a run that actually succeeded
  let hours = null;
  if (hltb) {
    try {
      hours = await syncHltb(db, hltb, { onProgress, refreshBudget: hltbRefreshBudget });
    } catch (err) {
      hours = { resolved: 0, notFound: 0, refreshed: 0, candidates: 0, stopped: err?.message ?? String(err) };
    }
  }
