/**
 * 同步引擎(原 runBatch / syncNewGames / syncAchievementSchema / rebuildSheetFromApi)
 * ------------------------------------------------
 * 和 Apps Script 版本最大的区别:**没有游标**。
 * 原版的 CURSOR + BATCH_SIZE + MAX_RUNTIME_MS 完全是为了绕开 Apps Script 单次执行 6 分钟
 * 上限——每天只能处理一部分行,第二天接着跑。本地没有这个限制,一次跑完整个库,
 * 于是也顺带消掉了原版"排序会打乱游标指向"那个坑(CLAUDE.md 里记着的)。
 */
import {
 ...
 *   这就是原版 rebuildSheetFromApi 的保留规则(判断依据是 owned 与否,不是 Status)
 */
 ...
 * sync_locked 的行跳过——对应原版 Status === 'Manual' 就跳过的行为。
 */
 ...
 * 刷新条件和原版一致:这个 appid 还没有任何记录,或者最近 7 天内成就总数变多过。
