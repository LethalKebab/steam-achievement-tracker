...
export function serve({ db, steam, config, log = console.log }) {
  ...
  async function startGuideGen(appid, overwrite = false) {
    ...
    if (guideGenState.snapshot().running) {
      const position = guideGenState.enqueue({ appid, overwrite, game: plan.game });
      log(`🕒 排队等生成:${plan.game}(${appid}),队列第 ${position} 个`);
      return { started: false, queued: true, position, game: plan.game };
    }

    return runGuideGen({ appid, overwrite, game: plan.game });
  }
   ...
   */
  async function runGuideGen({ appid, overwrite, game }) {
    let provider;
    try {
