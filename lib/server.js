...
export function createGuideGenState() {
  ...
  let queue = [];
  /**
   * 已经跑完的那些。**住在 state 外面**,和 `queue` 同一个理由,再加一条更硬的:
   * `begin()` 是 `{ ...idle }`,凡是放进 state 的东西都会在下一个任务开跑时被抹掉。
   *
   * 而「上一个跑完了」必须活过那一刻。队列里一个跑完到下一个 `begin()` 之间,只隔着
   * `drainNext()` 和 `createProvider()` 的一次动态 import(已缓存、不联网),也就是
   * 一个微任务;页面三秒轮询一次,于是 `state.result` 里那份结果**基本不可能被看到**。
   *
   * 表现:一次排队生成好几款,除了最后一个,每一个的结果都没人看见 —— 攻略链接不出现、
   * 那一行的按钮一直灰着,要刷新页面才恢复。看上去像"第一个生成完了但界面没刷新"。
   *
   * 页面按 `seq` 取增量,取过就不再看;上限只是防一次排几十个把快照撑大。
   */
  let finished = [];
  let seq = 0;
  ...
  return {
    ...
    warn(note) {
      ...
      /**
       * 同一份结果再存一份给页面按增量取。**成功和失败都要进** —— 失败那条同样要
       * 解除行上的置灰,漏了的话那一行会一直灰着;而没人会去点一个刚失败的东西,
       * 所以这种漏比成功那条更晚才被发现。
       *
       * `warnings` 一起带走:那几句("第 3 段没写出来")说的是**成品缺了什么**,
       * 下一个任务 `begin()` 一开跑就没了,而它必须跟着结果一直留在屏幕上。
       */
      finished = [...finished, {
        seq: ++seq,
        appid: state.appid,
