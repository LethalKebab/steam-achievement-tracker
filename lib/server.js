...
/** How much of a failure's own sentence fits on one line of the progress bar */
const REASON_MAX = 120;

/**
 * A degraded pass's own reason, cut to what fits beside the sentence that reports it.
 *
 * **Quoted rather than mapped to something friendlier.** Each of these names which gate rejected
 * the reply, and that is the only thing separating "the model answered badly" from "rearranging by
 * that answer would have lost a line" — one sentence covering both is what a landed guide's messy
 * sectioning had to be reasoned backwards from, weeks later, because nothing recorded the rest.
 * The kind is at the front of these messages, so a long one is cut rather than dropped.
 */
...
export function createGuideGenState() {
  const idle = {
    ...
    warnings: [],
    /**
     * **What has already happened**, oldest first, each with the time it happened.
     *
     * `note` alone could not answer the question issue #78 was actually asking. It holds one
     * sentence, the next event overwrites it seconds later, and nothing is kept — so a run that had
     * been working for twenty minutes looked exactly like one that had died ten minutes ago. A line
     * with a time on it settles that without anybody having to describe what "stuck" looks like.
     *
     * **Appended only when the sentence changes.** The same note arrives repeatedly (a shard's
     * entry count moves while the wording stays put), and a log that repeats itself is one nobody
     * reads to the end.
     */
    log: [],
    /** The run's steps, settled before it starts, and which one is current. See generationSteps */
    steps: [],
    step: 0,
    /** `{done, of}` while a shard is being written. Null the rest of the time */
    written: null,
    result: null, // {ok, path, rounds, covered, total, warnings, researched, searchQueries, usage}
  };
  /** A run of any size stays bounded; the tail is what anybody scrolls to */
  const LOG_MAX = 300;
  let state = { ...idle };
  /**
  ...
  return {
    ...
    onProgress(p) {
      state = { ...state, ...p };
      // **The log is written here rather than at each call site.** There are twenty-odd of those and
      // one of them would eventually forget; and a note that never reached `note` is one nobody saw
      if (p.note && p.note !== state.log.at(-1)?.text) {
        state = { ...state, log: [...state.log, { at: new Date().toISOString(), text: p.note }].slice(-LOG_MAX) };
      }
    },
    /**
 ...
 * say out loud.
 *
 * forbidden is the games Steam refused outright this run — a licence lost, or achievements set
 * private. Kept apart from the retry count because one means "ask later" and this one means
 * "never", and the number on those rows is frozen at the last successful read with nothing else on
 * screen saying so.
 *
 * familyAdded is the games this run found you playing from the family library and added by itself.
 * **It is the only row the sync creates without being asked**, and a row appearing among three
 * hundred others is not something anybody notices, so like bumped it has to be said out loud.
 *
...
export function createSyncState() {
  const idle = {
    ...
    error: null, at: null, bumped: [], ticked: [], tickError: null, familyAdded: [], forbidden: [],
  ...
  return {
    ...
    begin(phase = 'library') {
      ...
      bumped = [], ticked = [], tickError = null, familyAdded = [], forbidden = [],
      statusDone = [], statusStaged = [], statusError = null,
    } = {}) {
      state = {
        ...state,
...
export function serve({ db, steam, config, log = console.log }) {
  ...
  async function runGuideGen({ appid, overwrite, effort, scope = null, game }) {
    ...
    /**
     * The progress both pipelines report in the same terms: how many steps the run has, which one
     * it is on, and how many entries of the current request have been written.
     *
     * **Shared rather than written twice.** The two `onProgress` handlers below started as separate
     * copies, and the partial rewrite went a release reporting none of this — not because it could
     * not, but because the lines were added to the other copy. Returns true when the event needs
     * nothing further; each handler's own chain is for what genuinely differs between them.
     */
    const reportProgress = (ev) => {
      // **The step list is taken before anything runs**, so the interface can draw the whole run
      // and mark it off rather than growing a list while somebody watches
      if (ev.phase === 'plan' && ev.steps) guideGenState.onProgress({ steps: ev.steps, step: 0 });
      if (ev.phase === 'step') {
        guideGenState.onProgress({ step: ev.n });
        return true;
      }
      if (ev.phase === 'written') {
        // **No note.** This fires once per entry, and a log line per entry would bury the handful
        // of lines that say something. It has its own slot, read live and never kept
        guideGenState.onProgress({ written: { done: ev.done, of: ev.of } });
        return true;
      }
      return false;
    };
    ...
    const job = scope
      ...
      onProgress(ev) {
        ...
        if (ev.phase === 'plan' && ev.chunks > 1) {
        ...
        } else if (ev.phase === 'regroup-fill') {
          guideGenState.onProgress({ phase: 'plan', note: msg('gp.regroupFill', { n: ev.missing }) });
        } else if (ev.phase === 'regroup-filled') {
          guideGenState.onProgress({ phase: 'plan', note: msg('gp.regroupFilled', { n: ev.filled, of: ev.of }) });
        } else if (ev.phase === 'regroup-fill-failed') {
          // A note, not a warn: every one of those achievements already has a home, so the
          // consequence is a few entries staying put — and `regroup-done` reports the count
          guideGenState.onProgress({ phase: 'plan', note: msg('gp.regroupFillFailed', { reason: shortReason(ev.reason) }) });
        } else if (ev.phase === 'regroup-retry') {
          guideGenState.onProgress({ phase: 'plan', note: msg('gp.regroupRetry') });
          ...
          //
          // **Carrying the reason is the whole diagnosis.** Five different failures produce this one
          // sentence, and dropping `ev.reason` left the finished card saying a pass failed with
          // nothing anywhere recording which one — a guide landed with its shards' own headings and
          // the only way to find out why was to reason backwards from the page weeks later
          guideGenState.warn({ key: 'gp.regroupFailed', values: { reason: shortReason(ev.reason) } });
        } else if (ev.phase === 'merged-sections') {
          guideGenState.onProgress({ phase: 'plan', note: msg('gp.mergedSections', { n: ev.folded }) });
        } else if (ev.phase === 'merge-failed') {
          guideGenState.warn({ key: 'gp.mergeFailed', values: { reason: shortReason(ev.reason) } });
    ...
    job
      ...
      .then((r) => {
        ...
        if (scope) {
          ...
          // — that sentence has to reach the log, or 「未通过校验」 reads as though the guide was
  ...
  function startBackgroundSync({ guideAppids = [] } = {}) {
    ...
    // No client when the reader has turned it off, and `fullSync` skips the phase on a null —
    // the flag is expressed by what is passed in, so there is no second place to check it
    const hltb = config.hltbEnabled === false ? null : new HltbClient(config, { log });
    fullSync(db, steam, { onProgress: (p) => syncState.onProgress(p), selection, hltb, hltbRefreshBudget: config.hltbRefreshBudget })
      .then(async (r) => {
        ...
        syncState.end(null, {
          ...
          forbidden: r.stats.forbidden,
          familyAdded: r.library.familyAdded,
          ticked: tick.ticked,
