...
export function serve({ db, steam, config, log = console.log }) {
  ...
  const api = createApi({
    ...
    startGuideGen: (appid, overwrite, effort, scope, spoilerFold) => startGuideGen(appid, overwrite, effort, scope, spoilerFold),
   ...
   */
  async function startGuideGen(appid, overwrite = false, effort = null, scope = null, spoilerFold = false) {
    ...
    try {
      return await startGuideGenClaimed(appid, overwrite, effort, scope, spoilerFold);
    } finally {
  ...
  async function startGuideGenClaimed(appid, overwrite, effort, scope, spoilerFold = false) {
    let plan;
    try {
    ...
    if (!guideGenState.reserveRun()) {
      const position = guideGenState.enqueue({ appid, overwrite, effort, scope, spoilerFold, game: plan.game });
      log(clog('srv.queued', { game: plan.game, appid, n: position }));
      return { started: false, queued: true, position, game: plan.game };
    }

    return runGuideGen({ appid, overwrite, effort, scope, spoilerFold, game: plan.game });
  }
   ...
   */
  async function runGuideGen({ appid, overwrite, effort, scope = null, spoilerFold = false, game }) {
    ...
    const job = scope
      ...
      onProgress(ev) {
        ...
        } else if (ev.phase === 'spoiler') {
          // Another pass that takes tens of seconds; without it the floater sits still throughout
          guideGenState.onProgress({ phase: 'plan', note: msg('gp.spoiler') });
        } else if (ev.phase === 'spoiler-done') {
          guideGenState.onProgress({ phase: 'plan', note: msg('gp.spoilerDone', { n: ev.folded }) });
        } else if (ev.phase === 'spoiler-failed') {
          // Through warn, not note: the consequence stays in the finished guide — the spoilers are
          // written out in the open — while a note is painted over three seconds later
          guideGenState.warn({ key: 'gp.spoilerFailed' });
