...
export function createGuideGenState() {
  ...
  return {
    ...
    /** Accumulates one notice that has to survive the run. `onProgress` has merge semantics, so appending needs its own entrance */
    warn(note) {
...
export function serve({ db, steam, config, log = console.log }) {
  ...
  async function runCheckboxSync(appids) {
    ...
    try {
      ...
      log('⚠️  自动勾选失败(成就数据已同步完成,不受影响):' + (err.message ?? err));
  ...
  async function startGuideGen(appid, overwrite = false, effort = null, scope = null) {
    if (!config.ai?.apiKey) return { started: false, error: '还没配置 AI —— 去设置页填写供应商和密钥' };
    ...
    if (!guideGenState.claim(appid)) {
      return { started: false, error: '这款游戏已经在生成或排队了' };
    }
    try {
    ...
    const job = scope
      ? patchGuide(db, {
        ...
        onProgress(ev) {
          if (ev.phase === 'plan') {
            ...
            if (ev.extra) guideGenState.warn(`模型多写了 ${ev.extra} 条没要求的,已忽略`);
            guideGenState.onProgress({ phase: 'check', note: `交回 ${ev.wrote}/${ev.of} 条,校验` });
          } else if (ev.phase === 'lint') {
            guideGenState.onProgress({ phase: 'lint', note: `本次要改 ${ev.caused} 条,原有问题 ${ev.preExisting} 条` });
      ...
      onProgress(ev) {
        ...
        if (ev.phase === 'plan' && ev.chunks > 1) {
          ...
          guideGenState.warn('分区统一失败,同类成就可能散在几个小节里');
          ...
          guideGenState.onProgress({ phase: 'ask', note: `第 ${ev.chunk} 段太长,拆成 ${ev.to} 个成就重问` });
        } else if (ev.phase === 'chunk-failed') {
          guideGenState.warn(`第 ${ev.chunk} 段(${ev.count} 个成就)未生成,先写了后面的`);
        } else if (ev.phase === 'tool') guideGenState.onProgress({ phase: 'tool', note: ev.name });
    ...
    job
      .then((r) => {
        ...
        for (const c of r.chunkFailures ?? []) {
          log(`⚠️ 第 ${c.chunk}/${c.of} 段(${c.count} 个成就)未生成:${c.reason.split('\n')[0]}`);
        }
        if (scope) {
  ...
  function startBackgroundSync({ guideAppids = [] } = {}) {
    if (syncState.snapshot().running) return { started: false, error: '同步正在进行' };
    if (!config.steamApiKey || !config.steamId) {
