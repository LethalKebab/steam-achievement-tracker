    ...
    const job = scope
      ...
      onProgress(ev) {
        ...
        if (ev.phase === 'plan' && ev.chunks > 1) {
          ...
          guideGenState.onProgress({ phase: 'plan', note: `${ev.clusters} 组同类成就并回了一处` });
        } else if (ev.phase === 'unwrapped-toggles') {
          guideGenState.onProgress({ phase: 'plan', note: `摊开了 ${ev.titles.length} 个装着成就的折叠` });
        } else if (ev.phase === 'unwrap-failed') {
          // 同上:后果留在成品上(那一节点开还是空的),不能让它三秒后被盖掉
          guideGenState.warn('装着成就的折叠没能摊开 —— 那一节在页面上会显示 0 条');
