...
export function serve({ db, steam, config, log = console.log }) {
  ...
  const api = createApi({
    ...
    startGuideGen: (appid, overwrite) => startGuideGen(appid, overwrite),
   ...
   */
  async function startGuideGen(appid, overwrite = false) {
    ...
    generateGuide(db, {
      config, provider, steam, appid, rounds, overwrite, notion: new NotionClient(config),
      onProgress(ev) {
   ...
   * 措辞直接复用 `formatPreflight`,两处不会各说一套。
   */
  async function planGuidePreflight(appid, { overwrite = false } = {}) {
    const { planGuide } = await import('./guidegen.js');
    const { overwritePreflight, formatPreflight, BACKUPS_DIR } = await import('./guidebackup.js');
    ...
    return {
      ...
      summary: formatPreflight(pre, { defsCount: plan.defs.length }),
