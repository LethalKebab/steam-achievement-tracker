...
export function serve({ db, steam, config, log = console.log }) {
  ...
  async function startGuideGen(appid) {
    ...
    generateGuide(db, {
      config, provider, steam, appid, rounds, notion: new NotionClient(config),
      onProgress(ev) {
        ...
        if (ev.phase === 'plan' && ev.chunks > 1) {
          guideGenState.onProgress({ phase: 'plan', note: `${ev.achievements} 个成就,分 ${ev.chunks} 段写` });
        } else if (ev.phase === 'ask') {
          guideGenState.onProgress({ phase: 'ask', round: ev.round, note: `${seg}联网研究 + 撰写` });
        } else if (ev.phase === 'rewrite') {
          guideGenState.onProgress({ phase: 'rewrite', note: `重写其中 ${ev.chunks}/${ev.of} 段` });
  ...
  async function planGuidePreflight(appid) {
    ...
    const plan = await planGuide(db, { config, steam, appid, notion: new NotionClient(config) });
    return { game: plan.game, count: plan.defs.length, target: plan.target };
  }
