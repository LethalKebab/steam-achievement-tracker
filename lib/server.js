  ...
  async function runGuideGen({ appid, overwrite, effort, scope = null, game }) {
    ...
    const job = scope
      ? patchGuide(db, {
        ...
        onProgress(ev) {
          if (ev.phase === 'plan') {
            ...
            if (ev.missing) guideGenState.warn(msg('gp.missing', { n: ev.missing }));
            if (ev.extra) guideGenState.warn(msg('gp.extra', { n: ev.extra }));
      ...
      onProgress(ev) {
        ...
        if (ev.phase === 'plan' && ev.chunks > 1) {
          ...
          guideGenState.warn(msg('gp.regroupFailed'));
          ...
          guideGenState.warn(msg('gp.unwrapFailed'));
          ...
          guideGenState.warn(msg('gp.chunkFailed', { chunk: ev.chunk, count: ev.count }));
        } else if (ev.phase === 'tool') guideGenState.onProgress({ phase: 'tool', note: ev.name });
    ...
    job
      ...
      .catch((err) => {
        ...
        log(msg(scope ? 'srv.patchError' : 'srv.genError', { reason: err.stack ?? err }));
        drainNext();
      });
