...
import { patchGuide, planPatch, patchPresets, pickableEntries, PATCH_ROUNDS } from './guidepatch.js';
import { blocksToOutline } from './notionblocks.js';
import { groupBySection } from './guidescope.js';
...
export function serve({ db, steam, config, log = console.log }) {
  ...
  async function previewGuidePatch(appid) {
    ...
    const { plan, baseline, oldText, kind } = await planPatch(db, {
      ...
      // 只是要拿 plan 和基线校验,`all` 一定解析得出来,不会因为选择器本身被拒
      selector: 'all',
    });
    ...
    return {
      ...
      presets: patchPresets({ plan, baseline, oldText }),
      // 挑几条用的清单:按小节分组,每条带名字、稀有度、解锁状态
