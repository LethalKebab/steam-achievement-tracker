...
import { getMeta, recordAiUsage, aiUsageSince } from './db.js';
...
import { generateGuide, checkDailyBudget } from './guidegen.js';
...
export function serve({ db, steam, config, log = console.log }) {
  ...
  const api = createApi({
    ...
    startGuideGen: (appid) => startGuideGen(appid),
  });
   ...
   */
  async function startGuideGen(appid) {
    if (guideGenState.snapshot().running) return { started: false, error: '已经有一个攻略在生成了' };
    if (!config.ai?.apiKey) return { started: false, error: '还没配置 AI —— 去设置页填一个供应商和 key' };

    // 累计上限:per-run 拦"这一次跑飞了",这条拦"点了二十次"
    const daily = checkDailyBudget(db, config, { aiUsageSince });
    if (daily.over) return { started: false, error: daily.reason };

    let provider;
    let plan;
    try {
      provider = await createProvider(config);
      plan = await planGuidePreflight(appid);
    } catch (err) {
      return { started: false, error: String(err.message ?? err) };
    }

    const rounds = config.ai.maxRounds ?? 3;
    guideGenState.begin(appid, plan.game, rounds);
    log(`🤖 开始生成攻略:${plan.game}(${appid})`);

    generateGuide(db, {
      config, provider, steam, appid, rounds,
      onProgress(ev) {
        if (ev.phase === 'ask') guideGenState.onProgress({ phase: 'ask', round: ev.round, note: '联网研究 + 撰写' });
        else if (ev.phase === 'tool') guideGenState.onProgress({ phase: 'tool', note: ev.name });
        else if (ev.phase === 'check') guideGenState.onProgress({ phase: 'check', note: '机械打勾 + 校验' });
        else if (ev.phase === 'lint') {
          guideGenState.onProgress({ phase: 'lint', note: `勾上 ${ev.ticked} 个框,还剩 ${ev.blocking} 条要改` });
        }
      },
    })
      .then((r) => {
        // **不管成功失败都记账** —— 失败那次一样扣了额度,不记的话上限会被
        // "失败不算钱"这个错觉架空
        recordAiUsage(db, {
        ...
        log(r.ok ? `✅ 攻略写完:${r.path}` : `⚠️ 攻略没过校验,草稿留在 ${r.draftPath}`);
      })
      .catch((err) => {
    ...
    return { started: true, game: plan.game, achievements: plan.count };
  }

  /** 起跑前的检查。planGuide 自己会把所有拒绝理由一次性抛出来,这里只是借它跑一遍 */
  async function planGuidePreflight(appid) {
    const { planGuide } = await import('./guidegen.js');
    const plan = await planGuide(db, { config, steam, appid });
    return { game: plan.game, count: plan.defs.length };
  }
