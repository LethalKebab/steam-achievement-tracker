...
/**
 * The depth the aside runs at, **pinned rather than inherited**.
 *
 * `ai.effort` is chosen for writing a guide, and this pass does not write: it picks out sentences
 * somebody else already wrote. Measured on the same 16-entry guide, byte-identical answer both
 * times: inheriting the run's `high` cost **24,594** output tokens, `low` cost **2,636**. Against
 * the 40,615 the writing itself spent, that is the difference between the pass costing 60% extra
 * and costing 6%.
 *
 * It is `low` and not `off`: `off` stops the field being sent at all, which on this endpoint is
 * measured at 337 s and 145,955 characters of thinking — the uncapped case, i.e. the opposite of
 * what is wanted here.
 */
