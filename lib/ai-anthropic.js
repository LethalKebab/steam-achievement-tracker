 ...
 */
/**
 * Wire tool name → the vocabulary the rest of the project speaks, the same words `RESULT_KINDS`
 * gives results. **Matched by prefix**, because the names carry a dated suffix
 * (`web_search_20260209`) that moves with every tool revision, and pinning the whole string means
 * the next revision silently falls through to the raw name again.
 */
export function toolLabel(name) {
  const n = String(name ?? '');
  if (n.startsWith('web_search')) return 'search';
  if (n.startsWith('web_fetch')) return 'fetch';
  // **Not a tool we declare, and it still reaches the screen.** The `_20260209` web tools run code
  // internally to filter results, so this arrives on every search — observed on the official
  // endpoint, twice per query. Left unmapped it put the English constant `code_execution` on the
  // progress bar, which is the same defect as the wire names one step along
  if (n.startsWith('code_execution')) return 'filter';
  return n;
}
  ...
  return {
    ...
    push(ev) {
      switch (ev?.type) {
        ...
        case 'error': {
          const e = ev.error ?? {};
          throw new AiError(msg('ai.anthStreamBroke', {
            type: e.type ?? 'error', reason: e.message ?? '',
          }), {
            type: e.type ?? null,
...
export class AnthropicProvider {
  ...
  constructor(ai, { fetchImpl = globalThis.fetch, log = () => {} } = {}) {
    if (!ai?.apiKey) {
      throw new AiError(
        msg('ai.noKeyConfigured', { vendor: 'Anthropic', envVar: 'ANTHROPIC_API_KEY' })
      );
    }
   ...
   * while SKILL.md rule-7 explicitly requires that a guide's prose not state where its data came from.
   */
  webTools() {
    const ai = this.ai;
  ...
  /**
   * `effort` overrides `this.effort` **for this request only**, and exists for the aside passes.
   *
   * They inherit a depth setting chosen for *writing a guide*, and they do not write: the spoiler
   * pass classifies sentences somebody else already wrote. Measured on the same 16-entry guide,
   * same answer both times: inheriting `high` cost **24,594** output tokens, `low` cost **2,636**.
   * Nine times the price for the identical result.
   *
   * It is an override rather than a second provider because depth is the only thing that differs —
   * a second provider would also have to re-resolve the key, the model and the endpoint capability
   * table, which is three more chances for the aside to disagree with the run it belongs to.
   * **A falsy value means "no override", never "effort off"**; that would silently un-cap the very
   * calls this exists to cap.
   */
  buildBody({ system, messages, tools, effort = null }) {
    const body = {
      model: this.model,
    ...
    const useEffort = effort || this.effort;
    if (useEffort) body.output_config = { effort: useEffort };
  ...
  async #once(body, { onEvent, signal: externalSignal } = {}) {
    ...
    // **The external signal and the idle timeout share one AbortController.** Either cause tears
    // down the same fetch, and the catch block below tells them apart by asking the external
    // signal itself — not by which one fired first, which a listener cannot observe reliably.
    ...
    try {
      ...
      for await (const ev of sseEvents(res.body)) {
        // **Accumulate first, then report.** The other order is why a search could only ever be
        // announced as its tool name: a `server_tool_use` block's `input` arrives as streaming
        // JSON fragments and is not assembled until `content_block_stop`, so a reporter running
        // ahead of the accumulator has nothing but the name to say.
        acc.push(ev);
        emitProgress(ev, onEvent, acc.content);
      }
      ...
      if (err?.name === 'AbortError') {
        // **Which of the two abort sources fired is not "whoever we catch first"** — both go
        // through the same ac.signal, so the only reliable question is "did the caller's own
        // signal ask for this". Reporting a user-requested cancel as "took too long" would send
        // someone hunting for a slow network that was never the cause.
        if (externalSignal?.aborted) {
          throw new AiError(msg('ai.cancelled'), { retryable: false, cancelled: true });
        }
        // **States only what happened** — this sentence renders verbatim in the Dashboard's
        // floating bar. The terminal-specific fix (which config field, or the settings-page field
        // now that one exists) is added by tracker.js catching `ai-timeout`
        const timeoutErr = new AiError(
          msg('ai.timedOut', { seconds: Math.round(this.timeoutMs / 1000) })
            + msg('ai.timedOutSlowNote'),
          { retryable: false }
        );
   ...
   */
  async send({ system, messages, tools, onEvent, signal, effort = null } = {}) {
    ...
    for (;;) {
      last = await this.#withRetry(this.buildBody({ system, messages: convo, tools, effort }), { onEvent, signal });
      addUsage(usage, last.usage);
      if (last.stopReason !== 'pause_turn') break;
      if (continuations >= this.maxContinuations) {
        throw new AiError(
          msg('ai.anthContinuations', { n: continuations }),
          { retryable: false }
        );
      }
 ...
 * `.raw` for reference.
 */
function emitProgress(ev, onEvent, blocks = []) {
  if (!onEvent) return;
  if (ev.type === 'content_block_stop') {
    /**
     * **The query, the moment it is complete.** This is the whole reason the accumulator runs
     * first: at `content_block_start` the input is an empty scratch string, and at `stop` it has
     * been assembled and parsed. Without it the longest phase of a run — minutes of web research
     * on a large game — had one unchanging word on screen, which is indistinguishable from being
     * stuck (issue #78).
     *
     * Only search carries a query worth reading. A fetch's URL is already implied by the search
     * that surfaced it, and reporting both makes the line change twice as often while saying less.
     */
    const b = blocks[ev.index];
    if (b?.type === 'server_tool_use' && toolLabel(b.name) === 'search') {
      const q = b.input?.query;
      if (q) onEvent({ type: 'search', query: String(q), raw: ev });
    }
    return;
  }
  if (ev.type === 'content_block_start') {
    ...
    if (b.type === 'server_tool_use') {
      // **The vocabulary, not the wire name.** `web_search_20260209` reaching the progress bar is
      // the same defect as the missing query, one step earlier: it says nothing and it dates
      onEvent({ type: 'tool', name: toolLabel(b.name), raw: ev });
...
async function errorFromResponse(res, providerName = 'anthropic', envVar = 'ANTHROPIC_API_KEY') {
  ...
  if (res.status === 401) {
    ...
    hint = msg('ai.anthEffortRejected');
  } else if (res.status === 400) {
    hint = msg('ai.anthBadRequest');
  }

  return new AiError(msg('ai.httpError', {
    vendor: providerName, status: res.status, type: type ? ` ${type}` : '', detail, hint,
  }), {
    status: res.status,
    type,
    requestId,
