...
export class AnthropicProvider {
  ...
  constructor(ai, { fetchImpl = globalThis.fetch, log = () => {} } = {}) {
    ...
    // 打官方端点才发 Anthropic 专有扩展。**注意这不再是一个总开关** —— 见下面三行。
    this.extras = ai.anthropicExtras ?? !ai.baseUrl;

    // -----------------------------------------------------------------------
    // 推理深浅:**和 thinking 分开发,这两件事的命运不一样**
    //
    // 以前 `thinking` / `output_config` / `fallbacks` 捆在 `extras` 一个开关上,而
    // `extras` 问的是"这是不是官方端点"—— 那是关于**端点身份**的判断,拿来回答
    // "这个字段它认不认"是两个问题。后果:`provider: "deepseek"` 预设总会设 baseUrl,
    // 于是 `ai.effort`(config.js 里注释写着「深浅旋钮」、默认 'high')**从来没被发出去过**。
    //
    // 实测 2026-08-17,DeepSeek 的 /anthropic,同一段 10 个成就带联网工具:
    //
