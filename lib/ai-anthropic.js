...
export class AnthropicProvider {
  ...
  buildBody({ system, messages, tools, effort = null }) {
    ...
    /**
     * **A per-call override, for a pass whose depth is not the run's depth.** The classification
     * pass writes no prose — it assigns numbers to section names over prose that is already
     * written — and inheriting a run's `high` had it spend the whole `maxTokens` budget thinking
     * and get truncated before the answer (measured: 31,998 of 32,000 on a 162-achievement guide,
     * twice in a row, losing the whole sectioning each time).
     *
     * **It only replaces a value that would have been sent anyway.** Where the endpoint does not
     * take `output_config`, `this.effort` is null and an aside must not become the one request
     * that sends it — that is a 400 on a request the writing rounds survive.
     */
    const useEffort = this.effort ? (effort || this.effort) : null;
