 ...
 *
 * 这里一度对两个工具都用 `Array.isArray` 判成功 —— 那条规则**只对 web_search 成立**。
 * 于是每一次**成功**抓页都被记成一条错误(错误码正是成功块自己的 type,`web_fetch_result`),
 * 而 `checkResult` 见到任何一条工具错误就判整轮不可用:抓页成功 == 生成失败。
 * 之所以拖到用户手上才炸,是因为 web_fetch 只在官方端点默认开启(见 `webTools`),
 * 而此前跑通的实盘全在 DeepSeek 兼容端点上 —— 那里它默认关着。
 *
...
export class AnthropicProvider {
  ...
  constructor(ai, { fetchImpl = globalThis.fetch, log = () => {} } = {}) {
    ...
    // 打官方端点才发 Anthropic 专有扩展(thinking / output_config / fallbacks)。
    // 兼容端点默认不发 —— 它们多半不认,发过去就是一个指错方向的 400
  ...
  buildBody({ system, messages, tools }) {
    ...
    // **Anthropic 专有的扩展,只在打官方端点时才发。**
    // 兼容端点(比如 DeepSeek 的 /anthropic)一般只实现 Messages API 的核心部分,
    // 多发一个它不认的字段就是整个请求 400 —— 而这类 400 的报错通常只说"字段无效",
    // 指不到"你在拿官方专有参数打第三方端点"这个真因上。
    // 要在兼容端点上试这些,把 ai.anthropicExtras 显式设成 true。
    if (this.extras) {
      // thinking 显式写出来:Opus 5 省略等于 adaptive,但 4.8/4.7 省略等于**不思考**,
      // 而 model 是可配置的。显式比"靠默认值恰好对"可靠
      body.thinking = this.ai.showThinking
        ? { type: 'adaptive', display: 'summarized' }
        : { type: 'adaptive' };
      body.output_config = { effort: this.ai.effort ?? 'high' };
