...
const DEFAULTS = {
  ...
  ai: {
    ...
    maxTokens: 32000,
    // 超过这么多成就就拒绝生成:一次上下文装不下,质量会掉。100+ 的分片编排
    // 是独立的一块工程(SKILL.md 8.4),不在 v1
    maxAchievements: 100,
    ...
    // --- 花费上限 ---
    // **token 上限永远有效,美元上限只在算得出金额时才有效。** 这不是偷懒:
    // DeepSeek / Gemini 没有内置价格表(单价会变、也没核实过),纯美元上限对它们
    // 形同虚设 —— 设了却永远不触发,是个假装设了防的开关。token 每家都测得准。
    // 想让美元上限对它们生效,把单价填进下面的 pricing。
    maxTokensPerRun: 400000,   // 单次生成的 token 上限(轮与轮之间检查,超了当场停)
    maxSpendPerRunUsd: 0,      // 0 = 不限。有价格表时才起作用
    maxTokensPerDay: 0,        // 0 = 不限。跨次累计,起跑前检查
    maxSpendPerDayUsd: 0,      // 0 = 不限
    // 每百万 token 的单价,填了就按它算钱(内置价格表之外的供应商靠这个)。
    // 例:{ "input": 0.28, "output": 0.42 } —— 从你自己的账单上抄,别猜
    pricing: null,
    maxSearches: 30,
    maxFetches: 10,
...
const HINTS = {
  ...
  ai:
    ...
    '  ANTHROPIC_API_KEY=...  (provider 为 anthropic,按量付费,有联网搜索)\n' +
    '  GEMINI_API_KEY=...     (provider 为 gemini,有免费额度,有联网搜索)\n' +
    '  DEEPSEEK_API_KEY=...   (provider 为 deepseek,**没有联网**,只适合验证流水线)\n' +
