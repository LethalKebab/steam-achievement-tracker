...
const DEFAULTS = {
  ...
  ai: {
    ...
    //   anthropic / gemini — have server-side web search and can write researched guides
    ...
    //   }
    //
    // There are three vendors and these three fields originally had one slot each, so "try a
    ...
    // level; `geminiTools` / `webFetch` / `searchTool` / `anthropicExtras` are read by exactly one
    // vendor, so a leftover value is ignored rather than misapplied, and they stay outer too.
    //
    // Aliases collapse: google → gemini, deepseek-openai → deepseek (the same vendor's other
    // endpoint, the same configuration). The resolution order is in resolveAiKey
    providers: {},
    ...
    //
    // The environment variables ANTHROPIC_API_KEY / GEMINI_API_KEY / DEEPSEEK_API_KEY work too,
    ...
    // here: model names are vendor-specific (claude-* / gemini-* / deepseek-*), and putting an
    ...
    maxRounds: 3,
    // **Fold spoilers away after the guide is written** (lib/guidespoiler.js). One extra request
    // per generation, pinned to `low` effort because it classifies rather than writes: measured
    // 2,636 output tokens against the 40,615 the writing itself spent, so roughly 6% on top.
    // Default on at that price; `false` here, or `--no-spoiler` for one run, skips it entirely
    spoilerFold: true,
    ...
    // --- The settings below apply to gemini only ---
    // The declared names of the web tools. They are configurable because this vendor was integrated
    // without access to the official documentation: a renamed tool, or one the free tier will not
    // allow, is a change here rather than a code change.
    // For "fetch the full page" (the equivalent of Anthropic's web_fetch), add 'url_context' — but
    // declaring one more tool we are unsure of 400s the whole request, so get it working first
    geminiTools: ['google_search'],
    // The thinking budget. **Unset means the field is not sent at all**, letting the model use its
    // own default — sending a field that may not be accepted is more error-prone than not sending it
 ...
 *
 * `google` is an alias for `gemini` (as `createProvider` also treats it), and `deepseek-openai` is
 * DeepSeek's other endpoint — **a different endpoint with the same key**, so two slots would only
 * mean pasting it twice.
 *
 ...
 * replaced with that vendor's own: carrying the previous vendor's model name (claude-* / gemini-* /
 * deepseek-*) across trips `assertModelMatchesProvider`, while **clearing it unconditionally has its
 * own cost** — a version pinned for Anthropic disappears after switching to Gemini and back, without
...
export function loadConfig({ required = [] } = {}) {
  ...
  // "AI_PROVIDER=gemini is set and it is still looking for ANTHROPIC_API_KEY".
