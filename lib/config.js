...
const DEFAULTS = {
  ...
  ai: {
    provider: 'anthropic', // 目前只有 anthropic;OpenAI 是「动手顺序」第 4 步
    apiKey: '', // 也可以用环境变量 ANTHROPIC_API_KEY
    model: 'claude-opus-5',
...
export function loadConfig({ required = [] } = {}) {
  ...
  if (process.env.ANTHROPIC_API_KEY) cfg.ai.apiKey = process.env.ANTHROPIC_API_KEY;
...
const HINTS = {
  ...
  ai:
    ...
    '  ANTHROPIC_API_KEY=<https://platform.claude.com/settings/keys 拿到的 key>\n' +
    '这是花钱的功能——先跑 `node tracker.js ai-check --dry` 看清楚会发出去什么再说。',
};
