...
const DEFAULTS = {
  ...
  // 官方 Web API(api.steampowered.com)两次调用之间的间隔。同步的绝大多数请求走这里。
  // 实测过一次:GetPlayerAchievements 连打 400 个、间隔 0ms、11 次/秒持续 36 秒,
  // 一个 429 都没有,所以 100ms 已经留了一半余量。被限流(429)多的话往上调
  requestDelayMs: 100,
  // 商店接口(store.steampowered.com:appdetails / 商店页 HTML / 搜索)的间隔。
  // **和上面分开,因为这条路严得多,而且撞上去是 IP 级封禁,不是 key 级限流。**
  // 这一条没有实测数据 —— 为了一个数字去高频刷商店页,风险和收益不成比例。
  // 只在反查游戏名、找封面、搜索的时候走到,次数远少于 Web API,所以慢一点不影响同步时长
...
const HINTS = {
  ...
  notion:
    // 「Internal Integration secret」是概念名,Notion 界面上没有这几个字。照抄控件上的原字:
    // 按钮叫 New connection,密钥叫 Access token、在它的 Configuration 标签页里。
    // Notion 这几个名字改过不止一轮(以前是 New integration / Internal Integration Secret,
    // 页面在 notion.so/my-integrations),所以照抄的是当下这一版。走查在 docs/notion-setup.md
