...
const DEFAULTS = {
  ...
  ai: {
    // 'anthropic' | 'gemini'。gemini 有免费额度,适合先把整条流水线跑通;
    // anthropic 是按量付费,没有免费层
    provider: 'anthropic',
    // 环境变量 ANTHROPIC_API_KEY / GEMINI_API_KEY 也能填(按 provider 取对应那个)
    apiKey: '',
    // 换 provider 记得一起换 model。anthropic 用 claude-*,gemini 用 gemini-*
    // (留空的话各家用各自的默认值)。不确定有哪些可用就跑
    // `node tracker.js ai-check --models` —— **别照着记忆猜模型名**,
    // 实测过一次:默认值写死三个月就既过时、又撞上"这个模型不在免费层"
    model: 'claude-opus-5',
...
export function loadConfig({ required = [] } = {}) {
  ...
  const AI_KEY_ENV = { anthropic: 'ANTHROPIC_API_KEY', gemini: 'GEMINI_API_KEY', google: 'GEMINI_API_KEY' };
...
const HINTS = {
  ...
  ai:
    ...
    '  ANTHROPIC_API_KEY=...  (provider 为 anthropic,按量付费,没有免费层)\n' +
    '  GEMINI_API_KEY=...     (provider 为 gemini,有免费额度:https://aistudio.google.com/apikey)\n' +
