...
const DEFAULTS = {
  ...
  ai: {
    ...
    //
    // 这一条以前是个死旋钮:它跟着 `anthropicExtras` 一起发,而那个开关在设了 baseUrl
    // 时恒为 false,`provider: "deepseek"` 预设又总会设 baseUrl —— 于是它从来没被发出去过。
    //
    // 实测 2026-08-17(DeepSeek /anthropic,同一段 10 个成就,带联网工具):
    //
    //   什么都不发   337 s  搜 8 次  255 字/成就   ← 死旋钮时期跑的就是这个
    ...
    maxRounds: 3,
    // **第一轮同时写几段。** 各段内容互不相交,本来就没有先后依赖 —— 以前排队
    // 纯粹是因为全篇共用一个会话(一个会话就是一条链)。改成每段一条链之后,
    // 四段游戏的第一轮从"四段之和"变成"最慢那一段"。
    //
