...
import { stripGuideEcho, mapAchievementGuides, normalizeText, resolveTodoToAchievement } from './guides.js';
...
/**
 * 「没写打法」的默认线:去掉成就名和官方描述之后,剩下的打法不足 40 个字。
 *
  ...
  if (SELECTOR_KINDS.has(kind)) {
    ...
    if (kind === 'all') {
      ...
      for (const d of defs) {
        ...
        // 攻略里定位不到的成就不算"没写打法" —— 它是**根本没写**,那是缺 checkbox,
