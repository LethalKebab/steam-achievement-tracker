 ...
 * 「稀有」的默认线:全球解锁率 10% 以下。
 *
 * 和提示词里 `rarityTag` 的分档对齐(🔴 <5% / 🟠 <10%),**刻意共用同一条线**:
 * 提示词说「🔴🟠 这几条是攻略真正的价值所在」,那么「只重写值钱的那几条」就该
 * 正好选中同一批。两处各定一个阈值,用户读到的分档和实际选中的集合就会错开。
 *
 * **10%,不能放宽到 15%。** 15% 太松:51 条的游戏里能有 27 条落在线内 —— 一半以上
 * 都叫「稀有」,那这个词就不指任何东西了,而按它分配力气等于没分配。
 *
 * `rarityTag` 的中间档 **import 这个常量**,不许各写各的:两处各留一个数就会出现
 * 「界面说它稀有、提示词说它不稀有」。CLI 的帮助文本和 Dashboard 的兜底值还是各写各的字面量
 * (一个是 help 字符串,一个是服务端没下发时的 fallback),由测试钉住它们相等。
 */
 ...
 *
 * **这张表和 Dashboard 上那一排范围按钮是同一套东西**,不多不少。`all` / `thin` /
 * `unlocked` 三个是**刻意不在**的,理由各不相同但结果一样:界面上给不出它们,
 * 于是「这个功能能做什么」会在两个地方有两个答案,而文档只能挑一个说。
 *
 *   - `all`      —— 整篇重写有它自己的入口(`--overwrite`),这是第二条几乎一样的路
 *   - `thin`     —— 判据说不清楚,做不成按钮(见 guidepatch.js 里那段)
 *   - `unlocked` —— 「重写我已经打过的成就」不是任何人提过的需求,`locked` 才是
 *   - `failing`  —— 「当前校验没过、且重写能修的」。真实语料里这个集合几乎永远是空的,
 *                   而一个常年显示 0 的选项要人每次判断一次"这 0 是什么意思"。
 *                   真需要时 `guide-lint <appid>` 逐条列得更清楚,看完再点名
 *
 * 再加选择器之前先问一句:**它在 Dashboard 上长什么样?** 答不上来就先别加。
 */
 ...
 *
 * @param {object}   o
 * @param {string}   o.selector  'rare[:pct]' | 'locked' | 'section:名字' |
 *                               逗号分隔的成就名/api_name
 * @param {Array}    o.defs
...
export function resolveScope({
  ...
  if (SELECTOR_KINDS.has(kind)) {
    ...
    if (kind === 'rare') {
      ...
      // 「还没打的那些」。反过来那档(重写已解锁的)没人要过,所以不提供
      ...
      if (text === null) {
        /**
         * **这条限制在这个函数里,不在 Notion 上。**
         *
         * 早先这里写的是「Notion 上的攻略读不到小节结构」,那是错的:错的是我读的那个
         * 函数。`fetchAllToDoBlocks` 只收 checkbox,标题在它眼里不存在;而
         * `fetchAllBlocks` 返回整页所有块、含标题 —— Dashboard 的挑选列表正是靠它
         * 把 Notion 攻略按小节分组的(见 `blocksToOutline`)。
         *
         * 这里之所以还是拒绝,是因为 `resolveScope` 只收到 `text`(本地全文),
         * 没有拿到块。要在 CLI 上支持 Notion 的 `section:`,得把大纲一路传进来 ——
         * 那是能做的,只是还没做。**所以消息说的是「这条路」,不是「Notion」**
         */
        const err = new Error('这条路按小节挑成就需要本地攻略全文。');
  ...
  // **先把整串当一个名字试一次,再考虑拆。** 成就名里本来就有逗号 ——
  // 全库 10134 个成就里 302 个带逗号,其中 116 个属于已经写了攻略的游戏
  // (「拔掉插头,放松身心」就是一个)。上面那行刚把中文逗号真的加进分隔符,
  // 如果不先整串试一次,这 116 个就会被切成两半、两半都匹配不上,而报出来的是
  // 「这两条攻略里找不到」—— 指不到真正的原因。整串命中比拆开命中更具体,所以它优先。
  // `api_name` 一直是退路(它不含逗号,拆开那条路本来就认得),这里省掉的是「先去查 api_name」
  ...
  // **写成 `\uFF0C` 而不是直接贴那个字符。** 原来这里是 `/[,,]/` ——
  // 看着像「半角逗号 + 全角逗号」,实际是**同一个 U+002C 写了两遍**,而旁边的注释
  // 一直说「中文逗号也认」。两个字符在等宽字体里几乎一模一样,肉眼永远查不出来;
  // 转义写法把它变成读得出的东西
...
/**
 * 把攻略的大纲按小节分组:每个小节底下有哪几个成就。
 *
 * 吃的是 `markdown.js` 的 `guideOutline` 或 `notionblocks.js` 的 `blocksToOutline`
 * 产出的同一种序列,**所以两个后端共用这一份归属逻辑**。分两份写的话,同一份攻略
 * 搬个家小节就会变,而那种差别没人看得见。
 *
 * 归属仍然走 `resolveTodoToAchievement` —— 认不出的条目直接跳过,不猜。界面上的
 * 表现是那一条不出现在挑选列表里,而它本来也不该出现:定位不到的成就局部重写
 * 够不着(见 `scopeEntries` 的 `unlocatable`)。
 *
 * 标题之前就出现的成就归进一个 `heading: null` 的组 —— 攻略开头写「机制速查」再
 * 直接列几条是常见写法,把它们丢掉等于那几条永远选不上。
 *
