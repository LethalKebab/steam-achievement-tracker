 ...
 *
 * **原来是 15%,2026-08-18 收紧到 10%。** 15% 太松:《部落幸存者》51 条里有 27 条
 * 落在线内 —— 一半以上都叫「稀有」,那这个词就不指任何东西了,而按它分配力气
 * 等于没分配。
 *
 * 收紧的同时把 `rarityTag` 的中间档改成 **import 这个常量**,而不是跟着改一遍:
 * 那次它俩差点各留一个数。CLI 的帮助文本和 Dashboard 的兜底值还是各写各的字面量
 ...
 *
 * **这张表和 Dashboard 上那一排范围按钮是同一套东西**,不多不少。曾经多出过
 * `all` / `thin` / `unlocked` 三个,理由各不相同但结果一样:界面上没有它们,
 * 于是「这个功能能做什么」在两个地方有两个答案,而文档只能挑一个说。
 *
