...
That distinction has teeth, and the matcher now acts on it. **The gate closes per name, not per achievement** (changed 2026-08-09): `safeNamesOf()` drops only the names actually in `unsafeNames`, so an achievement whose Chinese name collides is still matchable by its unique English name. Previously *either* name being unsafe disqualified the whole achievement, and a box reading exactly `Nano-Virus Master` would not match despite that string being unique.

What did **not** change: exact equality, no substring, no prefix, and the colliding name itself is still never usable — genuine both-language twins behave identically. One structural consequence: `skippedAmbiguous` is now computed after *both* passes, because pass 2 can rescue an achievement pass 1 gave up on, and reporting it as skipped would be a false alarm. Verified behaviour-preserving on live data (44 games, 0 ticks / 0 log lines before and after); six cases pinned in `test/matching.test.js`.
