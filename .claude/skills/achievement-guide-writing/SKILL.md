...
**标签行必须也是 `- [ ]`,不能写成普通 bullet(`- 前置：`)。** 这一条是机制上的硬要求,
不是风格:`todoSpans` 按「连续的、更深缩进的 checkbox 行」圈定一条成就占哪几行(见
`lib/markdown.js`),中间夹一行非 checkbox 会把范围当场截断——实测七行的结构只圈到 1 行,
局部重写把新的贴上去、旧的六行还留在原地,变成重复。
