...
## [rule-1] 规则一:每个成就必须单独占一行 checkbox
...
### [rule-1/notes-lines] 心得段:前置、步骤、警告分行写
...
### [rule-1/sub-labels] 子 checkbox 的分组标签
...
### [rule-1/nesting] 子任务/子收集品:嵌套 checkbox 要满足三个条件
...
### [rule-1/duplicate-names] 重名成就的处理
...
## [rule-2] 规则二:截图只贴真正有信息量的

### [rule-2/worth-shots] 值得贴的截图
- 地图位置/路线标注
...
### [rule-2/video-instead] 贴不了截图的时候:带时间点的视频链接
...
### [rule-2/no-shots] 不要贴的截图
- "成就解锁弹窗"截图——除了标题文字本身不提供任何新信息
- 纯装饰性的游戏封面/宣传图
- 拿不准具体拍的是什么就别贴,改成贴参考攻略链接

### [rule-2/source-images] 来源攻略有图时的处理
...
## [rule-5] 规则五:长内容用 `<details><summary>` 折叠
...
## [rule-7] 规则七:不要在攻略里提数据来源

不要在攻略正文里写"勾选状态来自Steam真实解锁数据"之类的说明。

---

## [rule-8] 规则八:写攻略前的准备工作

### [8.0] 8.0 先决定写到哪个后端 —— 默认 Notion
...
## [rule-9] 规则九:用 `notion-update-page` 写入/替换内容
